...
## Library Mode patron promise

In a customized Library Mode deployment, every patron-facing recommendation must come from the configured library's owned local collection.

The deployment boundary establishes `local_collection_only` as the patron candidate-universe policy. The recommendation engine may normalize, evaluate, filter, score, rank, and select only within that universe. It must not widen the universe with Open Library, Google Books, ComicVine, Kitsu, NYT, or any other global bibliographic source.

An underfilled but relevant local slate is preferable to filling patron slots with titles the configured library does not own. A zero-result state must remain distinguishable from global-source fallback.
...
It is not part of the patron recommendation path. It must be hidden from patrons and ordinary users. Suggestions are private administrative evidence, not recommendations presented to patrons.

## Three separate concepts

### 1. Existing collection recommendation

An existing collection recommendation selects a title already owned by the configured library for a patron profile. Its candidate universe is local and closed. This is the only category permitted in customized Library Mode patron results.

### 2. Collection-gap analysis

Collection-gap analysis examines aggregate patterns indicating that the owned collection may serve some reader intents poorly. It describes a possible weakness; it does not identify a purchase automatically and does not modify patron recommendations.

### 3. Acquisition suggestion

An acquisition suggestion is an advisory librarian-facing record proposing that a specific work may address a documented collection gap. It may use global bibliographic information, but it remains outside the patron candidate universe and requires librarian judgment.

These concepts must have separate records, diagnostics, permissions, and presentation. A title appearing in an acquisition suggestion does not make it eligible for patron recommendation until it is owned, represented in the configured local collection, and admitted through the normal local-collection path.
...
- persistent absence of recommendations for a recurring reader-intent family despite sufficient aggregate demand;
- repeated dependence on a small subset of titles because the local collection lacks meaningful alternatives;
...
Persistent absence despite sufficient aggregate demand is distinct from ordinary underfill: it indicates that a recurring intent family repeatedly receives no recommendations, rather than merely receiving a smaller relevant slate.
...
- disabling the future capability must not affect patron recommendation eligibility.
...
A disposition must not silently become patron preference evidence or modify recommendation ranking. Dismissal and suppression policy will require explicit retention and audit decisions before implementation.
...
- `global_sources_allowed` and `local_collection_only` patron candidate policies;
...
The card is labeled `Planned`, contains no button or control, persists no setting, collects no data, and initiates no request. It is not rendered in patron or ordinary-user screens.
...
- changing patron recommendation routing, eligibility, scoring, ranking, or selection;
- adding global-source titles to Library Mode patron results;
...
1. A customized library deployment has a `local_collection_only` patron candidate universe.
...
4. Global-source results used for private acquisition analysis never enter patron Library Mode results.
5. A collection-gap observation is not an acquisition suggestion.
6. An acquisition suggestion is not an owned item.
7. Only owned local-collection items may become Library Mode patron candidates.
...
11. Patron interactions exist to improve reader recommendations, not to generate purchase pressure. Collection Opportunities may consume aggregated evidence derived from recommendation activity, but patron-facing workflows must never be designed primarily to influence acquisitions.
