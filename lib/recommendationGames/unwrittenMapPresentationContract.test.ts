...
test("Rain Camp result art maps by authoritative choice id with provenance", () => {
  const encounter = buildUnwrittenMapPresentationMetadata().find((item) => item.scenarioId === "rain-camp");
  assert.ok(encounter);
  const expected = {
    "crowded-table": ["rain-camp-result-crowded-table", "b6940ec312d97d61ed167009f0302653d864d9269d631f4a0cf5ad1994d68b70"],
    "paint-storm": ["rain-camp-result-paint-storm", "9389645ac74261ab007bbf98dd8e5dcda7851eb029ceed1a3d4e728a6ea42d38"],
    "sort-supplies": ["rain-camp-result-sort-supplies", "f5ab8947cc1697fe2c92837c0e45a3b8b7d5d9399c93346b9c0540b11732b228"],
    "rain-walk": ["rain-camp-result-rain-walk", "4f0c676c09a12df20f10b35d9a58bb67e575f80cb5aa2e1680fb5e3a3962181e"],
  } as const;

  for (const choice of encounter.choices) {
    const [localAssetId, sourceSha256] = expected[choice.choiceId as keyof typeof expected];
    assert.equal(RAIN_CAMP_RESULT_ASSET_IDS[choice.choiceId], localAssetId);
    assert.equal(choice.result.focalArt.localAssetId, localAssetId);
    assert.equal(choice.result.focalArt.status, "approved");
    assert.equal(choice.result.focalArt.targetAspectRatio, "3:2");
    const provenance = UNWRITTEN_MAP_FOCAL_ASSET_PROVENANCE[localAssetId];
    assert.ok(provenance);
    assert.equal(provenance.sourceSha256, sourceSha256);
    assert.equal(provenance.derivedDimensions, "1536x1024");
    const bytes = readFileSync(path.resolve(__dirname, "..", "..", choice.result.focalArt.assetPath));
    assert.equal(createHash("sha256").update(bytes).digest("hex"), provenance.derivedSha256);
  }
});

test("Kite Hill choice art maps by authoritative choice id with provenance", () => {
  const encounter = buildUnwrittenMapPresentationMetadata().find((item) => item.scenarioId === "paper-dragon");
  assert.ok(encounter);
  const expected = {
    "fly-with-dragon": ["paper-dragon-choice-fly-with-dragon", "0e744561aeb5169a64391a321f05fc23f23d8ce7b00761d0364c778b508571e8"],
    "dragon-riddle": ["paper-dragon-choice-dragon-riddle", "6b5d48f1e4da06048358288542f39f29f7203d022214b3d8832f6d74a1dd33aa"],
    "repair-tail": ["paper-dragon-choice-repair-tail", "18c49b0e9549ce2b4662efb1107e5f0533c018dbfd0439c964de98791f54a713"],
    "festival-chase": ["paper-dragon-choice-festival-chase", "8750bf114e853abb523518bccabc67745e7f5ec6c210198c897cef761ebec806"],
  } as const;

  for (const choice of encounter.choices) {
    const [localAssetId, sourceSha256] = expected[choice.choiceId as keyof typeof expected];
    assert.equal(PAPER_DRAGON_CHOICE_ASSET_IDS[choice.choiceId], localAssetId);
    assert.equal(choice.focalArt.localAssetId, localAssetId);
    assert.equal(choice.focalArt.status, "approved");
    assert.equal(choice.focalArt.targetAspectRatio, "4:3");
    assert.equal(choice.result.focalArt.status, "approved");
...
test("Old Lighthouse choice art maps by authoritative choice id with provenance", () => {
  const encounter = buildUnwrittenMapPresentationMetadata().find((item) => item.scenarioId === "old-lighthouse");
  assert.ok(encounter);
  const expected = {
    "repair-lens": ["old-lighthouse-choice-repair-lens", "5e6caf89cb2124a4f6f9b11be942640c415231e327b479c53b892fb53490f70e"],
    "keeper-journals": ["old-lighthouse-choice-keeper-journals", "d57b98cd1128fd7503717266712bf81ab12203204b34f8bb39c74b3850f25da3"],
    "storm-roof": ["old-lighthouse-choice-storm-roof", "4dc5a0764f995346a9fe28ce4accf77ac181561ba5d03de5833ce6986585b048"],
    "sea-listen": ["old-lighthouse-choice-sea-listen", "2ec99a24c773b60ac99853fff32e313becadfbe13ffb8a9d6081c4d82d868609"],
  } as const;

  for (const choice of encounter.choices) {
    const [localAssetId, sourceSha256] = expected[choice.choiceId as keyof typeof expected];
    assert.equal(OLD_LIGHTHOUSE_CHOICE_ASSET_IDS[choice.choiceId], localAssetId);
    assert.equal(choice.focalArt.localAssetId, localAssetId);
    assert.equal(choice.focalArt.status, "approved");
    assert.equal(choice.focalArt.targetAspectRatio, "4:3");
    assert.equal(choice.result.focalArt.status, "approved");
