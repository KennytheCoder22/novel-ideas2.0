...
test("missing commissioning slots block production completeness without pretending art exists", () => {
  const missing = buildUnwrittenMapPresentationMetadata().find((item) => item.scenarioId === "old-lighthouse");
  assert.ok(missing);
...
test("encounter milestone and ten complete choice/result sets are integrated while full inventory remains blocked", () => {
  ...
  assert.deepEqual(inventory, {
    ...
    choices: { required: 48, approved: 40, missing: 8 },
    results: { required: 48, approved: 40, missing: 8 },
  });
  const diagnostics = validateUnwrittenMapPresentation();
  assert.equal(diagnostics.filter((issue) => issue.code === "missing_required_asset").length, 16);
...
test("coverage summary identifies approved choice sets while every missing result remains blocked", () => {
  ...
  for (const row of summary) {
    ...
    if (row.scenarioId === "frog-parliament") {
      ...
      assert.equal(row.resultOkCount, 4);
      assert.equal(row.diagnosticCount, 0);
    } else if (row.scenarioId === "ember-library") {
      assert.equal(row.encounterOk, true);
      assert.equal(row.choiceOkCount, 4);
      assert.equal(row.resultOkCount, 4);
      assert.equal(row.diagnosticCount, 0);
    } else if (row.scenarioId === "giant-garden") {
      assert.equal(row.encounterOk, true);
      assert.equal(row.choiceOkCount, 4);
      assert.equal(row.resultOkCount, 4);
      assert.equal(row.diagnosticCount, 0);
...
test("human-readable summary reports blocked production status and exact inventory", () => {
  ...
  assert.match(summary, /Raster inventory: 92\/108 approved; 16 commissioning assets missing/);
