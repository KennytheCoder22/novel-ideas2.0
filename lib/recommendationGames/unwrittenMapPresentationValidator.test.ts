...
test("encounter milestone, five choice sets, and four result sets are complete while full inventory remains blocked", () => {
  ...
  assert.equal(diagnostics.filter((issue) => issue.code === "missing_required_asset").length, 60);
...
test("human-readable summary reports blocked production status and exact inventory", () => {
  ...
  assert.match(summary, /Raster inventory: 48\/108 approved; 60 commissioning assets missing/);
