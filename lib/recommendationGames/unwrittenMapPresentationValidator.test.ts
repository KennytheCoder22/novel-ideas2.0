...
test("missing commissioning slots block production completeness without pretending art exists", () => {
  const missing = buildUnwrittenMapPresentationMetadata().find((item) => item.scenarioId === "rain-camp");
  assert.ok(missing);
...
test("encounter milestone and six choice/result sets are complete while full inventory remains blocked", () => {
  ...
  assert.equal(diagnostics.filter((issue) => issue.code === "missing_required_asset").length, 48);
...
test("human-readable summary reports blocked production status and exact inventory", () => {
  ...
  assert.match(summary, /Raster inventory: 60\/108 approved; 48 commissioning assets missing/);
