...
function handleLeft() {
  ...
  async function handleCopyDiagnostics() {
    ...
    const sourceEnabledSummary = [
      ...
      `rawResultsPerQuery:${Array.isArray(lastDebugGcdDispatchTrace?.rawResultsPerQuery) && lastDebugGcdDispatchTrace.rawResultsPerQuery.length ? lastDebugGcdDispatchTrace.rawResultsPerQuery.map((row:any)=>`${row?.query || "(query)"}:${Number(row?.rawCount || 0)}`).join(" | ") : "(none)"}`,
      `survivingCandidatesPerQuery:${Array.isArray(lastDebugGcdDispatchTrace?.survivingCandidatesPerQuery) && lastDebugGcdDispatchTrace.survivingCandidatesPerQuery.length ? lastDebugGcdDispatchTrace.survivingCandidatesPerQuery.map((row:any)=>`${row?.query || "(query)"}:${Number(row?.survivingCandidates || 0)}`).join(" | ") : "(none)"}`,
      `keptAfterFilterPerQuery:${Array.isArray(lastDebugGcdDispatchTrace?.keptAfterFilterPerQuery) && lastDebugGcdDispatchTrace.keptAfterFilterPerQuery.length ? lastDebugGcdDispatchTrace.keptAfterFilterPerQuery.map((row:any)=>`${row?.query || "(query)"}:${Number(row?.keptCount || 0)}`).join(" | ") : "(none)"}`,
      `entityQueriesGenerated:${Array.isArray(lastDebugGcdDispatchTrace?.entityQueriesGenerated) && lastDebugGcdDispatchTrace.entityQueriesGenerated.length ? lastDebugGcdDispatchTrace.entityQueriesGenerated.join(" | ") : "(none)"}`,
