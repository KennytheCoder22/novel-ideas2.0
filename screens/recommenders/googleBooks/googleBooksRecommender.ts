...
async function fetchJsonWithRetry(url: string, timeoutMs: number, retries = 3): Promise<any> {
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      if (resp.status === 429) {
        if (attempt === retries) { const body = await resp.text().catch(() => ""); throw new Error(body ? `Google Books 429 ${body}` : "Google Books 429"); }
        await new Promise((resolve) => setTimeout(resolve, 500 * Math.pow(2, attempt))); attempt += 1; continue;
      }
      if (!resp.ok) { const body = await resp.text().catch(() => ""); throw new Error(body ? `Google Books error: ${resp.status} ${body}` : `Google Books error: ${resp.status}`); }
      return await resp.json();
    } catch (err) {
      if (attempt === retries) throw err;
      await new Promise((resolve) => setTimeout(resolve, 500 * Math.pow(2, attempt))); attempt += 1;
    } finally { clearTimeout(timer); }
  }
  throw new Error("Google Books retry loop exhausted");
}
function deckKeyToDomainMode(deckKey: DeckKey): RecommendationResult["domainMode"] { if (deckKey === "k2") return "chapterMiddle"; return "default"; }
...
function dedupeQueries(queries: string[]): string[] { const seen = new Set<string>(); const out: string[] = []; for (const query of queries) { const trimmed = String(query || "").trim(); if (!trimmed || seen.has(trimmed)) continue; seen.add(trimmed); out.push(trimmed); } return out; }
...
function toGoogleBooksQuery(query: string): string { const q = collapseRepeatedTerms(String(query || "").toLowerCase().trim()); if (!q) return ""; if (q.startsWith("subject:")) return q; return q; }
async function googleBooksSearch(query: string, limit: number, timeoutMs: number): Promise<any[]> {
  const q = toGoogleBooksQuery(query); if (!q) return [];
  const maxResults = Math.max(1, Math.min(40, Number(limit) || 10)); const apiKey = getGoogleBooksApiKey();
  ...
  const json = await fetchJsonWithRetry(url, timeoutMs); const items = Array.isArray(json?.items) ? json.items : [];
  return items.map((item: any) => {
    const volumeInfo = item?.volumeInfo ?? {}; const accessInfo = item?.accessInfo ?? {}; const saleInfo = item?.saleInfo ?? {}; const searchInfo = item?.searchInfo ?? {}; const authors = Array.isArray(volumeInfo.authors) ? volumeInfo.authors : []; const categories = Array.isArray(volumeInfo.categories) ? volumeInfo.categories : []; const imageLinks = volumeInfo.imageLinks ?? {};
    return { id: item?.id, key: item?.id, title: volumeInfo.title, subtitle: volumeInfo.subtitle, author_name: authors.length ? authors : undefined, first_publish_year: typeof volumeInfo.publishedDate === "string" && /^\d{4}/.test(volumeInfo.publishedDate) ? Number(volumeInfo.publishedDate.slice(0, 4)) : undefined, publisher: volumeInfo.publisher, description: typeof volumeInfo.description === "string" ? volumeInfo.description : typeof searchInfo?.textSnippet === "string" ? searchInfo.textSnippet : undefined, averageRating: typeof volumeInfo.averageRating === "number" ? volumeInfo.averageRating : undefined, ratingsCount: typeof volumeInfo.ratingsCount === "number" ? volumeInfo.ratingsCount : undefined, pageCount: typeof volumeInfo.pageCount === "number" ? volumeInfo.pageCount : undefined, edition_count: 1, cover_i: imageLinks.thumbnail || imageLinks.smallThumbnail, subject: categories.length ? categories : undefined, subjects: categories.length ? categories : undefined, categories, language: typeof volumeInfo.language === "string" ? [volumeInfo.language] : undefined, ebook_access: accessInfo?.epub?.isAvailable ? "epub" : accessInfo?.pdf?.isAvailable ? "pdf" : saleInfo?.isEbook ? "ebook" : "no_ebook", volumeInfo };
...
export async function getGoogleBooksRecommendations(input: RecommenderInput): Promise<RecommendationResult> {
  const deckKey = input.deckKey; const finalLimit = Math.max(1, Math.min(40, input.limit ?? 12)); const fetchLimit = Math.max(60, Math.min(200, finalLimit * 6)); const timeoutMs = Math.max(1000, Math.min(30000, input.timeoutMs ?? 15000)); const domainMode = deckKeyToDomainMode(deckKey);
  const explicitBucketPlan = (input as any)?.bucketPlan as { queries?: string[]; rungs?: StructuredFetchRung[] } | undefined;
  const planQueries = Array.isArray(explicitBucketPlan?.rungs) && explicitBucketPlan!.rungs!.length > 0 ? explicitBucketPlan!.rungs!.map(rungToGoogleBooksQuery) : (Array.isArray(explicitBucketPlan?.queries) && explicitBucketPlan?.queries?.length ? dedupeQueries(explicitBucketPlan.queries) : getBucketQueries(deckKey, input));
  const queriesToTry = planQueries.length ? planQueries : getBucketQueries(deckKey, input);
  const builtFromQuery = toGoogleBooksQuery(queriesToTry[0] || "");
  ...
  const docsRaw = collectedDocsRaw.length ? collectedDocsRaw : primaryDocsRaw.map((doc: any) => ({ ...doc, queryRung: 0, queryText: builtFromQuery, source: "googleBooks" }));
  const docs: RecommendationDoc[] = docsRaw.filter((doc: any) => doc && doc.title).map((doc: any) => ({ key: doc.key ?? doc.id, title: doc.title, author_name: Array.isArray(doc.author_name) ? doc.author_name : undefined, first_publish_year: typeof doc.first_publish_year === "number" ? doc.first_publish_year : undefined, cover_i: doc.cover_i, subject: Array.isArray(doc.subject) ? doc.subject : Array.isArray(doc.subjects) ? doc.subjects : Array.isArray(doc.categories) ? doc.categories : Array.isArray(doc.volumeInfo?.categories) ? doc.volumeInfo.categories : undefined, edition_count: typeof doc.edition_count === "number" ? doc.edition_count : typeof doc.editionCount === "number" ? doc.editionCount : undefined, publisher: doc.publisher, language: Array.isArray(doc.language) ? doc.language : typeof doc.volumeInfo?.language === "string" ? [doc.volumeInfo.language] : undefined, ebook_access: typeof doc.ebook_access === "string" ? doc.ebook_access : undefined, source: "googleBooks", queryRung: Number.isFinite(Number(doc.queryRung)) ? Number(doc.queryRung) : undefined, queryText: typeof doc.queryText === "string" ? doc.queryText : undefined, subtitle: typeof doc.subtitle === "string" ? doc.subtitle : undefined, description: typeof doc.description === "string" ? doc.description : undefined, averageRating: typeof doc.averageRating === "number" ? doc.averageRating : undefined, ratingsCount: typeof doc.ratingsCount === "number" ? doc.ratingsCount : undefined, volumeInfo: doc.volumeInfo } as any));
  return { engineId: "googleBooks", engineLabel: "Google Books", deckKey, domainMode, builtFromQuery, items: docs.map((doc) => ({ kind: "open_library", doc })) };
}

// PATCH APPLIED: film/criticism + series suppression filters added
