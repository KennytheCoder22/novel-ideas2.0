...
async function fetchDocsForQuery(query: string, queryRung: number, timeoutMs: number, fetchLimit: number, docs: RecommendationDoc[], seen: Set<string>) {
  ...
  try {
    ...
    return { rawCount: results.length, acceptedCount, rejectedCount: Math.max(0, results.length - acceptedCount), topTitles, rejectedReasons, stageCounts, error: null };
  } catch (err: any) {
    return {
      rawCount: 0,
      acceptedCount: 0,
...
export async function getGcdGraphicNovelRecommendations(input: RecommenderInput): Promise<RecommendationResult> {
  ...
  for (let i = 0; i < maxQueriesToFetch; i += 1) {
    ...
    const { rawCount, acceptedCount, rejectedCount, topTitles, rejectedReasons, stageCounts, error } = await fetchDocsForQuery(q, i, timeoutMs, fetchLimit, docs, seen);
