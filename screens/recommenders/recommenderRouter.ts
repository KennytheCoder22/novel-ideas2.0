...
export async function getRecommendations(
  ...
  let generatedComicVineQueriesFromTaste = Array.from(new Set([
    ...
    .filter(Boolean)
    .slice(0, 10);
  const superheroCompatibleSignal = [
    ...genres,
    ...themes,
    ...drives,
    ...moods,
  ...
  const bigTwoStoryArcExpansionEligible = /\b(superhero|heroes|comic|comics|marvel|dc|team|avengers|x-men|justice|spider|batman|superman|wonder woman|captain america|thor)\b/.test(superheroCompatibleSignal);
  if (bigTwoStoryArcExpansionEligible) {
    const bigTwoStoryArcQueries = [
      "Batman graphic novel",
      "Superman graphic novel",
      "Wonder Woman graphic novel",
...
const normalizedCandidatesRaw = [
  ...
  function canReturnTitleRejectReason(title: string, doc?: any): string | null {
    ...
    const bigTwoCanonicalRoots = new Set(["batman", "superman", "wonder-woman", "captain-america", "thor", "avengers", "spider-man", "x-men", "ms-marvel", "runaways", "sweet-tooth", "descender", "teen-titans"]);
    const singleSourceComicVineTopUpSafe =
      includeComicVine &&
      !sourceEnabled.googleBooks &&
      !sourceEnabled.openLibrary &&
      !sourceEnabled.localLibrary &&
      !includeKitsu &&
      collectedEditionLike &&
      bigTwoCanonicalRoots.has(root) &&
      !explicitIssueOrChapterMarker;
    if (!key) return "missing_title";
    if (terminalRejectReasonByTitle[key]) return `terminal_reject:${terminalRejectReasonByTitle[key]}`;
    if (lateFillNeverReturnTitles.has(key) && !singleSourceComicVineTopUpSafe && !(titleFallbackLike && bigTwoCanonicalRoots.has(root) && !explicitIssueOrChapterMarker)) return "late_fill_never_return";
