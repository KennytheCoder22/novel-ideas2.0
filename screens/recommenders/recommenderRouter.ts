...
const normalizedCandidatesRaw = [
  ...
  const kitsuTeenSafeButSuppressedTitles: string[] = [];
  const kitsuTeenAcceptableUnderfillTitles: string[] = [];
  ...
  const teenKitsuKnownAcceptableUnderfillTitlePattern = /\b(?:mystery\s+to\s+iu\s+nakare|roppyaku\s+page\s+no\s+mystery|yami\s+no\s+koe|black\s+paradox)\b/i;
  ...
  const isTeenKitsuKnownAcceptableUnderfillRow = (row: any) => {
    const title = String(row?.title || row?.doc?.title || "").trim();
    if (!teenKitsuKnownAcceptableUnderfillTitlePattern.test(title)) return false;
    return /^(mystery|horror|thriller)$/.test(String(routerFamily || ""));
  };
  const hasTeenKitsuPositiveEvidence = (row: any) =>
    Number(row?.semanticEvidenceCount || 0) > 0 || Number(row?.weightedTasteScore || 0) > 0 || Number(row?.positiveFitScore || 0) > 0;
  const isTeenKitsuHighConfidenceRescueRow = (row: any) => {
    if (!isTeenKitsuRescueContext) return false;
    if (!row?.sourceId) return false;
    if (Number(row?.positiveFitScore || 0) < -2) return false;
    if (row?.laneAligned && Number(row?.positiveFitScore || 0) >= -1) return true;
  ...
  const teenKitsuRejectedReasonForRow = (row: any) => {
    ...
    if (Number(row.positiveFitScore || 0) < -2) return `strong_negative_positive_fit_score:${Number(row.positiveFitScore || 0).toFixed(2)}`;
    if (Number(row.positiveFitScore || 0) < 0 && !hasTeenKitsuPositiveEvidence(row) && !row.laneAligned && !row.familyAligned) return `negative_positive_fit_score:${Number(row.positiveFitScore || 0).toFixed(2)}`;
    if (!row.laneAligned && !row.familyAligned && !isTeenKitsuKnownAcceptableUnderfillRow(row)) return "no_lane_or_router_family_alignment";
    return "";
  };
  const teenKitsuRescueTierForRow = (row: any) => {
    ...
    if (row?.familyAligned && hasTeenKitsuPositiveEvidence(row)) return { tier: "acceptable_underfill", rejectedReason: "" };
    if (isTeenKitsuKnownAcceptableUnderfillRow(row)) return { tier: "acceptable_underfill", rejectedReason: "" };
  ...
  const applyTeenKitsuRescuePolicy = (rows: any[], source: string) => {
    ...
    for (const row of rows) {
      ...
      else if (checked.teenKitsuRescueTier === "acceptable_underfill") acceptableUnderfill.push(checked);
    ...
    kitsuTeenRescueFamilyAlignedCount = Math.max(kitsuTeenRescueFamilyAlignedCount, [...highConfidence, ...acceptableUnderfill, ...weakFamilyFallback].filter((row: any) => Boolean(row.familyAligned)).length);
    ...
    const acceptableCap = Math.min(3, Math.max(1, finalLimit || 1));
    const cappedAcceptable = acceptableUnderfill.slice(0, acceptableCap);
    if (acceptableUnderfill.length > cappedAcceptable.length) kitsuTeenRescueWeakFallbackCapApplied = true;
    if (cappedAcceptable.length > 0) {
      kitsuTeenRescueWeakFallbackCount = Math.max(kitsuTeenRescueWeakFallbackCount, cappedAcceptable.length);
      kitsuTeenRescueCandidateWeakButReturnedReason = `acceptable_underfill:${source}`;
      for (const row of cappedAcceptable) pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenAcceptableUnderfillTitles, row.title);
      return cappedAcceptable;
    }
  ...
  if (shouldApplyTeenKitsuFinalRescueGuard) {
    ...
    const acceptableUnderfillItems: any[] = [];
    const weakFallbackItems: any[] = [];
    for (const item of finalOutputItems as any[]) {
      const doc = item?.doc || item;
      const title = String(doc?.title || item?.title || "").trim();
      if (title) pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenRescueFinalGuardInputTitles, title, 60);
      const source = detectCandidateSourceForGate(item);
      if (source !== "kitsu") {
        nonKitsuItems.push(item);
        if (title) pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenRescueFinalGuardAcceptedTitles, title, 60);
        continue;
      }
      ...
      if (checked.teenKitsuRejectReason) {
        ...
        pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenRescueFinalGuardSuppressedTitles, checked.title, 60);
        if (checked.title) kitsuTeenRescueFinalGuardSuppressedReasonByTitle[checked.title] = checked.teenKitsuRejectReason;
        continue;
      }
      if (checked.teenKitsuRescueTier === "high_confidence") highConfidenceItems.push(item);
      else if (checked.teenKitsuRescueTier === "acceptable_underfill") {
        acceptableUnderfillItems.push(item);
        pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenAcceptableUnderfillTitles, checked.title);
      } else {
    ...
    const acceptableCap = Math.min(3, Math.max(1, finalLimit || 1));
    const cappedAcceptableUnderfillItems = highConfidenceItems.length > 0 ? [] : acceptableUnderfillItems.slice(0, acceptableCap);
    const weakCap = Math.min(2, Math.max(1, finalLimit || 1));
    const cappedWeakFallbackItems = highConfidenceItems.length > 0 || cappedAcceptableUnderfillItems.length > 0 ? [] : weakFallbackItems.slice(0, weakCap);
    const suppressedAcceptable = acceptableUnderfillItems.slice(cappedAcceptableUnderfillItems.length);
    const suppressedWeak = weakFallbackItems.slice(cappedWeakFallbackItems.length);
    if (suppressedAcceptable.length > 0 || suppressedWeak.length > 0) kitsuTeenRescueWeakFallbackCapApplied = true;
    for (const item of [...suppressedAcceptable, ...suppressedWeak]) {
      const doc = item?.doc || item;
      const title = String(doc?.title || item?.title || "").trim();
      pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenRescueFinalGuardSuppressedTitles, title, 60);
      if (title) kitsuTeenRescueFinalGuardSuppressedReasonByTitle[title] = "underfill_cap_or_higher_tier_available";
      pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenSafeButSuppressedTitles, title);
    }
    kitsuTeenRescueHighConfidenceCount = Math.max(kitsuTeenRescueHighConfidenceCount, highConfidenceItems.length);
    kitsuTeenRescueWeakFallbackCount = Math.max(kitsuTeenRescueWeakFallbackCount, cappedAcceptableUnderfillItems.length + cappedWeakFallbackItems.length);
    const guardedItems = [...nonKitsuItems, ...highConfidenceItems, ...cappedAcceptableUnderfillItems, ...cappedWeakFallbackItems];
    for (const item of guardedItems) {
      const doc = item?.doc || item;
      const title = String(doc?.title || item?.title || "").trim();
      pushUniqueTeenKitsuDiagnosticTitle(kitsuTeenRescueFinalGuardAcceptedTitles, title, 60);
    }
    ...
    if (highConfidenceItems.length === 0 && cappedAcceptableUnderfillItems.length > 0) {
      returnedItemsBuiltFrom = `${String(returnedItemsBuiltFrom || "kitsu_rescue")}_acceptable_underfill`;
      finalReturnSourceUsed = returnedItemsBuiltFrom;
      kitsuTeenRescueCandidateWeakButReturnedReason = "acceptable_underfill:final_rescue_guard";
      sourceSkippedReason.push(`teen_kitsu_rescue_acceptable_underfill_returned:${cappedAcceptableUnderfillItems.length}`);
  ...
  return {
    ...
    kitsuTeenSafeButSuppressedTitles,
    kitsuTeenAcceptableUnderfillTitles,
