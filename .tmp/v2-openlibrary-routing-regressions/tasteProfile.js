...
function adultTastePrimaryContentFamily(rawValue) {
    const value = normalizeSignal(rawValue);
    if (!value || ADULT_TASTE_GENERIC_SIGNAL.test(value) || ADULT_TASTE_CONTEXT_ONLY_SIGNAL.test(value) || ADULT_TASTE_SUPPLEMENTAL_SIGNAL.test(value))
        return "";
...
function roundAdultTasteWeight(value) {
    return Math.round(value * 1000) / 1000;
}
function adultTasteContributionMultiplier(field) {
    if (field === "genres")
        return 1;
    if (field === "tags")
        return 0.5;
...
function adultTasteEvidenceLevel(field, canonicalTag) {
    if (field !== "genres")
        return "subfamily_or_tone_level";
    return /\s|-/.test(canonicalTag) ? "subfamily_or_tone_level" : "family_level";
}
function adultTasteFamilyContribution(rawValue, field, map, baseWeight, action, title) {
...
function adultTastePolarityDecision(positiveWeight, negativeWeight, positiveCount, negativeCount) {
    ...
    if (positiveWeight <= 0 && negativeWeight <= 0)
        return { decision: "insufficient_evidence", reason: "no_liked_or_disliked_family_evidence" };
    if (negativeWeight > 0 && positiveWeight <= 0)
        return { decision: "true_avoid", reason: "only_disliked_family_evidence" };
    ...
    if (positiveWeight > 0 && negativeWeight <= 0 && positiveCount >= 2 && positiveWeight >= 2)
        return { decision: "strongly_liked", reason: "multiple_liked_titles_no_negative_family_evidence" };
    if (positiveWeight > 0 && negativeWeight <= 0)
        return { decision: "weakly_liked", reason: "liked_family_evidence_no_negative_family_evidence" };
    if (net > 0)
        return { decision: "mixed_positive", reason: "liked_weight_exceeds_disliked_weight" };
    if (net < 0)
        return { decision: "mixed_negative", reason: "disliked_weight_exceeds_liked_weight_but_positive_evidence_exists" };
    return { decision: "mixed_neutral", reason: "liked_and_disliked_family_weights_tie" };
}
function buildAdultTasteFamilyDiagnostics(session, currentLikedFamilies, currentAvoidFamilies) {
    if (session.ageBand !== "adult")
        return {};
    const evidenceBySwipe = {};
    const positiveContributionBySwipe = {};
    const negativeContributionBySwipe = {};
    const reasonBySwipe = {};
    const sourceTagsBySwipe = {};
    const familyPositiveByTitle = {};
    const familyNegativeByTitle = {};
    const familyPositiveEvidenceLevelByTitle = {};
    const familyNegativeEvidenceLevelByTitle = {};
    const skippedTitlesByFamily = {};
    const skippedTitles = new Set();
    const addBestContribution = (store, evidenceStore, family, title, weight, evidenceLevel) => {
        if (!store[family])
            store[family] = {};
        store[family][title] = Math.max(Number(store[family][title] || 0), roundAdultTasteWeight(Math.abs(weight)));
        if (!evidenceStore[family])
            evidenceStore[family] = {};
        if (!evidenceStore[family][title])
            evidenceStore[family][title] = new Set();
    ...
    for (const signal of session.signals || []) {
        ...
        const contributions = [];
        const addContribution = (rawValue, field, map) => {
        ...
        for (const value of signal.genres || [])
            addContribution(value, "genres", signal.action === "dislike" ? "avoidSignals" : "genreFamily");
        for (const value of signal.tones || [])
            addContribution(value, "tones", signal.action === "dislike" ? "avoidSignals" : "tone");
        for (const value of signal.themes || [])
            addContribution(value, "themes", signal.action === "dislike" ? "avoidSignals" : "themes");
        for (const value of signal.characterDynamics || [])
            addContribution(value, "characterDynamics", signal.action === "dislike" ? "avoidSignals" : "characterDynamics");
        ...
        if (signal.format)
            addContribution(signal.format, "format", signal.action === "dislike" ? "avoidSignals" : "formatPreference");
    ...
    const netWeight = {};
    const likedTitlesByFamily = {};
    const dislikedTitlesByFamily = {};
    const positiveCount = {};
    const negativeCount = {};
    const polarityDecision = {};
    const polarityReason = {};
    const familyContributionEvidenceLevel = {};
    ...
    const productionMixedPositiveFamilies = [];
    const productionMixedNeutralFamilies = [];
    const productionMixedNegativeFamilies = [];
    const productionResolutionReasonByFamily = {};
    for (const family of families) {
        ...
        if (!overlapsCurrentProfile) {
            if (currentLikedSet.has(family))
                productionResolutionReasonByFamily[family] = "non_overlapping_family_keeps_current_liked_profile_behavior";
            else if (currentAvoidSet.has(family))
                productionResolutionReasonByFamily[family] = "non_overlapping_family_keeps_current_avoid_profile_behavior";
            else
                productionResolutionReasonByFamily[family] = "family_not_present_in_current_production_profile";
            continue;
        }
        if (decision === "mixed_positive" || decision === "strongly_liked" || decision === "weakly_liked") {
            ...
            if (decision === "mixed_positive")
                productionMixedPositiveFamilies.push(family);
    ...
    const productionPolarityExplanationByFamily = Object.fromEntries(families.map((family) => {
        ...
        const thresholdComparison = decision === "true_avoid" && positive <= 0
    ...
    const mixedFamilyProductionExplanationByFamily = Object.fromEntries(Object.entries(productionPolarityExplanationByFamily).filter(([, value]) => {
        const decision = String(value.decision || "");
        return decision === "insufficient_evidence" || /^mixed_/.test(decision);
    }));
    const productionPolarityRuleHistogram = {};
    const mixedFamilyProductionRuleHistogram = {};
    for (const family of families) {
    ...
    return {
        ...
        adultTasteWeightedChangedFamilies: families.filter((family) => currentLikedSet.has(family) !== weightedLikedSet.has(family)
...
function buildTasteProfile(session) {
    ...
    for (const signal of session.signals || []) {
        ...
        if (SOURCE_HINTS.has(source) && !adultSkip)
