...
exports.googleBooksSourceAdapter = void 0;
exports.googleBooksAdultNarrativeFetchQuery = googleBooksAdultNarrativeFetchQuery;
exports.queryFamilyFromQuery = queryFamilyFromQuery;
exports.googleBooksPublicationShapeDropReason = googleBooksPublicationShapeDropReason;
exports.inferGoogleBooksPublicationShape = inferGoogleBooksPublicationShape;
exports.analyzeGoogleBooksVolumeForAudit = analyzeGoogleBooksVolumeForAudit;
...
const GOOGLE_BOOKS_NON_NARRATIVE_SHAPES = new Set([
    "periodical",
    "reference",
    "critical_study",
    "academic_text",
...
const GOOGLE_BOOKS_NARRATIVE_SHAPES = new Set([
    "novel",
    "series_installment",
    "story_collection",
]);
...
function collectGoogleBooksContributorFields(volumeInfo) {
    const contributorFields = {};
    for (const [field, value] of Object.entries(volumeInfo)) {
        if (!/(author|contributor|editor|narrator|illustrator|translator|creator)/i.test(field))
            continue;
        if (Array.isArray(value)) {
            ...
            if (extracted.length > 0)
                contributorFields[field] = extracted;
            continue;
        }
        const single = String(value || "").trim();
        if (single)
            contributorFields[field] = [single];
    }
    return contributorFields;
}
function preNormalizationDispositionForDropReason(reason) {
    if (!reason)
        return "metadata_insufficient";
    if (reason === "missing_author")
        return "metadata_insufficient";
    if (reason.startsWith("publication_shape_unknown_insufficient_"))
        return "parser_improvement";
    if (reason === "publication_shape_unknown_insufficient_story_evidence")
        return "parser_improvement";
    if (reason.startsWith("publication_shape_writing_guide")
...
function likelyNarrativeFictionForDropReason(reason, unknownNarrativeCorroboration) {
    if (!reason)
        return "ambiguous";
    if (reason.startsWith("publication_shape_writing_guide")
        || reason.startsWith("publication_shape_critical_study")
    ...
    if (reason === "publication_shape_unknown_insufficient_story_evidence")
        return "ambiguous";
    if (reason === "publication_shape_anthology")
        return "ambiguous";
    if (reason === "missing_author")
        return "ambiguous";
    return "ambiguous";
}
function smallestAdditionalEvidenceForAdmission(reason, thresholdFailed) {
    if (reason === "publication_shape_unknown_insufficient_story_evidence") {
...
function shapeHistogramCount(histogram, shapes) {
    return shapes.reduce((sum, shape) => sum + Number(histogram[shape] || 0), 0);
}
function clampShapeScore(value) {
...
function inferGoogleBooksAudienceBand(params) {
    ...
    if (!text)
        return "unknown";
    if (/\b(young adult|ya\b|teen(?:s|age|ager)?|high school|new adult)\b/.test(text))
        return "teens";
    if (/\b(adult fiction|literary criticism|history and criticism|critical studies?|poetry|poems?|essays?|academic|monograph|scholarship)\b/.test(text))
        return "adult";
    if (/\b(middle grade|grades?\s*[3-8]|ages?\s*(?:8|9|10|11|12)\b)\b/.test(text))
        return "preteens";
    if (/\b(juvenile fiction|children'?s fiction|picture books?|picture book|early readers?|easy readers?|beginning readers?|read aloud|ages?\s*(?:3|4|5|6|7|8)\b|grades?\s*(?:k|1|2)\b|kindergarten|preschool)\b/.test(text))
        return "kids";
    return "unknown";
}
...
function uniqueQueryParts(values, limit = 24) {
    const seen = new Set();
    const output = [];
...
function googleBooksAdultNarrativeFetchQuery(query) {
    ...
    const quotedPhrases = phrases
        ...
        .map(({ quoted }) => quoted);
    const phraseWords = new Set(phrases
        .filter(({ phrase }) => normalized.includes(phrase))
...
function normalizePublicationIdentityTitle(value) {
    return normalizeText(value).replace(/^(?:the|a|an)\s+/, "").trim();
}
function queryFamilyFromQuery(query) {
    const normalized = normalizeQuery(query);
    if (/\b(thriller|suspense|conspiracy|manhunt|abduction)\b/.test(normalized))
        return "thriller";
    if (/\b(mystery|detective|whodunit|private investigator)\b/.test(normalized))
        return "mystery";
    if (/\b(horror|haunted|occult|ghost|supernatural)\b/.test(normalized))
        return "horror";
    if (/\b(science fiction|dystopian|space opera|speculative)\b/.test(normalized))
        return "science_fiction";
    if (/\b(fantasy|magic|dragon|gothic fantasy)\b/.test(normalized))
        return "fantasy";
    if (/\b(romance|love story|relationship)\b/.test(normalized))
        return "romance";
    if (/\b(historical|period fiction|civil war|19th century)\b/.test(normalized))
        return "historical";
    return "general";
}
...
function descriptionFromVolume(row, volumeInfo) {
    if (typeof volumeInfo.description === "string")
        return volumeInfo.description;
    if (typeof row.searchInfo?.textSnippet === "string") {
        return String(row.searchInfo.textSnippet);
    }
    return "";
}
...
function hasFictionCategoryEvidence(categories) {
...
function hasNarrativeDescriptionEvidence(description) {
...
function hasFictionPublisherEvidence(publisher) {
...
function googleBooksGenericCategoryTitleEvidence(titleText) {
    const evidence = [];
...
function googleBooksSubjectOfStudyTitleEvidence(titleText) {
    const evidence = [];
    const normalized = normalizeText(titleText)
        .replace(/[-/&:|]+/g, " ")
        .replace(/\s+/g, " ")
...
function googleBooksBundledPublicationEvidence(titleText, descriptionText, categoryBlob) {
    const evidence = [];
    if (!/\bmegapack\b/.test(titleText))
        return evidence;
...
function googleBooksAnnualAnthologyEvidence(titleText, descriptionText, categoryBlob) {
    const evidence = [];
    const annualBestAmericanTitle = /^the best american .+ (?:19|20)\d{2}$/.test(titleText);
    if (!annualBestAmericanTitle)
        return evidence;
...
function googleBooksCuratedBookGuideEvidence(titleText, descriptionText, categoryBlob) {
    const evidence = [];
...
function googleBooksPeriodicalIdentityEvidence(titleText, subtitleText, normalizedDescription, categoriesText, publisherText, combined) {
    const titleSubtitle = normalizeText([titleText, subtitleText].filter(Boolean).join(" "));
    const titleSignals = [];
    const corroboration = [];
    if (/\bmagazine\b/.test(titleSubtitle))
        titleSignals.push("title_magazine_term");
    if (/\bjournal\b/.test(titleSubtitle))
        titleSignals.push("title_journal_term");
    if (/\bbulletin\b/.test(titleSubtitle))
        titleSignals.push("title_bulletin_term");
    if (/\bquarterly\b/.test(titleSubtitle))
        titleSignals.push("title_quarterly_term");
    if (/\bgazette\b/.test(titleSubtitle))
        titleSignals.push("title_gazette_term");
    if (/\breview\b/.test(titleSubtitle))
        titleSignals.push("title_review_term");
    if (/\bmercury\b/.test(titleSubtitle))
        titleSignals.push("title_mercury_periodical_style_term");
...
function googleBooksPublicationShapeDropReason(analysis) {
    if (GOOGLE_BOOKS_NON_NARRATIVE_SHAPES.has(analysis.shape))
        return `publication_shape_${analysis.shape}`;
    if (analysis.shape === "anthology" || analysis.shape === "essay_collection")
        return `publication_shape_${analysis.shape}`;
    if (analysis.shape === "unknown" && !analysis.unknownShapeEligibility)
        return analysis.unknownShapeRejectedReason || "publication_shape_unknown_insufficient_narrative_identity";
    return undefined;
}
function preteenGoogleBooksPublicationShapeAuditRecord(params) {
    const identityAudit = (0, preteenGoogleBooksPublicationIdentity_1.preteenGoogleBooksPublicationIdentityAudit)({
        title: params.title,
    ...
    const strongPreteenNarrativeIdentity = identityAudit.allowed && (identityAudit.narrativeEvidence.some((item) => /middle_grade|juvenile_fiction|chapter_book|graphic_novel/.test(item))
    ...
    let disposition = "ambiguous_reject";
...
function preteenGoogleBooksPublicationShapeRescueDecision(params) {
    ...
    if (titleHasStructuredNarrativeIdentity)
        evidenceFamilies.push("title_subtitle_narrative_identity");
    ...
    if (bibliographicSignals >= 4)
        evidenceFamilies.push("normal_book_bibliographic_metadata");
...
function kidsGoogleBooksPublicationAuditRecord(params) {
    ...
    let counterfactualDecision = "ambiguous";
...
function inferGoogleBooksPublicationShape(params) {
    ...
    const evidence = [];
    const weakNarrativeEvidence = [];
    const storyLevelNarrativeEvidence = [];
    ...
    const explicitShapeCandidates = [];
    if (periodicalShape)
        explicitShapeCandidates.push({ shape: "periodical", evidence: periodicalIdentityEvidence, decision: periodicalIdentityDecision });
    ...
    if (readersAdvisoryShape)
        explicitShapeCandidates.push({ shape: "readers_advisory", evidence: curatedBookGuideIdentity ? curatedBookGuideEvidence : ["readers_advisory_publication_shape"], decision: "readers_advisory_identity_overrides_narrative_signals" });
    if (genericCategoryTitle)
        explicitShapeCandidates.push({ shape: "generic_category_catalog", evidence: genericCategoryEvidence, decision: "generic_category_title_overrides_narrative_signals" });
    if (sneakPreviewSampleShape)
        explicitShapeCandidates.push({ shape: "reference", evidence: ["promotional_sneak_preview_title_shape"], decision: "sneak_preview_promotional_identity_overrides_narrative_signals" });
    if (referenceShape)
        explicitShapeCandidates.push({ shape: "reference", evidence: ["reference_publication_shape"], decision: "reference_identity_overrides_narrative_signals" });
    if (productionHistoryShape)
        explicitShapeCandidates.push({ shape: "production_history", evidence: ["making_of_or_production_history_shape"], decision: "production_history_identity_overrides_narrative_signals" });
    if ((criticismShape || readingStudyShape || quotedWorkStudyShape) && academicPublisher)
        explicitShapeCandidates.push({ shape: "academic_text", evidence: ["academic_publisher_criticism_shape"], decision: "academic_scholarship_identity_overrides_narrative_signals" });
    else if (criticismShape || conceptStudyShape || readingStudyShape || quotedWorkStudyShape)
        explicitShapeCandidates.push({ shape: "critical_study", evidence: [quotedWorkStudyShape ? "quoted_work_academic_comparison_shape" : readingStudyShape ? "reading_title_study_shape" : conceptStudyShape ? "conceptual_genre_study_shape" : "criticism_or_scholarship_shape"], decision: "criticism_or_scholarship_identity_overrides_narrative_signals" });
    if (subjectOfStudyTitle)
        explicitShapeCandidates.push({ shape: "literary_history", evidence: subjectOfStudyEvidence, decision: "subject_of_study_title_overrides_narrative_signals" });
    if (interviewShape)
        explicitShapeCandidates.push({ shape: "interview_collection", evidence: ["interview_collection_shape"], decision: "interview_collection_identity_overrides_narrative_signals" });
    if (commentaryShape)
        explicitShapeCandidates.push({ shape: "author_commentary", evidence: ["author_or_work_commentary_shape"], decision: "commentary_about_work_or_author_overrides_narrative_signals" });
    if (genreSurveyShape)
        explicitShapeCandidates.push({ shape: "genre_survey", evidence: ["genre_survey_shape"], decision: "genre_survey_identity_overrides_narrative_signals" });
    if (literaryHistoryShape)
        explicitShapeCandidates.push({ shape: "literary_history", evidence: ["literary_history_shape"], decision: "literary_history_identity_overrides_narrative_signals" });
    if (anthologyShape)
        explicitShapeCandidates.push({ shape: "anthology", evidence: anthologyEvidence.length ? anthologyEvidence : ["anthology_or_omnibus_shape"], decision: "anthology_identity_overrides_narrative_signals" });
    if (essayCollectionShape)
        explicitShapeCandidates.push({ shape: "essay_collection", evidence: ["essay_collection_shape"], decision: "essay_collection_identity_overrides_narrative_signals" });
    if (miscellanyShape)
        explicitShapeCandidates.push({ shape: "miscellany", evidence: ["miscellany_or_digest_shape"], decision: "miscellany_identity_overrides_narrative_signals" });
    if (publicDomainCompilationShape)
        explicitShapeCandidates.push({ shape: "public_domain_compilation", evidence: ["public_domain_compilation_shape"], decision: "public_domain_compilation_identity_overrides_narrative_signals" });
    if (nonfictionShape)
        explicitShapeCandidates.push({ shape: "nonfiction", evidence: ["nonfiction_category_without_fiction_shape"], decision: "nonfiction_identity_overrides_narrative_signals" });
    ...
    const dominantPublicationShapeEvidence = [];
    const explicitNonNarrativeIdentity = [];
    let unknownShapeEligibility = false;
    const unknownShapeEvidence = [];
    ...
    const unknownNarrativeCorroboration = [];
    ...
    const distinctiveNarrativeTitle = titleText.split(/\s+/).filter(Boolean).length >= 2
        ...
        && !subjectOfStudyTitle;
    if (novelIdentity)
        unknownNarrativeCorroboration.push("explicit_novel_identity");
    if (seriesInstallmentIdentity)
        unknownNarrativeCorroboration.push("series_installment_identity");
    ...
    if (shape === "novel")
        narrativePriorityAdjustment += 5;
    else if (shape === "series_installment")
        narrativePriorityAdjustment += 3;
    else if (shape === "story_collection")
        narrativePriorityAdjustment -= 2;
    else if (shape === "anthology" || shape === "essay_collection")
        narrativePriorityAdjustment -= 6;
    else if (GOOGLE_BOOKS_NON_NARRATIVE_SHAPES.has(shape))
        narrativePriorityAdjustment -= 10;
    if (narrativeConfidence >= 5)
        narrativePriorityAdjustment += 1;
...
function googleBooksPeriodicalCorroboration(titleText, subtitleText, normalizedDescription, categoriesText, combined) {
    return googleBooksPeriodicalIdentityEvidence(titleText, subtitleText, normalizedDescription, categoriesText, "", combined);
}
function googleBooksArtifactReasons(title, subtitle, description, categories, publisher) {
    const reasons = [];
    ...
    if (annualAnthologyPhrase.test(titleText)
    ...
    if (/\b(writer'?s market|writers'? handbook|guide to literary agents|children'?s writer'?s and illustrator'?s market|places to sell manuscripts?|markets?\s+for\s+writ(?:er|ers)|manuscript markets?|publishing opportunities|literary agents?\s+guide|writer directory|submission guide)\b/.test(combined)) {
    ...
    if (/\bthrough\s+(?:(?:\w+\s+){0,5})(?:literature|fiction)\b/.test(titleText)
...
function googleBooksArtifactDropReason(title, subtitle, description, categories, publisher) {
    return googleBooksArtifactReasons(title, subtitle, description, categories, publisher)[0];
}
function buildGoogleBooksFetchQuery(query, ageBand) {
...
function isPublicDomainCatalogShape(title, publicationYear, description, categories) {
...
function isModernNarrativeRecord(title, publicationYear, description, categories, isbnPresent) {
    const normalizedDescription = normalizeText(description);
    const normalizedTitle = normalizeText(title);
...
async function fetchGoogleBooksJson(query, timeoutMs, signal) {
...
function emptyDiagnostics(plan, status, startedAt, overrides) {
 ...
 */
function analyzeGoogleBooksVolumeForAudit(volumeInfo, item) {
    ...
    const industryIdentifiers = Array.isArray(volumeInfo.industryIdentifiers)
        ...
        : [];
    const isbn13 = industryIdentifiers.find((id) => String(id?.type || "").toUpperCase() === "ISBN_13");
    const isbn10 = industryIdentifiers.find((id) => String(id?.type || "").toUpperCase() === "ISBN_10");
    const hasIsbn = Boolean((isbn13 && String(isbn13.identifier || "").trim())
        || (isbn10 && String(isbn10.identifier || "").trim()));
...
function kidsEditionQualityScore(item) {
    let score = 0;
    ...
    if (/\bjuvenile fiction\b/i.test(categoryBlob))
        score += 10;
    if (description.length > 40)
        score += 5;
    if (/\b(early reader|easy reader|beginning reader|beginner book|picture book|read aloud|i can read|step into reading|ready to read|easy-to-read)\b/i.test([title, subtitle, description].join(" ")))
        score += 8;
    const hasIsbn = Array.isArray(volumeInfo.industryIdentifiers)
        && volumeInfo.industryIdentifiers.some((id) => {
    ...
    if (pageCount > 0)
        score += 2;
    if (pageCount >= 20 && pageCount <= 80)
        score += 3;
    if (publisher && GOOGLE_BOOKS_MAINSTREAM_FICTION_PUBLISHER_PATTERN.test(publisher))
        score += 2;
    if (/\bjuvenile nonfiction\b/i.test(categoryBlob))
        score -= 8;
    if (categoryBlob === "" || categories.length === 0)
        score -= 3;
    if (/[\u0100-\uffff]{5,}/.test(title))
        score -= 15;
    return score;
}
function kidsDeduplicateEditions(items) {
    ...
    for (const item of items) {
        ...
        const score = kidsEditionQualityScore(item);
...
exports.googleBooksSourceAdapter = {
    source: "googleBooks",
    async search(plan, context) {
        ...
        const queries = plannedQueries.filter((intent) => {
            if (seenQueries.has(intent.fetchQuery))
                return false;
        ...
        const creatorsByTitle = {};
        const categoriesByTitle = {};
        const descriptionPresentByTitle = {};
        const isbnPresentByTitle = {};
        const ratingsCountByTitle = {};
        const ratingsAverageByTitle = {};
        const queryByTitle = {};
        const printTypeByTitle = {};
        const languageByTitle = {};
        const maturityRatingByTitle = {};
        const audienceBandByTitle = {};
        const contentMaturityByTitle = {};
        const sourceMaturityRatingByTitle = {};
        const requestedDeckByTitle = {};
        const publicationShapeByTitle = {};
        const narrativeConfidenceByTitle = {};
        const publicationShapeEvidenceByTitle = {};
        const narrativePriorityAdjustmentByTitle = {};
        const publicationShapeRejectedBeforeRankingByTitle = {};
        const dominantPublicationShapeEvidenceByTitle = {};
        const overriddenNarrativeEvidenceByTitle = {};
        const publicationShapePrecedenceDecisionByTitle = {};
        const explicitNonNarrativeIdentityByTitle = {};
        const storyLevelNarrativeEvidenceByTitle = {};
        const genericCategoryTitleByTitle = {};
        const genericCategoryEvidenceByTitle = {};
        const genericCategoryRejectedBeforeRankingByTitle = {};
        const unknownShapeEligibilityByTitle = {};
        const unknownShapeEvidenceByTitle = {};
        const unknownShapeRejectedReasonByTitle = {};
        const unknownStoryEvidenceCountByTitle = {};
        const unknownStoryEvidenceFamiliesByTitle = {};
        const unknownNarrativeCorroborationByTitle = {};
        const unknownEligibilityThresholdDecisionByTitle = {};
        const subjectOfStudyTitleByTitle = {};
        const subjectOfStudyEvidenceByTitle = {};
        const subjectOfStudyRejectedBeforeRankingByTitle = {};
        const curatedBookGuideIdentityByTitle = {};
        const curatedBookGuideEvidenceByTitle = {};
        const periodicalIdentityEvidenceByTitle = {};
        const periodicalIdentityDecisionByTitle = {};
        const preNormalizationRejectAuditRows = [];
        const preNormalizationRejectAuditBySourceId = {};
        const preNormalizationRejectAuditByTitle = {};
        const preteenPublicationShapeAuditByTitle = {};
        const preteenPublicationShapeRejectedTitles = [];
        const preteenPublicationShapeRejectedReasonByTitle = {};
        const preteenPublicationShapeNarrativeEvidenceByTitle = {};
        const preteenPublicationShapeArtifactEvidenceByTitle = {};
        const preteenPublicationShapeCounterfactualDecisionByTitle = {};
        const preteenPublicationShapeLikelyFalseRejectTitles = [];
        const preteenPublicationShapeLikelyCorrectRejectTitles = [];
        const preteenPublicationShapeAmbiguousRejectTitles = [];
        const preteenPublicationShapeFalseRejectHistogram = {};
        const preteenPublicationShapeRescueAppliedByTitle = {};
        const preteenPublicationShapeRescueReasonByTitle = {};
        const preteenPublicationShapeRescueEvidenceByTitle = {};
        const preteenPublicationShapeRescuedTitles = [];
        const preteenPublicationShapeRescueRejectedTitles = [];
        const preteenPublicationShapeRescueRejectedReasonByTitle = {};
        const kidsPublicationAuditByTitle = {};
        const kidsPublicationNarrativeEvidenceByTitle = {};
        const kidsPublicationFormatEvidenceByTitle = {};
        const kidsPublicationAgeEvidenceByTitle = {};
        const kidsPublicationTrustedMetadataByTitle = {};
        const kidsPublicationCounterfactualDecisionByTitle = {};
        const kidsPublicationLikelyFalseRejectTitles = [];
        const kidsPublicationLikelyCorrectRejectTitles = [];
        const kidsPublicationAmbiguousRejectTitles = [];
        const perQueryQuality = {};
        const modernNarrativeCountByQuery = {};
        const publicDomainCatalogShapeCountByQuery = {};
        ...
        const recordPreNormalizationReject = (params) => {
            ...
            if (row.sourceId)
                preNormalizationRejectAuditBySourceId[row.sourceId] = row;
            if (row.title)
                preNormalizationRejectAuditByTitle[row.title] = row;
        };
        ...
        const perQueryTimeoutMs = Math.max(1000, Math.floor(Math.max(plan.timeoutMs, 1000) / Math.max(1, queries.length)));
        ...
        for (let index = 0; index < queryExecutionOrder.length; index += 1) {
            ...
            for (const item of items) {
                ...
                const volumeInfo = (row.volumeInfo && typeof row.volumeInfo === "object") ? row.volumeInfo : null;
                ...
                const industryIdentifiers = Array.isArray(volumeInfo.industryIdentifiers)
                    ...
                    : [];
                const isbn13 = industryIdentifiers.find((identifier) => String(identifier?.type || "").toUpperCase() === "ISBN_13");
                const isbn10 = industryIdentifiers.find((identifier) => String(identifier?.type || "").toUpperCase() === "ISBN_10");
                ...
                if (shapeAnalysis.unknownShapeRejectedReason)
                    unknownShapeRejectedReasonByTitle[title] = shapeAnalysis.unknownShapeRejectedReason;
                ...
                let preteenPublicationShapeRescueDecision;
                if (ageBand === "preteens" && originalPublicationShapeDropReason) {
                    ...
                    if (!preteenPublicationShapeRejectedTitles.includes(title))
                        preteenPublicationShapeRejectedTitles.push(title);
                    ...
                    if (auditRecord.disposition === "likely_false_reject") {
                        if (!preteenPublicationShapeLikelyFalseRejectTitles.includes(title))
                            preteenPublicationShapeLikelyFalseRejectTitles.push(title);
                        ...
                        if (!preteenPublicationShapeLikelyCorrectRejectTitles.includes(title))
                            preteenPublicationShapeLikelyCorrectRejectTitles.push(title);
                    }
                    ...
                    if (preteenPublicationShapeRescueDecision.applied) {
                        if (!preteenPublicationShapeRescuedTitles.includes(title))
                            preteenPublicationShapeRescuedTitles.push(title);
                    }
                    else {
                        if (!preteenPublicationShapeRescueRejectedTitles.includes(title))
                            preteenPublicationShapeRescueRejectedTitles.push(title);
                ...
                if (ageBand === "kids" && originalPublicationShapeDropReason && KIDS_GOOGLE_BOOKS_UNKNOWN_SHAPE_REASONS.has(originalPublicationShapeDropReason)) {
                    ...
                    if (auditRecord.counterfactualDecision === "likely_k2_narrative_publication") {
                        if (!kidsPublicationLikelyFalseRejectTitles.includes(title))
                            kidsPublicationLikelyFalseRejectTitles.push(title);
                    }
                    else if (auditRecord.counterfactualDecision === "likely_correct_reject") {
                        if (!kidsPublicationLikelyCorrectRejectTitles.includes(title))
                            kidsPublicationLikelyCorrectRejectTitles.push(title);
                    }
                ...
                if (dropReason) {
                    ...
                    perQueryQuality[originalQuery].rejectedCandidateCount += 1;
                    if (publicationShapeDropReason)
                        publicationShapeRejectedBeforeRankingByTitle[title] = publicationShapeDropReason;
                    if (publicationShapeDropReason === "publication_shape_generic_category_catalog")
                        genericCategoryRejectedBeforeRankingByTitle[title] = publicationShapeDropReason;
                    if (shapeAnalysis.subjectOfStudyTitle && publicationShapeDropReason)
                        subjectOfStudyRejectedBeforeRankingByTitle[title] = publicationShapeDropReason;
                    droppedBeforeNormalization += 1;
                    continue;
                }
                ...
                const rawRow = {
                    ...
                    industryIdentifiers,
                    isbn13: isbn13 ? String(isbn13.identifier || "").trim() || undefined : undefined,
                    isbn10: isbn10 ? String(isbn10.identifier || "").trim() || undefined : undefined,
                ...
                if (rawTitles.length < 40)
                    rawTitles.push(title);
                queryByTitle[title] = originalQuery;
                if (Number.isFinite(Number(publicationYear)))
                    publicationYearByTitle[title] = Number(publicationYear);
                ...
                if (typeof averageRating === "number" && Number.isFinite(averageRating))
                    ratingsAverageByTitle[title] = averageRating;
                ...
                if (Number.isFinite(Number(publicationYear)))
                    perQueryQuality[originalQuery].publicationYearByTitle[title] = Number(publicationYear);
                ...
                if (typeof averageRating === "number" && Number.isFinite(averageRating))
                    perQueryQuality[originalQuery].averageRatingByTitle[title] = averageRating;
        ...
        const adultGoogleBooksQueryQualityByQuery = {};
        const publicationShapeHistogramByQuery = {};
        const rejectedShapeHistogramByQuery = {};
        const narrativeYieldByQuery = {};
        const narrativeEfficiencyByQuery = {};
        ...
        const preteenPublicationShapeAuditSummary = {
            scope: "preteen_googlebooks_publication_shape_rejections",
        ...
        const diagnostics = {
            ...
            dropReasons,
            fetches,
            rawItemPreview: rawItems.slice(0, 15).map((item) => item),
