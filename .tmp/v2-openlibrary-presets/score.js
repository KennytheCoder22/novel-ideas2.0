...
function rawTextParts(candidate) {
...
function kidsNarrativeSemanticEvidence(candidate, matches) {
    ...
    for (const signal of matches) {
        ...
        if (!value || isKidsGenericTasteSignal(signal))
            continue;
    ...
    if (/\b(story|stories|tale|tales|adventure|journey|friendship|friends?|silly|funny|laugh|imagin|wonder|character|characters|monster|magic|school|family|community)\b/.test(narrativeText))
        score += 0.8;
...
function signalPresentInText(text, value) {
    ...
    if (/\b(family adventure|adventure family)\b/.test(value))
        return hasFamily && hasAdventure;
    if (/\b(mystery adventure|adventure mystery)\b/.test(value))
        return hasMystery && hasAdventure;
    if (/\b(science fiction adventure|science adventure|sci fi adventure|sci-fi adventure|robot adventure)\b/.test(value))
        return hasScienceFiction && hasAdventure;
    if (/\b(superhero adventure|super hero adventure)\b/.test(value))
        return hasSuperhero && hasAdventure;
    if (/\b(school mystery|mystery school)\b/.test(value))
        return hasSchool && hasMystery;
    if (/\b(ocean adventure|adventure ocean|sea adventure|island adventure)\b/.test(value))
        return hasOcean && hasAdventure;
    if (/\b(survival adventure|adventure survival)\b/.test(value))
        return hasSurvival && hasAdventure;
    const variants = {
...
function isMiddleGradesGenericTasteSignal(signal) {
    return MIDDLE_GRADES_GENERIC_TASTE_SIGNAL.test(normalized(signal.value));
}
function addAvoidSignalBucket(matches, matched, breakdown) {
    let broadPenalty = 0;
    ...
    if (broadPenalty)
        breakdown.broadAvoidSignalPenalty = Number(breakdown.broadAvoidSignalPenalty || 0) + Math.max(-1.6, broadPenalty);
    if (precisePenalty)
        breakdown.avoidSignalPenalty = Number(breakdown.avoidSignalPenalty || 0) + precisePenalty;
}
function isKidsGenericTasteSignal(signal) {
    const value = normalized(signal.value);
...
function addSignalBucket(matches, multiplier, matched, breakdown, bucket) {
    for (const signal of matches) {
...
function queryRungBonus(candidate) {
...
function ageSuitabilityScore(candidate, profile) {
    const text = candidateMetadataText(candidate);
    if (profile.ageBand === "adult") {
        if (/\b(juvenile fiction|children'?s books?|easy readers?|middle grade|rainbow magic)\b/.test(text))
            return -2;
        return 0.5;
    }
    ...
    if (/\b(lolita|nabokov|erotic|erotica|pornography|incest|sexual abuse)\b/.test(text))
        return -6;
    if (/\b(demoness|vixen|seductress|sensual|forbidden desire|dark lover|new adult|adult romance|college romance|bret easton ellis|the informers|icebreaker|midnight fantasies|blaze|harlequin|silhouette desire)\b/.test(text))
        return -4.5;
    if (/^(the clown hunt|clown hunt|pope|phantoms)$/.test(normalizedTitle) && !hasStrongTeenMetadata(text))
        return -2.5;
    if (hasStrongTeenMetadata(text))
        return 1;
    if (candidate.publicationYear && candidate.publicationYear >= 2000)
        return 0.8;
    if (candidate.publicationYear && candidate.publicationYear >= 1950)
        return 0.35;
    return -0.5;
}
...
function querySpecificityScore(candidate) {
    ...
    if (/^(young adult fantasy|fantasy|mystery novel)$/i.test(query.trim()) && matches.length <= 1)
        score -= 0.6;
    return score;
}
function sourceQualityRelevanceScore(candidate, profile, genreMatches, positiveMatches) {
    ...
    if (raw.cover_i)
        score += 0.15;
    if (metadataCount >= 8 && candidate.creators.length > 0 && candidate.sourceId)
        score += 0.75;
    if (metadataCount >= 12)
        score += 0.2;
    if (metadataCount >= 16)
        score += 0.15;
    if (metadataCount >= 10 && strongTeenMetadata && strongGenreMetadata)
        score += 0.45;
    if (metadataCount >= 14 && candidate.creators.length > 0 && strongGenreMetadata)
        score += 0.3;
    if (profile.ageBand === "adult" && authorCount === 1 && uniqueSubjectCount >= 8 && strongGenreMetadata)
        score += 0.35;
    if (profile.ageBand === "adult" && titleWordCount >= 2 && titleWordCount <= 6 && uniqueSubjectCount >= 6)
        score += 0.25;
    if (profile.ageBand === "adult" && titleWordCount >= 10 && !strongGenreMetadata)
        score -= 0.7;
    if (profile.ageBand === "adult" && authorCount === 0)
        score -= 0.5;
    if (metadataCount <= 2)
        score -= 1.25;
    if (metadataCount <= 5 && !strongGenreMetadata)
        score -= 0.8;
    if (metadataCount <= 6 && !strongTeenMetadata)
        score -= 0.45;
    if (genreMatches.length > 0)
        score += 0.7 + Math.min(0.35, genreMatches.length * 0.08);
    if (positiveMatches.length > 0)
        score += 0.4 + Math.min(0.3, positiveMatches.length * 0.06);
    if (strongTeenMetadata)
        score += 0.25;
    if (normalizedTitle.split(" ").length >= 3 && strongGenreMetadata)
        score += 0.15;
    if (normalizedTitle.split(" ").length <= 2 && !strongTeenMetadata && !strongGenreMetadata)
        score -= 0.7;
    if (/^(deception|departures|the departures|end is here|the end is here|refigurations of freedom|tell freedom i said hello|facility|fang)$/.test(normalizedTitle) && metadataCount < 12)
        score -= 2.8;
    if (profile.ageBand === "teens" && /\b(my secret garden|sexual fantasies|women\s+sexual fantasies)\b/.test(text))
        score -= 8;
    if (metadataCount <= 4 && !strongTeenMetadata && !strongGenreMetadata)
        score -= 1.2;
    if (/^(the clown hunt|clown hunt|pope|phantoms)$/.test(normalizedTitle) && profile.ageBand === "teens" && !strongTeenMetadata)
        score -= 3.2;
    if (/\b(survival guide|survival handbook|survival manual|field guide|handbook|choose your own adventure|mountain survival|star trek survival|kane chronicles survival guide|survival of the richest|cultural survival|survival culture|survival skills?)\b/.test(text) && !strongTeenMetadata)
        score -= 5;
    if (profile.ageBand === "teens" && /\b(king of flesh and bone|married to a pirate|flesh and bone|dark romance|dark romantasy|monster romance|alien sex|alien romance|alien lover|pirate romance|captive bride|reverse harem|why choose|possessive alpha|mafia romance)\b/.test(text) && !strongTeenMetadata)
        score -= 5;
    if (/\b(library programs? for teens|library programming|programs? for teens|teen programs?|genre guide|curriculum|classroom|lesson plans?|activity book|activities for teens|teacher'?s? guide|study guide|reader'?s? advisory|book lists? for teens|guides?[^.]{0,40}for teens|for teens[^.]{0,40}(guides?|nonfiction|curriculum|programming|activities))\b/.test(text))
        score -= 6;
    if (/\b(echoes and ashes|raven'?s sight|max porter)\b/.test(text))
        score -= 1.4;
    if (/\b(coloring|colouring|workbook|worksheet|activity book|teacher'?s? guide|study guide)\b/.test(text))
        score -= 4;
    if (/\b(playing with fantasy|fantasy drama book)\b/.test(text))
        score -= 3;
    if (/\bgo to hell\b/.test(text) && !/\b(young adult|juvenile fiction|teen|adolescent|dystopian|science fiction|fantasy|horror|mystery|thriller|adventure)\b/.test(text))
        score -= 4;
    if (/\bdrunk\b/.test(text) && genreMatches.length === 0)
        score -= 2.5;
    if (profile.ageBand === "teens" && /\b(demoness|vixen|seductress|sensual|new adult|adult romance|college romance|bret easton ellis|the informers|icebreaker|midnight fantasies|blaze|harlequin|silhouette desire)\b/.test(text))
        score -= 2.5;
    if (profile.ageBand === "adult" && /\b(corpus of ancient near eastern seals|archaeological catalog|museum collections?|crime and punishment notes|the poet and the murderer|mystery in the mainstream|study notes?|notes on|book notes?|study aids?|companions? to|criticism|critical essays?|literary history|bibliograph(?:y|ies)|true crime nonfiction|wizardry and wild romance|king of flesh and bone|married to a pirate|pirate romance|dark romance|dark romantasy|monster romance|reverse harem|writing guide|horror criticism|genre history)\b/.test(text))
        score -= 5;
    if (profile.ageBand === "adult" && metadataCount <= 5 && !strongGenreMetadata)
        score -= 0.6;
    if (/^[A-Z0-9\s:;,'!?.-]{12,}$/.test(candidate.title) && candidate.title !== candidate.title.toLowerCase())
        score -= 1.25;
    if (/^[A-Z][a-z]+\s+[A-Z][a-z]+$/.test(candidate.title) && metadataCount <= 2)
        score -= 1.5;
    if (genreMatches.length === 0 && positiveMatches.length === 0)
        score -= 1.5;
    return score;
}
function scoreCandidates(candidates, profile) {
    return candidates.map((candidate) => {
        ...
        if (kidsNarrativeEvidence.score > 0)
            scoreBreakdown.narrativeSemanticEvidence = kidsNarrativeEvidence.score;
