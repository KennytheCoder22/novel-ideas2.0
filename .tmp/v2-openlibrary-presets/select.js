...
function seriesKey(candidate) {
    ...
    if (/\b(hunger games|catching fire|mockingjay)\b/.test(text))
        return "hunger games";
...
function collectionRootKey(candidate) {
...
function finalReturnedRootKey(candidate) {
...
function isContemporaryLowScoreAcceptable(candidate, profile) {
...
function needsAdultWeakOpenLibraryEmptySlateFallback(candidate, profile) {
...
function adultQueryFamily(candidate) {
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily].filter(Boolean).join(" "));
    if (/\b(science fiction|sci fi|speculative|dystopia|dystopian|space)\b/.test(text))
        return "speculative";
    if (/\b(cozy|cosy)\b/.test(text))
        return "cozy_fantasy";
    if (/\bfantasy\b/.test(text))
        return "fantasy";
    if (/\b(historical|history|period)\b/.test(text))
        return "historical";
    if (/\b(crime|mystery|thriller|detective|noir|suspense)\b/.test(text))
        return "crime_thriller";
    if (/\bhorror\b/.test(text))
        return "horror";
    return "other";
}
function adultSignalWeight(profile, pattern) {
    return [...profile.genreFamily, ...profile.themes].reduce((sum, row) => {
        if (!pattern.test(normalized(row.value)))
            return sum;
...
function adultSpeculativeReserveTarget(candidates, profile) {
...
function addAdultFamilyDiagnostics(candidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "adult")
        return;
    const scoredCounts = {};
    const selectedCounts = {};
    for (const candidate of candidates)
        scoredCounts[adultQueryFamily(candidate)] = Number(scoredCounts[adultQueryFamily(candidate)] || 0) + 1;
    for (const candidate of selected)
        selectedCounts[adultQueryFamily(candidate)] = Number(selectedCounts[adultQueryFamily(candidate)] || 0) + 1;
...
function applyAdultSpeculativeFamilyBalance(rankedCandidates, selected, rejectedReasons, profile, limit) {
    ...
    if (selectedSpeculative >= reserveTarget)
        return;
    ...
    const speculativePool = rankedCandidates.filter((candidate) => {
        if (selectedSet.has(candidate) || selectedTitles.has(normalized(candidate.title)))
            return false;
        if (adultQueryFamily(candidate) !== "speculative")
            return false;
    ...
    for (const candidate of speculativePool) {
        if (selectedSpeculative >= reserveTarget || selected.length >= Math.max(3, Math.min(5, limit)))
            break;
...
function isMiddleGradesFantasyHumorCandidate(candidate) {
    return candidate.source === "openLibrary" && /middle_grades_fantasy_humor/i.test(String(candidate.diagnostics?.routingReason || ""));
}
function isMiddleGradesFantasyHumorAlignedCandidate(candidate) {
    if (!isMiddleGradesFantasyHumorCandidate(candidate))
        return false;
    ...
    if (/\b(adventure|fantasy adventure|friendship)\b/.test(queryText) && !/\b(humor|funny)\b/.test(queryText))
        return true;
    return !queryText && /\b(adventure|fantasy adventure|friendship)\b/.test(fallbackText) && !/\b(humor|funny)\b/.test(fallbackText);
}
function isMiddleGradesFantasyHumorDefaultCandidate(candidate) {
    if (!isMiddleGradesFantasyHumorCandidate(candidate))
        return false;
...
function isMiddleGradesAntiZeroFallbackCandidate(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return false;
...
function middleGradesRawText(candidate) {
...
function middleGradesQueryText(candidate) {
...
function middleGradesRouteEvidencePattern(candidate) {
    ...
    if (/\b(science adventure|science fiction|sci fi|sci-fi|space|dystopian|dystopia)\b/.test(routeText))
        return /\b(science|scientist|experiment|space|planet|galaxy|robot|robots?|technology|invention|dystopian|dystopia|sci fi|sci-fi|science fiction|nonfiction|animals?|nature|wildlife|wolf|wolves)\b/;
    if (/\b(robot|ai|artificial intelligence|superhero|superheroes)\b/.test(routeText))
        return /\b(robot|robots?|ai|artificial intelligence|technology|invention|superhero|superheroes|powers?)\b/;
    if (/\b(animal adventure|animals?|nature|wildlife)\b/.test(routeText))
        return /\b(animal|animals|dog|cat|horse|wolf|wolves|wildlife|nature|forest|woods|survival|cozy|community|farm|creature|creatures)\b/;
    if (/\b(school adventure|school story|school|classroom|children s school stories)\b/.test(routeText))
        return /\b(school|class|classroom|teacher|student|students|friendship|friends?|community|family|comedy|funny|humor|humour)\b/;
    if (canUseRecoveryEvidencePattern && /\b(superhero|super hero)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure)\b/.test(sourceQueryText))
        return /\b(superhero|super hero|heroes|hero|powers?|friendship|friends?|team|adventure|quest)\b/;
    if (canUseRecoveryEvidencePattern && /\bfamily\b/.test(sourceQueryText) && /\badventure\b/.test(sourceQueryText))
        return /\b(family|families|parents?|siblings?|home|superhero|super hero|heroes|hero|powers?|adventure|quest|journey|wild)\b/;
    if (canUseRecoveryEvidencePattern && /\b(ocean|sea|island)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure|fantasy)\b/.test(sourceQueryText))
        return /\b(ocean|sea|island|marine|friendship|friends?|adventure|quest|fantasy|magic|magical)\b/;
    if (canUseRecoveryEvidencePattern && /\b(survival|survive|wilderness|forest|stranded)\b/.test(sourceQueryText) && /\b(adventure|friendship|fiction)\b/.test(sourceQueryText))
        return /\b(survival|survive|survives|wilderness|wild|forest|island|stranded|adventure|quest|team|friendship|friends?)\b/;
    if (canUseRecoveryEvidencePattern && /\b(science|robot|technology)\b/.test(sourceQueryText) && /\b(adventure|friendship|fiction)\b/.test(sourceQueryText))
        return /\b(science|scientist|experiment|robot|robots?|technology|invention|friendship|friends?|adventure|quest)\b/;
    if (canUseRecoveryEvidencePattern && /\b(fantasy|magic|magical|mythology)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure|family)\b/.test(sourceQueryText))
        return /\b(fantasy|magic|magical|wizard|witch|fairy|fairies|dragon|quest|kingdom|hero|heroic|myth|myths|mythology|adventure|friendship|friends?|family|school)\b/;
    if (/\b(friendship|community)\b/.test(routeText))
        return /\b(friendship|friends?|community|school|family|team|classroom)\b/;
    if (/\b(fantasy mystery|mystery adventure|mystery|detective)\b/.test(routeText))
        return /\b(mystery|detective|clue|clues|case|secret|secrets|puzzle|investigate|investigation)\b/;
    if (/\b(humor|funny|funny family|fantasy humor)\b/.test(routeText))
        return /\b(humor|humour|funny|comedy|comic|joke|laugh|laughs|giggle|silly|school|friendship|friends?|family|quest|adventure|trail)\b/;
    if (/\b(fantasy adventure|family fantasy|fantasy|magic|magical)\b/.test(routeText))
        return /\b(fantasy|magic|magical|wizard|witch|dragon|quest|kingdom|hero|heroic|adventure)\b/;
    if (/\b(contemporary|realistic)\b/.test(routeText))
        return /\b(realistic|contemporary|school|classroom|friendship|friends?|family|community)\b/;
    return undefined;
}
function middleGradesEvidenceTierRank(tier) {
    if (tier === "strong_evidence")
        return 3;
    if (tier === "medium_evidence")
        return 2;
    if (tier === "weak_evidence")
        return 1;
    return 0;
}
function middleGradesRouteAlignmentEvidence(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return { queryLevel: false, documentLevel: false, fields: [], evidenceTextByField: {}, tier: "query_only" };
    ...
    const tier = richFields.length > 0 && (titleFields.length > 0 || richFields.some((field) => ["subjects", "description"].includes(field)))
        ? "strong_evidence"
...
function isMiddleGradesRouteAlignedSuccessCandidate(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return false;
    if (isMiddleGradesAntiZeroFallbackCandidate(candidate))
        return false;
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    if (!evidence.documentLevel)
        return false;
    if (evidence.tier === "weak_evidence")
        return false;
    if (evidence.fields.length > 0 && evidence.fields.every((field) => field === "title" || field === "subtitle"))
        return false;
    return true;
}
function isMiddleGradesOpenLibraryCandidate(candidate) {
    return candidate.source === "openLibrary" && /middle_grades_/i.test(String(candidate.diagnostics?.routingReason || ""));
}
function middleGradesFictionAgeEvidence(candidate) {
...
function isMiddleGradesReferenceOrLocalHistoryArtifact(candidate) {
...
function isMiddleGradesHumorRouteCandidate(candidate) {
...
function middleGradesNonHumorAlignment(candidate) {
    const text = middleGradesRawText(candidate);
    return /\b(adventure|friendship|friends?|community|survival|school|family|quest|team)\b/.test(text);
}
function middleGradesAdultOrYaHumorLeakage(candidate) {
    if (!isMiddleGradesHumorRouteCandidate(candidate))
        return false;
    if (middleGradesFictionAgeEvidence(candidate))
        return false;
    const text = middleGradesRawText(candidate);
...
function middleGradesHumorKeywordOnlyLeakage(candidate) {
    if (!isMiddleGradesHumorRouteCandidate(candidate))
        return false;
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    ...
    if (!/\b(funny|humor|humour|comedy|comic|joke|laugh|giggle)\b/.test(titleText))
        return false;
    return !middleGradesFictionAgeEvidence(candidate) && !middleGradesNonHumorAlignment(candidate);
}
function middleGradesTitleOnlyRouteEvidence(candidate) {
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    return evidence.fields.length > 0 && evidence.fields.every((field) => field === "title" || field === "subtitle");
}
function middleGradesSupportedRouteEvidenceFields(candidate) {
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    return evidence.fields.filter((field) => !["title", "subtitle"].includes(field));
}
function middleGradesFinalEligibility(candidate) {
    if (!isMiddleGradesOpenLibraryCandidate(candidate))
        return { allowed: true, evidence: ["not_middle_grades_openlibrary"] };
    if (isMiddleGradesReferenceOrLocalHistoryArtifact(candidate))
        return { allowed: false, evidence: [], rejectedReason: "middle_grades_reference_or_local_history_artifact" };
    if (middleGradesAdultOrYaHumorLeakage(candidate))
        return { allowed: false, evidence: [], rejectedReason: "adult_or_ya_humor_leakage" };
    if (middleGradesHumorKeywordOnlyLeakage(candidate))
        return { allowed: false, evidence: [], rejectedReason: "humor_keyword_only_leakage" };
    ...
    if (routeEvidence.documentLevel && routeEvidence.fields.length > 0 && hasIndependentSupport)
        return { allowed: true, evidence: routeEvidence.fields.map((field) => `document_route:${field}`) };
    if (middleGradesFictionAgeEvidence(candidate) && hasIndependentSupport)
        return { allowed: true, evidence: ["middle_grade_fiction_metadata", ...routeEvidence.fields.map((field) => `document_route:${field}`)] };
    ...
    if (middleGradesTitleOnlyRouteEvidence(candidate))
        return { allowed: false, evidence: routeEvidence.fields.map((field) => `document_route:${field}`), rejectedReason: "title_only_route_evidence_missing_support" };
    if (isMiddleGradesAntiZeroFallbackCandidate(candidate))
        return { allowed: true, evidence: ["explicit_emergency_fallback"], emergencyOverride: true };
    if (routeEvidence.queryLevel && !routeEvidence.documentLevel)
        return { allowed: false, evidence: [], rejectedReason: "middle_grades_query_only_missing_document_evidence" };
    return { allowed: false, evidence: [], rejectedReason: "middle_grades_missing_route_or_fiction_evidence" };
}
function applyMiddleGradesQueryOnlyScoreCaps(candidates, profile, rejectedReasons) {
    if (profile.ageBand !== "preteens")
        return;
    const queryOnlyScoreCapAppliedByTitle = {};
    const queryOnlyScoreCapReasonByTitle = {};
    let missingDocumentEvidenceCount = 0;
    for (const candidate of candidates) {
        if (!isMiddleGradesOpenLibraryCandidate(candidate))
            continue;
        const routeEvidence = middleGradesRouteAlignmentEvidence(candidate);
        if (!routeEvidence.queryLevel || routeEvidence.documentLevel || routeEvidence.fields.length > 0)
            continue;
        const breakdown = candidate.scoreBreakdown || {};
    ...
    if (missingDocumentEvidenceCount > 0) {
        rejectedReasons.queryOnlyScoreCapAppliedByTitle = queryOnlyScoreCapAppliedByTitle;
        rejectedReasons.queryOnlyScoreCapReasonByTitle = queryOnlyScoreCapReasonByTitle;
        rejectedReasons.documentEvidenceRequiredButMissingCount = missingDocumentEvidenceCount;
    }
}
function addMiddleGradesSlateDiagnostics(selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
    ...
    const fallbackSpecificityScore = selected.reduce((sum, candidate) => {
        ...
        let score = 0;
        if (/\b(friendship|family|contemporary|realistic|school|mystery|ai|robot|superhero|animal|nature|science fiction|dystopian)\b/.test(text))
            score += 1;
        if (/\b(fantasy adventure|family fantasy|funny family|school adventure|mystery adventure|friendship adventure|animal adventure|robot|superhero)\b/.test(text))
            score += 1;
        if (isMiddleGradesAntiZeroFallbackCandidate(candidate))
            score -= 0.25;
        return sum + score;
    ...
    if (antiZeroCount >= Math.ceil(selected.length / 2) && roundedAverage <= 0) {
        ...
        for (const candidate of selected.filter(isMiddleGradesAntiZeroFallbackCandidate))
            candidate.rejectedReasons.push("middle_grades_mostly_fallback_zero_genre_match_penalty");
    }
    if (genericDefaults.length >= Math.min(4, selected.length)) {
        ...
        for (const candidate of genericDefaults)
            candidate.rejectedReasons.push("generic_default_slate_detected_query_text_without_doc_specificity");
    }
    for (const candidate of selected) {
        ...
        if (genericDefaults.length >= Math.min(4, selected.length))
            candidate.diagnostics.genericDefaultSlateReason = "query_text_without_doc_specificity";
    }
}
function applyMiddleGradesAntiZeroFallbackGate(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    ...
    if (!selectedAntiZero.length) {
        ...
        if (routeAlignedCount > 0)
            rejectedReasons.middle_grades_route_aligned_success = routeAlignedCount;
        return;
    }
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    ...
    const safeRouteAlignedPool = rankedCandidates.filter((candidate) => {
        if (!isMiddleGradesRouteAlignedSuccessCandidate(candidate))
            return false;
        ...
        if (rootKey && selectedRoots().has(rootKey))
            return false;
        return true;
    });
    ...
    if (selected.filter(isMiddleGradesRouteAlignedSuccessCandidate).length === 0 && safeRouteAlignedPool.length === 0) {
        ...
        for (const candidate of selectedAntiZero)
            candidate.rejectedReasons.push("middle_grades_anti_zero_fallback_only_slate");
        return;
    }
    for (const candidate of safeRouteAlignedPool) {
        ...
        const rootKey = seriesKey(candidate);
        if (selectedTitles().has(titleKey) || (rootKey && selectedRoots().has(rootKey)))
            continue;
    ...
    if (remainingAntiZero > 0)
        rejectedReasons.middle_grades_anti_zero_fallback_success = remainingAntiZero;
}
function middleGradesRouteKey(candidate) {
...
function middleGradesHumorCapBucketKey(candidate) {
    const routeKey = middleGradesRouteKey(candidate);
    if (/mystery|contemporary_school/i.test(routeKey))
        return routeKey;
    if (/humor|adventure/i.test(routeKey))
        return "middle_grades_humor_adventure";
    return routeKey;
}
function isMiddleGradesHumorDefaultQueryFamily(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return false;
...
function isMiddleGradesHumorCapAlternative(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return false;
    if (isMiddleGradesHumorDefaultQueryFamily(candidate))
        return false;
    const text = normalized([
        candidate.diagnostics?.queryText,
        candidate.diagnostics?.queryFamily,
...
function applyMiddleGradesHumorDefaultCap(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
    ...
    for (const routeKey of routeKeys) {
        ...
        if (selectedDefaults.length <= 3)
            continue;
        const safeAlignedPool = routeCandidates.filter((candidate) => {
            if (!isMiddleGradesHumorCapAlternative(candidate))
                return false;
            if (rejectReason(candidate, profile))
                return false;
            return true;
        });
        if (safeAlignedPool.length < 2)
            continue;
...
function isMiddleGradesContemporarySchoolDefaultCandidate(candidate) {
    if (!isMiddleGradesContemporarySchoolCandidate(candidate))
        return false;
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily, candidate.title, candidate.subtitle].filter(Boolean).join(" "));
    if (/\b(humor|humorous|funny)\b/.test(text))
        return true;
    return /\bschool\b/.test(text) && !/\b(realistic|friendship|family|family life|classroom|contemporary)\b/.test(text);
}
function isMiddleGradesContemporarySchoolSaferAlignedCandidate(candidate) {
    if (!isMiddleGradesContemporarySchoolCandidate(candidate))
        return false;
    if (isMiddleGradesContemporarySchoolDefaultCandidate(candidate))
        return false;
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily, candidate.title, candidate.subtitle].filter(Boolean).join(" "));
    return /\b(realistic|school|friendship|classroom|family|family life|contemporary)\b/.test(text);
}
function applyMiddleGradesContemporarySchoolDefaultCap(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    ...
    const safeAlignedPool = rankedCandidates.filter((candidate) => {
        if (!isMiddleGradesContemporarySchoolSaferAlignedCandidate(candidate))
            return false;
        if (selected.includes(candidate))
            return false;
        if (rejectReason(candidate, profile))
    ...
    for (const candidate of safeAlignedPool) {
        if (selected.filter(isMiddleGradesContemporarySchoolDefaultCandidate).length <= 3)
            break;
        const replacementIndex = selected
            .map((row, index) => ({ row, index }))
...
function applyMiddleGradesFantasyHumorAlignedBalance(rankedCandidates, selected, rejectedReasons, profile, limit) {
    if (profile.ageBand !== "preteens")
        return;
    ...
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
    const safeAlignedPool = routeCandidates.filter((candidate) => {
        if (!isMiddleGradesFantasyHumorAlignedCandidate(candidate))
            return false;
        if (selected.includes(candidate))
            return false;
        if (rejectReason(candidate, profile))
    ...
    if (targetAligned < 2 || selected.filter(isMiddleGradesFantasyHumorAlignedCandidate).length >= targetAligned)
        return;
    rejectedReasons.middle_grades_fantasy_humor_aligned_balance_candidates = safeAlignedPool.length;
    for (const candidate of safeAlignedPool) {
        if (selected.filter(isMiddleGradesFantasyHumorAlignedCandidate).length >= targetAligned)
            break;
        const titleKey = normalized(candidate.title);
        const rootKey = seriesKey(candidate);
        if (selectedTitles().has(titleKey) || (rootKey && selectedRoots().has(rootKey)))
        ...
        const fallbackReplacementIndex = replacementIndex ?? selected
            ...
            .sort((a, b) => a.row.score - b.row.score)[0]?.index;
        if (fallbackReplacementIndex === undefined)
            break;
...
function isMiddleGradesContemporarySchoolCandidate(candidate) {
    return candidate.source === "openLibrary" && /middle_grades_contemporary_school/i.test(String(candidate.diagnostics?.routingReason || ""));
}
function isMiddleGradesContemporarySchoolAlignedCandidate(candidate) {
    if (!isMiddleGradesContemporarySchoolCandidate(candidate))
        return false;
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily, candidate.title, candidate.subtitle].filter(Boolean).join(" "));
    return /\b(realistic|school|friendship|classroom|family|family life|contemporary)\b/.test(text);
}
function isMiddleGradesContemporarySchoolAdventureFallback(candidate) {
    if (!isMiddleGradesContemporarySchoolCandidate(candidate))
        return false;
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily, candidate.title, candidate.subtitle].filter(Boolean).join(" "));
    return /\b(adventure|fantasy|magic|magical|quest)\b/.test(text) && !isMiddleGradesContemporarySchoolAlignedCandidate(candidate);
}
function applyMiddleGradesContemporarySchoolAlignment(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    const routeCandidates = rankedCandidates.filter(isMiddleGradesContemporarySchoolCandidate);
    if (!routeCandidates.some(isMiddleGradesContemporarySchoolAdventureFallback))
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
    const safeAlignedPool = routeCandidates.filter((candidate) => {
        if (!isMiddleGradesContemporarySchoolAlignedCandidate(candidate))
            return false;
        if (selected.includes(candidate))
            return false;
        if (rejectReason(candidate, profile))
...
function middleGradesRouteAlignmentScore(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return 0;
    const breakdown = candidate.scoreBreakdown || {};
    const routeKey = middleGradesRouteKey(candidate);
    ...
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    let score = 0;
    if (evidence.tier === "strong_evidence")
        score += 2.5;
    else if (evidence.tier === "medium_evidence")
        score += 1.5;
    else if (evidence.tier === "weak_evidence")
        score += 0.35;
    if (/fantasy_humor|humor/i.test(routeKey) && /\b(humor|funny|friendship|school|family|comedy|adventure)\b/.test(docText))
        score += 0.7;
    if (/contemporary_school|contemporary|school|realistic/i.test(routeKey) && /\b(realistic|contemporary|school|classroom|friendship|family)\b/.test(docText))
        score += 0.9;
    if (/mystery/i.test(routeKey) && /\b(mystery|detective|clue|school mystery)\b/.test(docText))
        score += 0.9;
    if (/\b(ai|robot|superhero|animal|nature|dystopian|science fiction|space|nonfiction)\b/.test(docText))
        score += 0.5;
    if (evidence.queryLevel && !evidence.documentLevel)
        score -= 1.1;
...
function middleGradesFallbackPenalty(candidate) {
    if (candidate.source !== "openLibrary" || !/middle_grades_/i.test(String(candidate.diagnostics?.routingReason || "")))
        return 0;
    const breakdown = candidate.scoreBreakdown || {};
    const text = normalized([candidate.diagnostics?.queryText, candidate.diagnostics?.queryFamily, candidate.title, candidate.subtitle].filter(Boolean).join(" "));
    ...
    if (evidence.tier === "weak_evidence")
        penalty += 0.8;
    if (/\b(middle grade adventure|fantasy adventure|school adventure|school story|science adventure|animal adventure|humor|funny)\b/.test(text) && Number(breakdown.genreFacetMatch || 0) <= 0)
        penalty += 0.7;
    if (/\b(school story|school adventure|humor|funny|middle grade humor|children s funny books)\b/.test(text) && Number(breakdown.genreFacetMatch || 0) <= 0 && (!candidate.matchedSignals || candidate.matchedSignals.length === 0))
        penalty += 0.9;
    return Math.round(penalty * 1000) / 1000;
}
function middleGradesSelectionScore(candidate, profile) {
    if (profile.ageBand !== "preteens")
        return candidate.score;
    return candidate.score + (middleGradesRouteAlignmentScore(candidate) * 1.2) - (middleGradesFallbackPenalty(candidate) * 1.4);
}
function middleGradesTasteAlignment(candidate) {
    const breakdown = candidate.scoreBreakdown || {};
    const matchedSignals = Array.isArray(candidate.matchedSignals) ? candidate.matchedSignals.length : 0;
    return Math.round((Number(breakdown.positiveTasteMatch || 0)
        + Number(breakdown.genreFacetMatch || 0)
        + Number(breakdown.themeMatch || 0)
        + Number(breakdown.toneMatch || 0)
        + matchedSignals * 0.25) * 1000) / 1000;
}
function compareForInitialSelection(a, b, profile) {
...
function isMiddleGradesCanonicalFranchiseTitle(candidate) {
...
function middleGradesRepresentativePenalty(candidate) {
    if (!isMiddleGradesOpenLibraryCandidate(candidate))
        return 0;
    ...
    let penalty = 0;
    if (/\b(companion|guide|guidebook|handbook|manual|confidential|insider|insider s|field guide|survival guide|encyclopedia|facts?|activity|workbook|journal|atlas|almanac|behind the scenes|making of|sampler|preview)\b/.test(text))
        penalty += 6;
    if (/\b(complete|collected|collection|collections|treasury|storybook|stories|tales|adventures|omnibus|anthology|library|set|boxed|box)\b/.test(titleText))
        penalty += 3;
    if (/\b(movie tie in|tie in|official tie in|video game|game guide)\b/.test(text))
        penalty += 1.5;
    ...
    if (/#\s*(?:[3-9]|\d{2,})\b/.test(rawTitle)
        ...
        || trailingTitleNumber >= 3)
        penalty += 3;
    if (/\b(nightfall|lodestar|neverseen)\b/.test(titleText) && /\b(shannon messenger|keeper|lost cities)\b/.test(text))
        penalty += 3;
    if (/\b(book|volume|vol)\s*(1|one)\b/.test(text) || /\bfirst\b[a-z0-9 ]{0,60}\b(series|novel|book|installment)\b/.test(text))
        penalty -= 1.5;
    if (isMiddleGradesCanonicalFranchiseTitle(candidate))
        penalty -= 2;
    return Math.round(penalty * 1000) / 1000;
}
function middleGradesFranchiseKey(candidate) {
    if (!isMiddleGradesOpenLibraryCandidate(candidate))
        return "";
    const text = middleGradesRepresentativeText(candidate);
    const author = primaryAuthor(candidate);
    if (/\b(rick riordan|percy jackson|camp half blood|half blood|trials of apollo|hidden oracle|heroes of olympus|olympus|greek gods|apollo)\b/.test(`${author} ${text}`))
        return "rick_riordan_mythology";
    if (/\bdork diaries\b/.test(text))
        return "dork_diaries";
    if (/\bdiary of an 8 bit warrior|8 bit warrior\b/.test(text))
        return "diary_of_an_8_bit_warrior";
    if (/\b(keeper of the lost cities|keeper lost cities|lost cities|neverseen|nightfall|lodestar)\b/.test(text))
        return "keeper_of_the_lost_cities";
    if (/\bwild robot\b/.test(text))
        return "wild robot";
    if (/\bnevermoor|morrigan crow\b/.test(text))
        return "nevermoor";
    if (/\bmasterminds\b/.test(text))
        return "masterminds";
    if (/\bminecraft\b/.test(text))
        return "minecraft";
    return "";
}
function middleGradesRepresentativeSelectionScore(candidate, profile) {
    const tierRank = middleGradesEvidenceTierRank(middleGradesRouteAlignmentEvidence(candidate).tier);
    return Math.round((middleGradesSelectionScore(candidate, profile)
        + tierRank * 0.8
        + middleGradesTasteAlignment(candidate) * 0.15
        - middleGradesRepresentativePenalty(candidate)) * 1000) / 1000;
}
function compareMiddleGradesRepresentativeCandidates(a, b, profile) {
...
function middleGradesSameRepresentativeCluster(a, b) {
...
function middleGradesWouldConflictAfterRepresentativeSwap(candidate, selected, replacementIndex) {
    const titleKey = normalized(candidate.title);
    ...
    return selected.some((other, index) => {
        if (index === replacementIndex)
            return false;
        if (normalized(other.title) === titleKey)
            return true;
        const otherRootKey = finalReturnedRootKey(other) || seriesKey(other);
        if (rootKey && otherRootKey && rootKey === otherRootKey)
            return true;
...
function applyMiddleGradesFranchiseRepresentativePreference(rankedCandidates, selected, deferred, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
...
function isMiddleGradesFallbackOrDefaultCandidate(candidate) {
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
...
function middleGradesDocumentBackedTasteSignals(candidate) {
...
function rawOpenLibraryDescription(raw) {
...
function profilePositiveSignals(profile) {
...
function middleGradesZeroTasteEvidenceAudit(candidate, profile) {
    const raw = (candidate.raw || {});
    ...
    const signalEvidenceFailures = matchedLikedSignalsBeforeEvidenceGate.reduce((acc, signal) => {
        const value = normalized(signal);
        if (!value)
            return acc;
        if (documentBackedSignals.has(value))
            return acc;
        if (queryText.includes(value) && !normalizedDocumentText.includes(value))
            acc[signal] = "matched_only_query_or_family_text_not_document_metadata";
        else if (!normalizedDocumentText.includes(value))
            acc[signal] = "missing_from_title_description_subjects_genres_themes";
        else
            acc[signal] = "present_in_document_text_but_not_credited_by_document_backed_signal_extraction";
        return acc;
    }, {});
    return {
...
function isMiddleGradesExplicitEmergencyFallback(candidate) {
...
function middleGradesMeaningfulTasteEligibility(candidate, allowExplicitEmergency = false) {
    if (candidate.source !== "openLibrary")
        return { allowed: true };
    if (allowExplicitEmergency && isMiddleGradesExplicitEmergencyFallback(candidate))
        return { allowed: true };
    if (middleGradesCleanExpansionRouteFictionSupport(candidate))
        return { allowed: true };
    ...
    if (!hasDocumentBackedTasteDiagnostics)
        return { allowed: true };
    const breakdown = candidate.scoreBreakdown || {};
    const tasteScore = Number(breakdown.genreFacetMatch || 0) + Number(breakdown.positiveTasteMatch || 0);
    if (tasteScore <= 0)
        return { allowed: false, reason: "zero_doc_backed_taste_match" };
    const backedSignals = middleGradesDocumentBackedTasteSignals(candidate);
    if (backedSignals.length > 0 && backedSignals.every((signal) => signal === "adventure"))
        return { allowed: false, reason: "broad_adventure_only_taste_match" };
    return { allowed: true };
}
function isMiddleGradesCleanExpansionCandidate(candidate) {
...
function middleGradesCleanExpansionRouteFictionSupport(candidate) {
    if (!isMiddleGradesCleanExpansionCandidate(candidate))
        return false;
    ...
    if (middleGradesSupportedRouteEvidenceFields(candidate).length === 0 && backedTasteSignals.length === 0)
        return false;
    if (candidate.score < 0)
        return false;
    if (candidate.rejectedReasons.includes("middle_grades_query_only_score_cap_applied"))
        return false;
    if (candidate.rejectedReasons.includes("humor_keyword_only_leakage"))
        return false;
    if (candidate.rejectedReasons.includes("non_positive_score"))
        return false;
    return true;
}
function isMiddleGradesCleanFinalCandidate(candidate) {
    const finalEligibility = middleGradesFinalEligibility(candidate);
...
function applyMiddleGradesCleanFinalTopUp(rankedCandidates, selected, rejectedReasons, profile, limit) {
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
    if (rankedCandidates.some((candidate) => candidate.diagnostics?.meaningfulTasteRecovery || candidate.diagnostics?.scoringHandoffStage === "meaningful_taste_recovery"))
        return;
    if (!rankedCandidates.some(isMiddleGradesCleanExpansionCandidate))
        return;
    ...
    const cleanPool = rankedCandidates
        ...
        if (selected.includes(candidate))
            return false;
        if (!isMiddleGradesCleanFinalCandidate(candidate))
            return false;
        if (candidate.maturityBand && String(candidate.maturityBand) !== profile.maturityBand)
            return false;
        if (rejectReason(candidate, profile))
            return false;
        if (selectedTitles().has(normalized(candidate.title)))
...
function applyMiddleGradesMeaningfulTasteFinalGate(selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    for (let index = selected.length - 1; index >= 0; index -= 1) {
        ...
        if (isMiddleGradesExplicitEmergencyFallback(candidate)) {
            if (!candidate.rejectedReasons.includes("emergency_fallback_zero_taste_fill"))
                candidate.rejectedReasons.push("emergency_fallback_zero_taste_fill");
...
function middleGradesQualityAuditRow(candidate, selected, selectedRoots, profile, rejectionReason) {
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
...
function applyMiddleGradesFallbackEvidencePrecedence(rankedCandidates, selected, rejectedReasons, profile) {
    var _a;
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
    ...
    for (let index = 0; index < selected.length; index += 1) {
        ...
        if (!isMiddleGradesFallbackOrDefaultCandidate(fallback))
            continue;
        ...
        const stronger = rankedCandidates.find((candidate) => {
            ...
            if (!eligibility.allowed)
                return false;
            const evidence = middleGradesRouteAlignmentEvidence(candidate);
            if (middleGradesEvidenceTierRank(evidence.tier) <= fallbackTierRank)
                return false;
            if (middleGradesTasteAlignment(candidate) + 0.001 < fallbackTaste)
                return false;
            if (selectedTitles().has(normalized(candidate.title))) {
        ...
        if (!stronger) {
            explanations[_a = fallback.title] || (explanations[_a] = "no_stronger_document_evidence_candidate_with_equal_or_better_taste_alignment_survived_safety_diversity_gates");
            continue;
        }
...
function applyMiddleGradesMediumStrongEvidencePreference(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map((candidate) => finalReturnedRootKey(candidate) || seriesKey(candidate)).filter(Boolean));
    const mediumStrongPool = rankedCandidates
        ...
        if (selected.includes(candidate))
            return false;
        const evidence = middleGradesRouteAlignmentEvidence(candidate);
        if (middleGradesEvidenceTierRank(evidence.tier) < middleGradesEvidenceTierRank("medium_evidence"))
            return false;
        if (isMiddleGradesTitleOnlyEvidence(candidate))
            return false;
        if (rejectReason(candidate, profile))
            return false;
        if (!middleGradesFinalEligibility(candidate).allowed)
            return false;
        if (selectedTitles().has(normalized(candidate.title)))
            return false;
        const root = finalReturnedRootKey(candidate) || seriesKey(candidate);
...
function applyMiddleGradesFinalCountRecovery(rankedCandidates, selected, rejectedReasons, profile, limit) {
    if (profile.ageBand !== "preteens" || selected.length >= Math.min(5, limit))
        return;
    const target = Math.min(5, limit);
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
    ...
    if (selected.length < target) {
        rejectedReasons.underfillRecoveryStoppedReason = safePool.length ? "safe_recovery_candidates_exhausted_before_target" : "no_safe_recovery_candidates_remaining";
    }
    else {
        rejectedReasons.underfillRecoveryStoppedReason = "count_contract_satisfied_after_final_recovery";
    }
    rejectedReasons.remainingUnattemptedReliableFallbacks = safePool
        .filter((candidate) => !selected.includes(candidate))
        .map((candidate) => String(candidate.diagnostics?.queryText || candidate.diagnostics?.queryFamily || "unknown"))
        .filter((value, index, array) => value && array.indexOf(value) === index)
        .slice(0, 8);
}
function applyMiddleGradesFinalReturnedRootCollapseAndRecovery(rankedCandidates, selected, rejectedReasons, profile, limit) {
    if (profile.ageBand !== "preteens" || selected.length === 0)
        return;
    const diagnostics = rejectedReasons;
    const target = Math.min(5, limit);
    const beforeCount = selected.length;
    const seenReturnedRoots = new Set();
    ...
    if (rootCollapseCausedUnderfill)
        diagnostics.rootCollapseCausedUnderfill = true;
    if (selected.length >= target)
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    ...
    if (rootCollapseCausedUnderfill && safeRoutePool.length > 0)
        diagnostics.recoveryAfterRootCollapseAttempted = true;
    for (const candidate of safeRoutePool) {
        if (selected.length >= target)
            break;
        const rootKey = finalReturnedRootKey(candidate);
        if (selectedTitles().has(normalized(candidate.title)) || (rootKey && selectedRoots().has(rootKey)))
            continue;
        ...
        selected.push(candidate);
        if (rootCollapseCausedUnderfill)
            rejectedReasons.recoveryAfterRootCollapseAcceptedCount = Number(rejectedReasons.recoveryAfterRootCollapseAcceptedCount || 0) + 1;
...
function isMiddleGradesTitleOnlyEvidence(candidate) {
    const evidence = middleGradesRouteAlignmentEvidence(candidate);
    if (evidence.tier === "query_only")
        return false;
    return evidence.fields.length > 0 && evidence.fields.every((field) => field === "title" || field === "subtitle");
}
function applyMiddleGradesTitleOnlyEvidenceCap(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    const maxTitleOnly = 3;
    const selectedTitleOnly = () => selected.filter(isMiddleGradesTitleOnlyEvidence);
    if (selectedTitleOnly().length <= maxTitleOnly)
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
...
function applyMiddleGradesRouteAlignmentReplacement(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const selectedRoots = () => new Set(selected.map(seriesKey).filter(Boolean));
    const routeAlignedPool = rankedCandidates.filter((candidate) => {
        if (!isMiddleGradesRouteAlignedSuccessCandidate(candidate))
            return false;
        if (isMiddleGradesHumorDefaultQueryFamily(candidate) || isMiddleGradesContemporarySchoolDefaultCandidate(candidate))
            return false;
        if (selected.includes(candidate))
            return false;
        if (rejectReason(candidate, profile))
    ...
    for (const candidate of routeAlignedPool) {
        ...
        if (candidateTierRank <= replacedTierRank && candidateAdjusted + 0.35 < middleGradesSelectionScore(replaced, profile))
            continue;
...
function addMiddleGradesSelectionObservability(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "preteens")
        return;
    const diagnostics = rejectedReasons;
    const selectedTitles = new Set(selected.map((candidate) => normalized(candidate.title)));
    const routeAlignmentScoreByTitle = {};
    const genreFacetMatchScoreByTitle = {};
    const queryTextSignalsRemovedFromTasteMatchByTitle = {};
    const documentOnlyTasteMatchByTitle = {};
    const fallbackPenaltyByTitle = {};
    const finalSelectionReasonByTitle = {};
    ...
    const middleGradesScoredCandidateAttribution = [];
    const genericTasteSignalsRemovedByTitle = {};
    const documentBackedTasteSignalsByTitle = {};
    const genericOnlyTasteMatchTitles = [];
    const queryLevelRouteAlignmentByTitle = {};
    const documentLevelRouteAlignmentByTitle = {};
    const routeAlignmentEvidenceFieldsByTitle = {};
    const documentEvidenceTierByTitle = {};
    const documentEvidenceTextByTitle = {};
    const routeAlignmentDemotedReasonByTitle = {};
    const finalEligibilityEvidenceByTitle = {};
    const finalEligibilityEvidenceFieldCountByTitle = {};
    const emergencyFallbackOverrideUsedByTitle = {};
    const routeAlignedCandidateRejectedForFallbackReason = {};
    const finalEligibilityRejectedQueryOnlyTitles = [];
    const humorKeywordOnlyLeakageByTitle = {};
    const humorKeywordOnlyRejectedTitles = [];
    const preteenAgeShapeEvidenceByTitle = {};
    const adultOrYaHumorLeakageRejectedTitles = [];
    for (const candidate of rankedCandidates) {
        ...
        const genreTasteScore = Math.round((Number(scoreBreakdown.genreFacetMatch || 0)
            + Number(scoreBreakdown.positiveTasteMatch || 0)) * 1000) / 1000;
        const penaltyScore = Math.round((Number(scoreBreakdown.avoidSignalPenalty || 0)
            + Number(scoreBreakdown.broadAvoidSignalPenalty || 0)
            - middleGradesFallbackPenalty(candidate)) * 1000) / 1000;
        ...
        if (candidate.diagnostics?.genericOnlyTasteMatch)
            genericOnlyTasteMatchTitles.push(candidate.title);
        ...
        if (finalEligibility.rejectedReason === "humor_keyword_only_leakage")
            humorKeywordOnlyRejectedTitles.push(candidate.title);
        if (finalEligibility.rejectedReason === "adult_or_ya_humor_leakage")
            adultOrYaHumorLeakageRejectedTitles.push(candidate.title);
        if (finalEligibility.emergencyOverride && selectedTitles.has(normalized(candidate.title)))
            emergencyFallbackOverrideUsedByTitle[candidate.title] = "explicit_emergency_fallback_after_better_options_failed";
        if (!finalEligibility.allowed && routeEvidence.queryLevel && !routeEvidence.documentLevel)
            finalEligibilityRejectedQueryOnlyTitles.push(candidate.title);
        if (routeEvidence.demotedReason)
            routeAlignmentDemotedReasonByTitle[candidate.title] = routeEvidence.demotedReason;
        ...
        if (finalEligibility.emergencyOverride && selectedTitles.has(normalized(candidate.title)))
            candidate.diagnostics.emergencyFallbackOverrideUsed = true;
        if (routeEvidence.demotedReason)
            candidate.diagnostics.routeAlignmentDemotedReason = routeEvidence.demotedReason;
    }
    ...
    if (selectedHasFallback) {
        for (const candidate of rejectedRouteAlignedForFallback)
            routeAlignedCandidateRejectedForFallbackReason[candidate.title] = candidate.rejectedReasons.join(",") || "route_aligned_candidate_lost_to_fallback_selection";
    }
    ...
    const titleOnlyEvidenceFinalEligibleTitles = selected
        ...
        .map((candidate) => candidate.title);
    const repeatedTitleTokenCounts = selected.reduce((acc, candidate) => {
    ...
    const hasRepeatedClusterTitleToken = (candidate) => Boolean(repeatedTitleTokenClusterDetected
        && repeatedTitleTokenClusterToken
        && normalized(candidate.title).split(" ").includes(repeatedTitleTokenClusterToken));
    const isCleanFinalEligibleCandidate = (candidate) => isMiddleGradesCleanFinalCandidate(candidate)
    ...
    const recoveryShortfallGateReason = (candidate) => {
        if (selectedTitles.has(normalized(candidate.title)))
            return "selected";
        const joinedRejected = candidate.rejectedReasons.join(",");
        if (/query_only_score_cap|query_only/i.test(joinedRejected))
            return "recovery_query_quality_query_only_cap";
        if (/humor_keyword_only_leakage|adult_or_ya_humor_leakage/i.test(joinedRejected))
            return "humor_or_leakage_rejection";
        if (/duplicate|same_root|same_series|root_collapsed/i.test(joinedRejected))
            return "duplicate_root_suppression";
        const finalEligibility = middleGradesFinalEligibility(candidate);
        if (!finalEligibility.allowed) {
            if (/query_only|missing.*document|evidence/i.test(String(finalEligibility.rejectedReason || "")))
                return "missing_document_evidence";
            if (/humor|leakage/i.test(String(finalEligibility.rejectedReason || "")))
                return "humor_or_leakage_rejection";
            return `final_eligibility_${finalEligibility.rejectedReason || "rejected"}`;
        }
        const tasteEligibility = middleGradesMeaningfulTasteEligibility(candidate, true);
        if (!tasteEligibility.allowed)
            return "missing_document_backed_taste_evidence";
        if (candidate.score <= 0 && !isContemporaryLowScoreAcceptable(candidate, profile))
            return "non_positive_scoring";
        const rootKey = finalReturnedRootKey(candidate) || seriesKey(candidate);
        if (rootKey && selectedRootKeysForRecovery.has(rootKey))
            return "duplicate_root_suppression";
    ...
    const middleGradesRecoveryRejectedReasonCounts = recoveryRejectedRows.reduce((acc, row) => {
        acc[row.reason] = Number(acc[row.reason] || 0) + 1;
        return acc;
    }, {});
    const middleGradesRecoveryBestRejectedTitlesByReason = recoveryRejectedRows.reduce((acc, row) => {
    ...
    const lockQualityFailReasons = [];
    if (selected.length !== 5)
        lockQualityFailReasons.push("final_items_length_not_five");
    if (selected.length >= 5 && finalEligibilityCleanCandidateCount < 5 && meaningfulTasteRecoveryCandidates.length === 0)
        lockQualityFailReasons.push("final_clean_items_less_than_five");
    if (finalCountContractStatus === "underfilled_fallback_only" || finalCountContractStatus === "full_fallback_only" || finalCountContractStatus === "full_weak_evidence")
        lockQualityFailReasons.push(finalCountContractStatus);
    if (weakEvidenceOnlySlate)
        lockQualityFailReasons.push("weak_evidence_only_slate");
    if (selectedTitleOnlyCount > 0 && selectedMediumStrongEvidenceCount === 0)
        lockQualityFailReasons.push("title_only_slate_downgraded_lock_quality");
    if (underfilledBecauseOnlyWeakOrZeroTaste)
        lockQualityFailReasons.push("underfilled_because_only_weak_or_zero_taste");
    if (emergencyFallbackUsedForZeroTasteFill)
        lockQualityFailReasons.push("emergency_fallback_used_for_zero_taste_fill");
    if (sameSeriesTitleOnlyClusterDetected)
        lockQualityFailReasons.push("same_series_title_only_cluster_detected");
    if (repeatedTitleTokenClusterDetected)
        lockQualityFailReasons.push("repeated_title_token_cluster_detected");
    if (finalCountContractStatus === "full_mixed_recovery" && selectedRouteAlignedCount < 2)
        lockQualityFailReasons.push("mixed_recovery_has_fewer_than_two_route_aligned_items");
    if (documentRouteAlignmentEvidenceMissingButTrueCount > 0)
        lockQualityFailReasons.push("document_route_alignment_missing_evidence_fields");
    if (rejectedReasons.genericDefaultSlateDetected && selected.length >= 5)
        lockQualityFailReasons.push("generic_default_slate_detected_without_true_shortage");
    if (!genericAdventureUsedAsLastResortOnly)
        lockQualityFailReasons.push("generic_adventure_used_before_last_resort");
    if (weakEvidenceSelectedOverStrongEvidence)
        lockQualityFailReasons.push("weak_evidence_selected_over_strong_evidence");
    if (genericFunnySlateDetected)
        lockQualityFailReasons.push("generic_funny_slate_detected_without_non_humor_alignment");
    ...
    if (!diagnostics.underfillRecoveryStoppedReason)
        diagnostics.underfillRecoveryStoppedReason = selected.length >= 5 ? "count_contract_satisfied" : "not_enough_safe_candidates_after_selection";
    if (!diagnostics.remainingUnattemptedReliableFallbacks)
        diagnostics.remainingUnattemptedReliableFallbacks = [];
    ...
    if (selectedFallbackCount > 0 && rejectedRouteAlignedCount > 0)
        rejectedReasons.middle_grades_fallback_survived_with_route_aligned_available = selectedFallbackCount;
}
function isKidsSuspiciousSelectionCandidate(candidate) {
    return /^(?:the )?(?:friends|lantern archive)$/i.test(String(candidate.title || "").trim());
}
function kidsTasteScore(candidate) {
    const breakdown = candidate.scoreBreakdown || {};
    return Math.round((Number(breakdown.genreFacetMatch || 0)
        + Number(breakdown.positiveTasteMatch || 0)
        + Number(breakdown.toneMatch || 0)
        + Number(breakdown.themeMatch || 0)) * 1000) / 1000;
}
function kidsDistinctiveTasteSignals(candidate) {
    const matchedSignals = Array.isArray(candidate.matchedSignals) ? candidate.matchedSignals.map(String) : [];
...
function kidsNonTitleDocumentText(candidate) {
    const raw = candidate.raw;
    const rawDescription = typeof raw?.description === "string" ? raw.description : raw?.description?.value;
...
function kidsObviousNonK2CleanLeakage(candidate) {
    const text = normalized([candidate.title, candidate.subtitle, kidsNonTitleDocumentText(candidate)].filter(Boolean).join(" "));
    if (/\bwatership down\b/.test(text))
        return true;
    if (kidsCollectionArtifactTitleShape(candidate))
        return true;
    if (/\b(adult coloring|adult colouring|for adults|adult picture book|young adult|teen|teens|ya)\b/.test(text))
        return true;
    if (/\b(coloring|colouring|workbook|worksheet|activity book|activities|puzzles?|picture dictionary|dictionary|encyclopedia|reference|field guide|identification guide|guidebook|handbook|manual|first words|look and find|search and find|i spy|spot the)\b/.test(text))
        return true;
...
function kidsWeakOrderingPenalty(candidate) {
    const text = normalized([candidate.title, candidate.subtitle, kidsNonTitleDocumentText(candidate)].filter(Boolean).join(" "));
    let penalty = 0;
    if (/\b(bible|biblical|jesus|moses|noah|christian|prayer|religion|saints?)\b/.test(text))
        penalty += 4;
    if (/\b(folklore|folk tale|folktale|mythology|myths?|legend|legends)\b/.test(text))
        penalty += 2;
    if (/\b(christmasaurus|mr mischief|madeleine|fawn|animal talent fairy|fairies|spider man|spiderman|superman|batman|star wars|pokemon|marvel|disney)\b/.test(text))
        penalty += 3;
    if (/\bwho will be my friends\b/.test(text))
        penalty += 3;
    if (kidsWeakFallbackTitleShape(candidate) && !kidsPreferredK2FallbackTitle(candidate))
        penalty += 2;
    if (!kidsHasStrongStoryReaderEvidence(candidate) && !kidsDistinctiveSignalsSupportedByDocument(candidate).length && !kidsPreferredK2FallbackTitle(candidate))
        penalty += 1.5;
    if (kidsTasteScore(candidate) < 0.75 && kidsDistinctiveSignalsSupportedByDocument(candidate).length === 0 && !kidsPreferredK2FallbackTitle(candidate))
        penalty += 2;
    return penalty;
}
function kidsFinalSelectionScore(candidate, profile) {
    const breakdown = candidate.scoreBreakdown || {};
...
function compareKidsFinalSelectionCandidates(a, b, profile) {
...
function kidsQueryAnchoredStoryCandidate(candidate) {
...
function kidsNonNarrativeInformationalArtifact(candidate) {
...
function kidsProfileCoverageSignals(profile) {
...
function kidsCandidateCoverageSignals(candidate, profile) {
    const text = kidsNonTitleDocumentText(candidate);
    return Array.from(new Set(kidsProfileCoverageSignals(profile).filter((signal) => text.includes(signal)))).slice(0, 12);
}
function applyKidsProfileCoverageDiversification(rankedCandidates, selected, rejectedReasons, profile, limit) {
    if (profile.ageBand !== "kids" || selected.length < Math.min(3, limit))
        return;
    const targetSignals = kidsProfileCoverageSignals(profile);
    if (targetSignals.length < 2)
        return;
    const selectedTitles = () => new Set(selected.map((candidate) => normalized(candidate.title)));
    const coverageCounts = () => {
        const counts = new Map();
        for (const candidate of selected) {
            for (const signal of kidsCandidateCoverageSignals(candidate, profile))
    ...
    while (replacements < 2) {
        ...
        if (replacement.candidate.score < replaced.score - 5)
            break;
...
function kidsQualityAuditRow(candidate, selectedTitles, label = "candidate") {
    const breakdown = candidate.scoreBreakdown || {};
    const matchedSignals = Array.isArray(candidate.matchedSignals) ? candidate.matchedSignals.map(String) : [];
    return {
        title: candidate.title,
...
function addKidsSelectionObservability(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "kids")
        return;
    const diagnostics = rejectedReasons;
    const selectedTitles = new Set(selected.map((candidate) => normalized(candidate.title)));
    ...
    const lockQualityFailReasons = [];
    if (selected.length < 5 && !cleanUnderfilledSlate)
        lockQualityFailReasons.push("final_items_length_less_than_five");
    if (finalEligibilityAcceptedTitles.length < Math.min(5, selected.length || 5))
        lockQualityFailReasons.push("k2_clean_items_less_than_five");
    if (selectedSuspiciousTitles.length > 0)
        lockQualityFailReasons.push("k2_suspicious_title_selected");
    diagnostics.rankedDocsTitles = rankedCandidates.map((candidate) => candidate.title);
    diagnostics.finalEligibilityAcceptedTitles = finalEligibilityAcceptedTitles;
...
function teenOpenLibraryDiagnosticSignals(candidate, field) {
    const diagnosticSignals = candidate.diagnostics?.[field];
    if (Array.isArray(diagnosticSignals))
        return Array.from(new Set(diagnosticSignals.map(normalized).filter(Boolean)));
    if (field === "metadataBackedMatchedLikedSignals")
        return middleGradesDocumentBackedTasteSignals(candidate);
    return [];
}
function teenOpenLibraryNonTitleMetadataText(candidate) {
    const raw = (candidate.raw || {});
    return normalized([
        candidate.description,
...
function teenOpenLibrarySignalSupportedByNonTitleMetadata(signal, metadataText) {
    const value = normalized(signal);
    if (!value)
        return false;
    return (0, score_1.signalPresentInText)(metadataText, value);
}
function teenOpenLibraryAuthoritySignalsFromChecks(metadataText, checks) {
    const authoritySignals = [];
    for (const [label, pattern] of checks) {
        if (pattern.test(metadataText))
            authoritySignals.push(label);
    }
    return uniqueSignals(authoritySignals);
}
function teenOpenLibraryStrongAuthoritySignals(metadataText) {
    return teenOpenLibraryAuthoritySignalsFromChecks(metadataText, [
        ...
        ["adolescent fiction", /\b(adolescent|adolescence|teenage)\b.{0,40}\b(fiction|novel|story|stories)\b|\b(fiction|novel|story|stories)\b.{0,40}\b(adolescent|adolescence|teenage)\b/],
        ["high school fiction", /\bhigh school\b.{0,40}\b(fiction|novel|story|stories)\b|\b(fiction|novel|story|stories)\b.{0,40}\bhigh school\b/],
        ["boarding school fiction", /\b(boarding school|boarding schools|preparatory school|prep school)\b.{0,40}\b(fiction|novel|story|stories)\b|\b(fiction|novel|story|stories)\b.{0,40}\b(boarding school|boarding schools|preparatory school|prep school)\b/],
        ["school fiction", /\bschool(?:s| students?)?\b.{0,40}\b(fiction|novel|story|stories)\b|\b(fiction|novel|story|stories)\b.{0,40}\bschool(?:s| students?)?\b/],
    ]);
}
function teenOpenLibraryWeakAuthoritySignals(metadataText) {
    return teenOpenLibraryAuthoritySignalsFromChecks(metadataText, [
...
function teenOpenLibraryAuthoritySignals(metadataText) {
    return uniqueSignals([...teenOpenLibraryStrongAuthoritySignals(metadataText), ...teenOpenLibraryWeakAuthoritySignals(metadataText)]);
}
function teenOpenLibraryNarrativeFictionShape(metadataText) {
    return /\b(young adult fiction|american young adult fiction|juvenile fiction|teen fiction|adolescent fiction|high school fiction|boarding school fiction|school fiction|fiction|novel|novels|stories|horror stories|horror tales|fantasy fiction|science fiction|romance fiction|mystery fiction|dystopian fiction|thrillers? and suspense)\b/.test(metadataText);
}
function teenOpenLibraryNonNarrativeShapeReasons(metadataText, hasTeenAuthority) {
    const reasons = [];
    const checks = [
...
function teenOpenLibraryMeaningfulTasteEligibility(candidate, profile) {
    if (profile.ageBand !== "teens" || candidate.source !== "openLibrary") {
        return { allowed: true, signals: [], nonTitleSignals: [], contentSignals: [], contextOnlySignals: [], authoritySignals: [], strongAuthoritySignals: [], weakAuthoritySignals: [], likedDislikedOverlapSignals: [], narrativeFictionShape: false, nonNarrativeShapeReasons: [] };
    }
    ...
    const strongAuthoritySignals = teenOpenLibraryStrongAuthoritySignals(nonTitleMetadataText);
    const weakAuthoritySignals = teenOpenLibraryWeakAuthoritySignals(nonTitleMetadataText);
    ...
    const narrativeFictionShape = teenOpenLibraryNarrativeFictionShape(nonTitleMetadataText);
    ...
    if (positiveTasteScore <= 0)
        return { allowed: false, reason: "teen_openlibrary_no_positive_metadata_taste", ...baseResult };
    if (!likedSignals.length)
        return { allowed: false, reason: "teen_openlibrary_no_metadata_liked_signals", ...baseResult };
    ...
    if (nonNarrativeShapeReasons.length > 0)
        return { allowed: false, reason: "teen_openlibrary_non_narrative_or_adult_shape", ...resultEvidence };
    if (!contentSignals.length)
        return { allowed: false, reason: nonTitleLikedSignals.length ? "teen_openlibrary_context_or_generic_only_metadata_taste" : "teen_openlibrary_title_only_metadata_taste", ...resultEvidence };
    ...
    if (!hasAllowedTasteEvidence)
        return { allowed: false, reason: "teen_openlibrary_single_broad_metadata_taste", ...resultEvidence };
...
function addTeenOpenLibrarySelectionObservability(rankedCandidates, selected, rejectedReasons, profile) {
    if (profile.ageBand !== "teens" || !rankedCandidates.some((candidate) => candidate.source === "openLibrary"))
        return;
    const diagnostics = rejectedReasons;
    const selectedTitles = new Set(selected.map((candidate) => normalized(candidate.title)));
    ...
    const teenOpenLibraryContentSignals = {};
    const teenOpenLibraryContextOnlySignals = {};
    const teenOpenLibraryAuthoritySignals = {};
    const teenOpenLibraryStrongAuthoritySignals = {};
    const teenOpenLibraryWeakAuthoritySignals = {};
...
function rejectReason(candidate, profile) {
    ...
    if (profile.ageBand === "preteens") {
        ...
        const eligibility = middleGradesFinalEligibility(candidate);
        if (!eligibility.allowed)
            return eligibility.rejectedReason || "middle_grades_final_eligibility_missing_evidence";
        if (middleGradesNonNarrativeInformationalArtifact(candidate) && !profileExplicitlyRequestsNonfictionReference(profile))
            return "middle_grades_non_narrative_informational_artifact";
        const tasteEligibility = middleGradesMeaningfulTasteEligibility(candidate, true);
        if (!tasteEligibility.allowed)
...
function nonPositiveScoreDetail(candidate) {
    const breakdown = candidate.scoreBreakdown || {};
    return [
...
function isLowScoreRescueCandidate(candidate) {
    const breakdown = candidate.scoreBreakdown || {};
    const sourceQuality = Number(breakdown.sourceQualityRelevance || 0);
...
function recordRejected(candidate, rejectedReasons, reason) {
...
function selectRecommendations(candidates, profile, limit = 10) {
    ...
    const adultWeakOpenLibraryCandidates = [];
    ...
    for (const candidate of rankedCandidates) {
        ...
        if (reason) {
            ...
            if (reason === "non_positive_score") {
                ...
                if (isLowScoreRescueCandidate(candidate))
    ...
    if (selected.length === 0 && lowScoreRescue.length > 0) {
        ...
        for (const candidate of lowScoreRescue.sort((a, b) => b.score - a.score)) {
            ...
            if (recurringClusterKey)
                seenRecurringOpenLibraryClusters.add(recurringClusterKey);
            selected.push(candidate);
            if (selected.length >= Math.min(5, Math.max(3, lowScoreRescue.length)))
                break;
        }
    }
    ...
    if (profile.ageBand === "teens" && selected.length < Math.min(5, limit) && candidates.some((candidate) => candidate.source === "openLibrary")) {
        ...
        for (const row of deferred) {
            if (selected.length >= teenOpenLibraryTarget)
                break;
            if (row.candidate.source !== "openLibrary")
                continue;
            if (selected.includes(row.candidate))
                continue;
            const titleKey = normalized(row.candidate.title);
            if (seenTitles.has(titleKey))
                continue;
    ...
    if (profile.ageBand === "preteens" && selected.length < Math.min(5, limit) && candidates.some((candidate) => candidate.source === "openLibrary")) {
        ...
        for (const row of deferred) {
            if (selected.length >= middleGradesOpenLibraryTarget)
                break;
            if (row.candidate.source !== "openLibrary")
                continue;
            if (selected.includes(row.candidate))
    ...
    for (const row of deferred) {
        if (!selected.includes(row.candidate))
            recordRejected(row.candidate, rejectedReasons, row.reason);
    }
    for (const candidate of adultWeakOpenLibraryCandidates) {
        if (!selected.includes(candidate))
            recordRejected(candidate, rejectedReasons, "adult_weak_openlibrary_source_quality");
    }
    ...
    if (openLibraryOnlySlate && selected.length > 5 && meaningfulQualityCount < 6) {
        ...
        for (const candidate of removed)
            recordRejected(candidate, rejectedReasons, "openlibrary_quality_cap_weak_slate");
    }
    applyMiddleGradesHumorDefaultCap(rankedCandidates, selected, rejectedReasons, profile);
    applyMiddleGradesAntiZeroFallbackGate(rankedCandidates, selected, rejectedReasons, profile);
