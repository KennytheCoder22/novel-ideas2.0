...
exports.buildSearchPlan = buildSearchPlan;
const DEFAULT_SOURCE_TIMEOUT_MS = 2500;
const OPEN_LIBRARY_SOURCE_TIMEOUT_MS = 8000;
const ADULT_OPEN_LIBRARY_SOURCE_TIMEOUT_MS = 22000;
function topValues(rows, count) {
    return rows.slice(0, count).map((row) => row.value).filter(Boolean);
}
function normalizedTerm(value) {
    return String(value || "").toLowerCase().replace(/[^a-z0-9\s-]/g, " ").replace(/\s+/g, " ").trim();
}
function uniqueTerms(values, limit = 4) {
    const seen = new Set();
    const output = [];
...
function googleBooksAgePrefix(ageBand) {
    if (ageBand === "teens")
        return "young adult";
    if (ageBand === "preteens")
        return "middle grade";
...
function googleBooksGenreAnchor(value) {
    const normalized = normalizedTerm(value);
    if (!normalized)
        return undefined;
    if (/\bscience fiction|sci fi|sci-fi|speculative\b/.test(normalized))
        return "science fiction";
    if (/\bdystopian|dystopia\b/.test(normalized))
        return "dystopian";
    if (/\bfantasy\b/.test(normalized))
        return "fantasy";
    if (/\bmystery\b/.test(normalized))
        return "mystery";
    if (/\bthriller|suspense\b/.test(normalized))
        return "thriller";
    if (/\bhorror|supernatural|paranormal\b/.test(normalized))
        return "horror";
    if (/\bromance\b/.test(normalized))
        return "romance";
    if (/\bhistorical|history\b/.test(normalized))
        return "historical";
    if (/\badventure|survival|quest\b/.test(normalized))
        return "adventure";
    if (/\bmythology|mythological|mythologies\b/.test(normalized))
        return "mythology fantasy";
    if (/\bsuperhero|superheroes\b/.test(normalized))
        return "superhero adventure";
    if (/\bcomedy|humou?r|funny\b/.test(normalized))
        return "humorous";
    if (/\bcontemporary|realistic|drama|literary\b/.test(normalized))
        return "contemporary";
    return undefined;
}
function googleBooksDescriptor(value) {
    const normalized = normalizedTerm(value);
    if (!normalized)
        return undefined;
    if (/\bcomedy|humou?r|funny\b/.test(normalized))
        return "humorous";
    if (/\bdark|gothic\b/.test(normalized))
        return "dark";
    if (/\bpsychological\b/.test(normalized))
        return "psychological";
    if (/\bquirky|offbeat|weird\b/.test(normalized))
        return "quirky";
    if (/\bcozy|gentle|warm\b/.test(normalized))
        return "cozy";
    if (/\bepic\b/.test(normalized))
        return "epic";
    return undefined;
}
function googleBooksAdjacentGenre(primary) {
    if (!primary)
        return undefined;
    if (primary === "dystopian" || primary === "science fiction")
        return "mystery";
    if (primary === "horror")
        return "mystery";
    if (primary === "fantasy")
        return "adventure";
    if (primary === "mystery" || primary === "thriller")
        return "historical";
    return "fiction";
}
function googleBooksAdultNarrativeQuery(genre, descriptor) {
...
function googleBooksAdultPairedNarrativeQuery(primaryGenre, secondaryGenre, descriptor) {
...
function kidsGoogleBooksTheme(signals) {
    const normalized = signals.map((value) => normalizedTerm(value)).join(" ");
    if (/\b(humorous|humour|humor|funny|laugh)\b/.test(normalized))
        return "humorous";
    if (/\b(animal|animals|pet|pets|dog|dogs|cat|cats)\b/.test(normalized))
        return "animal";
    if (/\b(friendship|friend|friends)\b/.test(normalized))
        return "friendship";
    if (/\b(nature|forest|garden|ocean|sea|river|weather)\b/.test(normalized))
        return "nature";
    if (/\b(fantasy|magic|magical|dragon|fairy)\b/.test(normalized))
        return "fantasy";
    if (/\b(adventure|quest|journey|explore)\b/.test(normalized))
        return "adventure";
    if (/\b(alphabet|letters|phonics)\b/.test(normalized))
        return "alphabet";
    if (/\b(rhythm|rhythmical|rhyme|rhyming|repetition|repetitive)\b/.test(normalized))
        return "rhyming";
    return "general";
}
function kidsGoogleBooksThemeFromSignal(value) {
    const normalized = normalizedTerm(value);
    if (!normalized)
        return undefined;
    if (/\b(humorous|humour|humor|funny|laugh)\b/.test(normalized))
...
function kidsGoogleBooksThemeCandidates(genres, tones, themes, formats, blockedFamilies) {
    const orderedSignals = [...genres, ...themes, ...tones, ...formats];
    const seen = new Set();
    const orderedFamilies = [];
    for (const signal of orderedSignals) {
        const family = kidsGoogleBooksThemeFromSignal(signal);
        if (!family || seen.has(family) || blockedFamilies.has(family))
            continue;
        seen.add(family);
        orderedFamilies.push(family);
    }
    const fallback = kidsGoogleBooksTheme(orderedSignals);
    if (fallback !== "general" && !seen.has(fallback) && !blockedFamilies.has(fallback))
        orderedFamilies.push(fallback);
    return orderedFamilies.length ? orderedFamilies : ["general"];
}
function kidsThemePhrase(theme) {
    if (theme === "general")
        return "";
    if (theme === "rhyming")
        return "rhyming";
...
function kidsGoogleBooksSupportsEarlyReader(signals) {
    const normalized = signals.map((value) => normalizedTerm(value)).join(" ");
...
function kidsGoogleBooksSupportsFantasyRetrieval(signals) {
    const normalized = signals.map((value) => normalizedTerm(value)).join(" ");
    return /\b(fantasy|magic|magical|dragon|dragons|wizard|witch|fairy|myth|mythology)\b/.test(normalized);
}
function kidsGoogleBooksFormatQuery(theme, format) {
...
function buildKidsGoogleBooksIntents(profile, genres, tones, themes, formats) {
    const signalPool = [...genres, ...themes, ...tones, ...formats];
    const isLikeBacked = (evidence) => evidence.some((entry) => /^like:/i.test(String(entry || "")));
    const positiveFamilies = Array.from(new Set([...profile.genreFamily, ...profile.themes, ...profile.tone]
        .filter((row) => isLikeBacked(row.evidence || []))
        .map((row) => kidsGoogleBooksThemeFromSignal(row.value))
        .filter(Boolean)));
    const blockedFamilies = new Set((profile.avoidSignals || [])
    ...
    const themeByQuery = {};
    const rungByQuery = {};
    const duplicateSuppressionReasons = [];
    const seenQueryToRung = new Map();
    const intents = [];
...
function teenGoogleBooksCanonicalFamilyQuery(agePrefix, family) {
    ...
    if (normalizedFamily === "fiction")
        return `${agePrefix} fiction novel`;
    return uniqueTerms([agePrefix, normalizedFamily, "fiction", "novel"], 4).join(" ");
}
function teenGoogleBooksScienceFictionCompositeOverrideQueries() {
...
function teenGoogleBooksScienceFictionOverrideFamily(query) {
    const normalized = normalizedTerm(query);
    if (normalized.includes("dystopian"))
        return "dystopian";
    if (normalized.includes("space opera"))
        return "space_opera";
    if (normalized.includes("speculative"))
        return "speculative";
    if (normalized.includes("sci fi") || normalized.includes("science fiction"))
        return "science_fiction";
    return "science_fiction";
}
function teenGoogleBooksScienceFictionProductionCompositeQueries(primaryOverride) {
    const primary = String(primaryOverride || "").trim() || "young adult sci fi";
    return [primary, "YA dystopian", "YA speculative fiction"];
}
function teenGoogleBooksDistinctThirdQuery(agePrefix, primaryGenre, secondaryGenre, adjacentGenre) {
    const primary = normalizedTerm(primaryGenre || "");
    const secondary = normalizedTerm(secondaryGenre || "");
...
function buildTeenGoogleBooksIntents(profile, genres, tones, themes, formats) {
    const agePrefix = googleBooksAgePrefix(profile.ageBand);
    const genreAnchors = uniqueTerms(genres.map(googleBooksGenreAnchor).filter(Boolean), 2);
    const descriptors = uniqueTerms([...tones, ...themes].map(googleBooksDescriptor).filter(Boolean), 1);
    ...
    const candidates = scienceFictionCompositeOverrideApplied
...
function buildGoogleBooksIntents(profile, genres, tones, themes, formats) {
    ...
    const genreAnchors = profile.ageBand === "preteens"
        ...
        : uniqueTerms(genres.map(googleBooksGenreAnchor).filter(Boolean), 2);
    const descriptors = uniqueTerms([...tones, ...themes].map(googleBooksDescriptor).filter(Boolean), 1);
    ...
    const adjacentOrToneQuery = uniqueTerms([
        ...(useAgePrefix ? [agePrefix] : []),
        ...(profile.ageBand === "adult"
            ? [googleBooksAdultPairedNarrativeQuery(primaryGenre, secondaryGenre && secondaryGenre !== primaryGenre ? secondaryGenre : adjacentGenre, primaryDescriptor)]
            : [
    ...
    if (profile.ageBand === "preteens") {
        ...
        const skippedAfterSatisfied = [];
        const satisfiedAnchors = new Set();
        for (const g of extendedGenreRaw) {
            if (satisfiedAnchors.size >= 2) {
                ...
                if (a)
                    satisfiedAnchors.add(a);
            }
        }
        const anchorBySignal = {};
        ...
        const genreSignalScoreHistogram = {};
...
function buildSearchPlan(profile, enabledSources = {}) {
    ...
    const allSources = ["mock", "googleBooks", "openLibrary", "kitsu", "comicVine", "localLibrary", "nyt"];
    const sourcePlans = allSources.map((source) => {
