...
const ADULT_OPEN_LIBRARY_FIRST_RUN_RETRY_TIMEOUT_MS = 2500;
const ADULT_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS = 19000;
const TEEN_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS = 4500;
const TEEN_OPEN_LIBRARY_TIMEOUT_CASCADE_SPECIFIC_QUERY_CAP_MS = 1500;
const TEEN_OPEN_LIBRARY_TIMEOUT_CASCADE_SPECIFIC_QUERY_FLOOR_MS = 500;
const TEEN_OPEN_LIBRARY_DELAYED_RETRY_MIN_BUDGET_MS = 1000;
const TEEN_OPEN_LIBRARY_TIMEOUT_CIRCUIT_BREAKER_LIMIT = 3;
const TEEN_OPEN_LIBRARY_SOURCE_CANDIDATE_POOL_LIMIT = 10;
const MIDDLE_GRADES_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS = 1500;
const K2_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS = 1500;
const MIDDLE_GRADES_OPEN_LIBRARY_INITIAL_QUERY_TIMEOUT_MS = 1500;
const MIDDLE_GRADES_OPEN_LIBRARY_TARGETED_QUERY_CAP_MS = 1500;
...
const MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_CAP_MS = 1500;
const MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_FLOOR_MS = 1000;
const MIDDLE_GRADES_OPEN_LIBRARY_DELAYED_RETRY_MIN_BUDGET_MS = 1000;
const MIDDLE_GRADES_OPEN_LIBRARY_FINAL_SAFE_RECOVERY_MIN_BUDGET_MS = 1000;
const MIDDLE_GRADES_OPEN_LIBRARY_POST_FALLBACK_ROUTE_RECOVERY_MIN_BUDGET_MS = 1000;
const MIDDLE_GRADES_OPEN_LIBRARY_TOTAL_BUDGET_MS = 24000;
const MIDDLE_GRADES_OPEN_LIBRARY_DEBUG_TOTAL_BUDGET_MS = 180000;
const MIDDLE_GRADES_OPEN_LIBRARY_DEBUG_PER_QUERY_BUDGET_MS = 20000;
...
function dedupeOpenLibraryTerms(value) {
    ...
    for (const [pattern, replacement] of protectedPhrases)
        protectedValue = protectedValue.replace(pattern, replacement);
...
function isUsefulOpenLibraryQueryPart(value) {
    if (!value)
        return false;
    if (value.length < 3)
        return false;
    if (/^(and|or|the|with|for|adult|kids|preteens|children)$/.test(value))
        return false;
    if (ABSTRACT_OPEN_LIBRARY_TERMS.has(value))
        return false;
    if (MEDIA_FORMAT_TERMS.has(value))
        return false;
    return true;
}
function isGenreLikeOpenLibraryPart(value) {
    return isUsefulOpenLibraryQueryPart(value) && GENRE_QUERY_HINT.test(value);
}
function queryFamilyForOpenLibraryQuery(query) {
    const q = query.toLowerCase();
    if (/\b(horror|paranormal|supernatural|dark fantasy)\b/.test(q))
        return "horror_paranormal";
    if (/\bmystery|thriller|suspense\b/.test(q))
        return "mystery_thriller";
    if (/\b(contemporary|realistic|coming of age)\b/.test(q))
        return "contemporary_drama";
    if (/\bfantasy\b/.test(q))
        return "fantasy";
    if (/\bscience fiction|sci-fi|speculative|dystopia|dystopian\b/.test(q))
        return "speculative";
    if (/\badventure|action|survival\b/.test(q))
        return "adventure";
    if (/\bdrama\b/.test(q))
        return "contemporary_drama";
    if (/\bromance|historical\b/.test(q))
        return "romance_historical";
    if (/\bcomedy|humor\b/.test(q))
        return "comedy";
    if (/\bgraphic novel|manga|comic\b/.test(q))
        return "graphic";
    return "open_library_broad";
}
function teenOpenLibrarySourceFamilyForQuery(query) {
    const normalized = cleanOpenLibraryQueryPart(query);
    if (/\b(teen adventure|action adventure|survival fiction|dystopian survival|fantasy survival)\b/.test(normalized))
        return "adventure";
    if (/\b(young adult dystopian|dystopian|science fiction|sci-fi|speculative|space)\b/.test(normalized))
        return "speculative";
    if (/\b(young adult horror|survival horror|horror thriller)\b/.test(normalized))
        return "horror";
    if (/\b(coming of age|young adult contemporary|teen realistic|contemporary fantasy)\b/.test(normalized))
        return "contemporary";
    if (/\b(historical drama|teen historical|historical fiction)\b/.test(normalized))
        return "historical";
    if (/\b(young adult romance|historical romance|romance fantasy)\b/.test(normalized))
        return "romance";
    if (/\bsports?\b/.test(normalized))
        return "sports";
    if (/\b(fantasy|magic|magical|paranormal|supernatural)\b/.test(normalized))
        return "fantasy";
    const queryFamily = queryFamilyForOpenLibraryQuery(normalized);
    return queryFamily === "open_library_broad" ? "" : queryFamily;
}
function nonSkipSignalWeight(rows, pattern) {
    return rows.reduce((sum, row) => {
        if (!pattern.test(String(row.value || "").toLowerCase()))
            return sum;
...
function likedSignalWeight(rows, pattern) {
    return rows.reduce((sum, row) => {
        if (!pattern.test(String(row.value || "").toLowerCase()))
            return sum;
        const evidence = Array.isArray(row.evidence) ? row.evidence : [];
...
function maxLikedSignalWeight(rows, pattern) {
...
function hasNonSkipSignal(rows, pattern) {
...
function combineOpenLibraryQueryParts(primary, modifier) {
...
function isTeenBroadFallbackOpenLibraryQuery(query) {
...
function buildTeenOpenLibraryQueryPlans(plan, profile, ageProfile) {
    ...
    const branchFamilyCoverage = new Set();
    for (const query of queryCandidates) {
        if (!preservedKnownGoodQueries.test(String(query || "")))
            continue;
        const sourceFamily = teenOpenLibrarySourceFamilyForQuery(String(query || ""));
        if (sourceFamily)
            branchFamilyCoverage.add(sourceFamily);
    }
    const familyPrefixQueries = [];
    const familyPrefixCoverage = new Set();
    for (const row of balancedPrefixFamilyRows) {
        if (familyPrefixQueries.length >= 3)
            break;
        if (branchFamilyCoverage.has(row.family) || familyPrefixCoverage.has(row.family))
            continue;
...
function buildAdultOpenLibraryQueryPlans(plan, profile, ageProfile) {
    ...
    const queryCandidates = (() => {
        if (wantsAdultFantasyWarCrime)
            return ["dark fantasy", "fantasy war", "political fantasy", "crime thriller"];
        if (wantsAdultDystopianFantasySurvival)
            return ["dystopian adventure", "post-apocalyptic fiction", "survival fiction", "adventure fiction"];
        if (wantsAdultHistoricalCrimeDrama)
            return ["crime drama", "historical thriller", "historical mystery", wantsAdultLiteraryCrime ? "literary crime" : "realistic drama"];
        if (wantsAdultGothicHorrorFantasy)
            return ["gothic horror", "supernatural horror", "dark fantasy", "contemporary gothic fiction"];
        if (wantsAdultFantasyHistoricalSurvival)
            return adultRomanceWeight >= Math.max(1.25, adultCrimeWeight)
        ...
        if (wantsAdultFantasyRomanceAdventure)
            return ["fantasy romance", "fantasy adventure", "mythological fantasy", "romantic fantasy"];
        if (wantsAdultFantasyAdventureMystery)
            return ["fantasy adventure", "dark fantasy", "speculative mystery", "science fiction mystery"];
        if (wantsAdultMixedSpeculative)
            return wantsAdultDystopianFirstMixedSpeculativeQueries
        ...
        if (wantsAdultSciFi)
            return ["science fiction thriller", "speculative fiction", "science fiction adventure"];
        if (wantsAdultCozyFantasy)
            return ["cozy fantasy", "contemporary fantasy", "fantasy fiction", "romance fantasy"];
        if (wantsAdultHistoricalSpeculativeThriller)
            return ["historical fiction", "historical thriller", "literary fiction", "speculative fiction"];
        if (wantsAdultCrimeThriller)
            return ["psychological thriller", "mystery thriller", "crime fiction", wantsAdultLiteraryCrime ? "literary crime" : "crime thriller"];
        if (wantsAdultHorror)
            return ["horror fiction", "psychological horror", "supernatural horror", "dark fantasy"];
        if (hasScienceFiction && hasHistorical)
            return ["speculative thriller", "dystopian drama", "historical fiction", "survival fiction"];
        if (hasScienceFiction)
            return ["speculative thriller", "dystopian drama", "survival fiction", "science fiction"];
        if (hasFantasy)
            return ["fantasy fiction", "epic fantasy", "dark fantasy", "historical fantasy"];
        if (hasRomance && hasHistorical)
            return ["historical romance", "romance novel", "romantic suspense", "historical fiction"];
        if (hasRomance)
            return ["contemporary romance", "romance novel", "romantic suspense", "women fiction"];
        if (hasHistorical && hasAdventure)
            return ["historical adventure", "historical fiction", "adventure fiction", "alternate history fiction"];
        if (hasHistorical)
            return ["historical fiction", "literary fiction", "historical mystery", "biographical fiction"];
        if (hasAdventure)
            return ["adventure fiction", "action adventure", "survival fiction", "thriller"];
        if (hasContemporaryDrama)
            return ["literary fiction", "contemporary fiction", "family drama fiction", "book club fiction"];
        return [
    ...
    const routingReason = (() => {
        if (wantsAdultFantasyWarCrime)
            return "adult_fantasy_war_crime";
        if (wantsAdultDystopianFantasySurvival)
            return "adult_dystopian_fantasy_survival";
        if (wantsAdultHistoricalCrimeDrama)
            return "adult_historical_crime_drama";
        if (wantsAdultGothicHorrorFantasy)
            return "adult_gothic_horror_fantasy";
        if (wantsAdultFantasyHistoricalSurvival)
            return "adult_fantasy_historical_survival";
        if (wantsAdultFantasyRomanceAdventure)
            return "adult_fantasy_romance_adventure";
        if (wantsAdultFantasyAdventureMystery)
            return "adult_fantasy_adventure_mystery";
        if (wantsAdultMixedSpeculative)
            return "adult_mixed_speculative";
        if (wantsAdultSciFi)
            return "adult_scifi";
        if (wantsAdultCozyFantasy)
            return "adult_cozy_fantasy";
        if (wantsAdultHistoricalSpeculativeThriller)
            return "adult_historical_speculative_thriller";
        if (wantsAdultCrimeThriller)
            return "adult_crime_thriller";
        if (wantsAdultHorror)
            return "adult_horror";
        if (hasScienceFiction)
            return "adult_mixed_speculative_fallback";
        if (hasFantasy)
            return "adult_fantasy";
        if (hasRomance)
            return "adult_romance";
        if (hasHistorical)
            return "adult_historical";
        if (hasAdventure)
            return "adult_adventure";
        if (hasContemporaryDrama)
            return "adult_contemporary_literary";
...
function buildMiddleGradesOpenLibraryQueryPlans(plan, profile, ageProfile) {
...
function buildKidsOpenLibraryQueryPlans(plan, profile, ageProfile) {
    ...
    const add = (query) => {
        ...
        if (isUsefulOpenLibraryQueryPart(clean))
            queries.push(clean);
    };
    const addFallback = (query) => {
        if (queries.length < 5)
            add(query);
    };
    const forceSemanticExpansion = Boolean(profile.diagnostics?.forceKidsCleanCandidateShortfallExpansion);
    if (forceSemanticExpansion) {
        if (/cozy|calm/.test(likedText) && /adventure|wonder|magic|explor/.test(likedText))
            add("cozy adventure picture books");
        if (/cozy|life sim|calm/.test(likedText) && !(/folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText)))
            add("cozy everyday picture books");
        if (/cozy|calm/.test(likedText) && /adventure|wonder|magic|explor/.test(likedText))
            add("gentle adventure picture books");
        if (/cozy|life sim/.test(likedText) && !(/folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText)))
            add("cozy animal village picture books");
        if (/reading|literacy|letters?|words?|problem[_ ]solving|problem solving|puzzles?|solve|super why/.test(likedText))
            add("reading problem solving picture books");
        if (/problem[_ ]solving|problem solving|puzzles?|solve/.test(likedText))
            add("problem solving early reader books");
        if (/reading|literacy|letters?|words?|stories/.test(likedText) && /adventure|problem[_ ]solving|problem solving|series/.test(likedText))
            add("reading adventure picture books");
        if (/animals?|monkeys?|dogs?|playful|silly/.test(likedText) && /mischief|humou?r|funny|silly|playful|community/.test(likedText))
            add("funny animal picture books");
        if (/monkeys?/.test(likedText) && /mischief|repetition|call and response/.test(likedText))
            add("mischievous monkey picture books");
        if (/teamwork|helping|courage|brave|heroic|heroes?|rescue|kindness|uplifting/.test(likedText))
            add("teamwork helping picture books");
        if (/helping|kindness|caring/.test(likedText) && /teamwork|courage|brave|heroic|heroes?|rescue|community/.test(likedText))
            add("helping teamwork picture books");
        if (/courage|brave|heroic|heroes?|rescue/.test(likedText))
            add("brave rescue picture books");
        if (/pooh|bear/.test(likedText))
            add("gentle bear picture books");
        if (/folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText))
            add("folklore animal picture books");
        if (/winter|snow|cozy|calm|nature|woodland|forest|puffin/.test(likedText) && /animals?/.test(likedText))
            add("woodland animal picture books");
        if (/calm|nature|puffin/.test(likedText))
            add("calm nature picture books");
        if (/animals?/.test(likedText) && !/humou?r|funny|silly|playful/.test(likedText))
            add("woodland animal picture books");
        if (/mischief|humou?r|funny|silly/.test(likedText))
            add("funny mischief picture books");
        if (/mischief|trouble|naughty|david/.test(likedText))
            add("mischief picture books");
        if (/big feelings?|feelings?|simple/.test(likedText))
            add("big feelings picture books");
        if (/simple|humou?r|funny|silly/.test(likedText))
            add("simple funny picture books");
        if (/imagination|creative|creativity|pretend|box|curious|curiosity/.test(likedText))
            add("imaginative picture books");
        if (/imagination|pretend|creative|box/.test(likedText))
            add("pretend play picture books");
        if (/learning|songs?|music|school|letters|numbers/.test(likedText))
            add("learning picture books");
        if (/songs?|music/.test(likedText))
            add("sing along picture books");
        if (/fantasy|magic|wonder|adventure/.test(likedText))
            add("imaginative adventure picture books");
        if (/science|curiosity|experiment|space|robots?|robotics|science_fiction|sci fi|animation/.test(likedText))
            add("science story picture books");
        if (/robots?|robotics|wall e|space/.test(likedText))
            add("robot picture books");
        if (!queries.length) {
            add("imaginative picture books");
            add("early reader stories");
        }
    }
    else if (/cozy|calm/.test(likedText) && /adventure|wonder|magic|explor/.test(likedText))
        add("cozy adventure picture books");
    if (!forceSemanticExpansion && /cozy|life sim|calm/.test(likedText) && !(/folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText)))
        add("cozy everyday picture books");
    if (!forceSemanticExpansion && /cozy|calm/.test(likedText) && /adventure|wonder|magic|explor/.test(likedText))
        add("gentle adventure picture books");
    if (!forceSemanticExpansion && /cozy|life sim/.test(likedText) && !(/folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText)))
        add("cozy animal village picture books");
    if (!forceSemanticExpansion && /reading|literacy|letters?|words?|problem[_ ]solving|problem solving|puzzles?|solve|super why/.test(likedText))
        add("reading problem solving picture books");
    if (!forceSemanticExpansion && /problem[_ ]solving|problem solving|puzzles?|solve/.test(likedText))
        add("problem solving early reader books");
    if (!forceSemanticExpansion && /reading|literacy|letters?|words?|stories/.test(likedText) && /adventure|problem[_ ]solving|problem solving|series/.test(likedText))
        add("reading adventure picture books");
    if (!forceSemanticExpansion && /animals?|monkeys?|dogs?|playful|silly/.test(likedText) && /mischief|humou?r|funny|silly|playful|community/.test(likedText))
        add("funny animal picture books");
    if (!forceSemanticExpansion && /monkeys?/.test(likedText) && /mischief|repetition|call and response/.test(likedText))
        add("mischievous monkey picture books");
    if (!forceSemanticExpansion && /teamwork|helping|courage|brave|heroic|heroes?|rescue|kindness|uplifting/.test(likedText))
        add("teamwork helping picture books");
    if (!forceSemanticExpansion && /helping|kindness|caring/.test(likedText) && /teamwork|courage|brave|heroic|heroes?|rescue|community/.test(likedText))
        add("helping teamwork picture books");
    if (!forceSemanticExpansion && /courage|brave|heroic|heroes?|rescue/.test(likedText))
        add("brave rescue picture books");
    if (!forceSemanticExpansion && /pooh|bear/.test(likedText))
        add("gentle bear picture books");
    if (!forceSemanticExpansion && /folklore|folk tale|folktale|classic|winter|mitten/.test(likedText) && /animals?/.test(likedText))
        add("folklore animal picture books");
    if (!forceSemanticExpansion && /winter|snow|cozy|calm|nature|woodland|forest|puffin/.test(likedText) && /animals?/.test(likedText))
        add("woodland animal picture books");
    if (!forceSemanticExpansion && /calm|nature|puffin/.test(likedText))
        add("calm nature picture books");
    if (!forceSemanticExpansion && /animals?/.test(likedText) && !/humou?r|funny|silly|playful/.test(likedText))
        add("woodland animal picture books");
    if (!forceSemanticExpansion && /imagination|creative|creativity|curious|curiosity|drawing|art/.test(likedText))
        add("imagination picture books");
    if (!forceSemanticExpansion && /learning|songs?|music|school|letters|numbers/.test(likedText))
        add("learning picture books");
    if (!forceSemanticExpansion && /fantasy|magic|imagination|wonder/.test(likedText))
        add("fantasy picture books");
    if (!forceSemanticExpansion && /cozy|gentle|calm/.test(likedText) && /adventure|wonder|magic/.test(likedText))
        add("cozy adventure picture books");
    if (!forceSemanticExpansion && /science|curiosity|experiment|space|robots?|robotics|science_fiction|sci fi|animation/.test(likedText))
        add("science fiction picture books");
    if (!forceSemanticExpansion && /science|curiosity|experiment|space|robots?|robotics|science_fiction|sci fi/.test(likedText))
        add("science easy reader");
    if (!forceSemanticExpansion && /robots?|robotics|wall e|space/.test(likedText))
        add("robot picture books");
    if (!forceSemanticExpansion && /feelings?|kindness|empathy|emotional|warm/.test(likedText))
        add("picture books feelings kindness");
    if (!forceSemanticExpansion && /friendship|friends?|growing up|growing_up|lessons?|school/.test(likedText))
        add("early reader friends");
    if (!forceSemanticExpansion && /friendship|friends?|belonging|kindness/.test(likedText))
        add("picture book friends kindness");
    if (!forceSemanticExpansion && /calm|gentle|cozy|bedtime|kindness|feelings?/.test(likedText))
        add("gentle picture books");
    if (!forceSemanticExpansion && /calm|gentle|bedtime|kindness|feelings?/.test(likedText))
        add("picture books calm friendship");
    if (!forceSemanticExpansion && /bear|bears|toy|toys/.test(likedText))
        add("bear friendship picture book");
    if (!forceSemanticExpansion && /humou?r|funny|comedy|playful|silly/.test(likedText))
        add("funny picture books");
    if (!forceSemanticExpansion && /science|curiosity|experiment|space|robots?|science_fiction/.test(likedText) && /humou?r|funny|comedy|playful|silly/.test(likedText))
        add("funny science picture books");
    if (!forceSemanticExpansion && /fairy tale|fairytale|clever|twist|unreliable narrator|pigs?/.test(likedText))
        add("fractured fairy tales picture books");
    if (!forceSemanticExpansion && /fairy tale|fairytale|clever|twist|unreliable narrator|pigs?/.test(likedText) && /humou?r|funny|comedy|playful|silly/.test(likedText))
        add("funny fairy tale picture book");
    if (!forceSemanticExpansion && /clever|twist|wonder|curiosity/.test(likedText))
        add("clever picture books");
    if (!forceSemanticExpansion && /adventure|wonder|fantasy|magic|animals?/.test(likedText) && !/mystery|scary|frightening/.test(avoidText))
        add("children picture book adventure");
    if (!forceSemanticExpansion && /growing up|growing_up|family|friendship|lessons?/.test(likedText))
        add("beginning reader growing up");
    if (!forceSemanticExpansion) {
...
function buildGenericOpenLibraryQueryPlans(plan, ageProfile) {
    const plannedIntents = plan.intents.length ? plan.intents : [{ query: ageProfile.diagnosticProbeQuery, facets: [], id: `${ageProfile.key}-open-library-fallback`, priority: 0, rationale: [] }];
...
function buildOpenLibraryQueryPlans(plan, profile, ageProfile) {
    if (ageProfile.key === "teen")
        return buildTeenOpenLibraryQueryPlans(plan, profile, ageProfile);
    if (ageProfile.key === "adult")
        return buildAdultOpenLibraryQueryPlans(plan, profile, ageProfile);
    if (ageProfile.key === "middleGrades")
        return buildMiddleGradesOpenLibraryQueryPlans(plan, profile, ageProfile);
    if (ageProfile.key === "k2")
        return buildKidsOpenLibraryQueryPlans(plan, profile, ageProfile);
    return buildGenericOpenLibraryQueryPlans(plan, ageProfile);
}
function buildOpenLibraryQueryPlansForRegression(plan, profile, ageProfile) {
    return buildOpenLibraryQueryPlans(plan, profile, ageProfile);
}
function configuredOpenLibraryProxyBase() {
...
function openLibraryRequest(query, limit, forceDirect = false) {
    ...
    if (!forceDirect && typeof window !== "undefined")
        return { url: `/api/openlibrary?${params}`, fetchPath: "proxy" };
    const proxyBase = configuredOpenLibraryProxyBase();
    if (!forceDirect && proxyBase)
        return { url: `${proxyBase}/api/openlibrary?${params}`, fetchPath: "proxy" };
    return { url: `https://openlibrary.org/search.json?${params}&language=eng`, fetchPath: "direct" };
}
function bodyPrefix(value) {
    const trimmed = value.trim();
    return trimmed ? trimmed.slice(0, RESPONSE_BODY_PREFIX_LIMIT) : undefined;
}
function normalizeOpenLibraryDoc(doc, queryPlan) {
...
function emptyDiagnostics(plan, status, startedAt, extra) {
...
function openLibraryProxyClientTimeoutMs(ageProfile) {
    if (ageProfile.key === "adult")
        return ADULT_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS;
    if (ageProfile.key === "teen")
        return TEEN_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS;
    if (ageProfile.key === "middleGrades")
        return MIDDLE_GRADES_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS;
    if (ageProfile.key === "k2")
        return K2_OPEN_LIBRARY_PROXY_CLIENT_TIMEOUT_MS;
    return undefined;
}
let openLibraryAbortControllerSequence = 0;
async function fetchOpenLibraryDocs(queryPlan, limit, signal, diagnosticOnly = false, timeoutMs = openLibraryProfiles_1.DEFAULT_OPEN_LIBRARY_PROFILE.perQueryTimeoutMs, attemptNumber = 1, proxyClientTimeoutMs, forceDirect = false, sourceBudgetRemainingAtFetchStartMs) {
    const query = queryPlan.query;
    ...
    const markControllerAborted = (reason, origin) => {
        abortReason = reason;
    ...
    try {
        ...
        try {
            ...
            if (Number.isFinite(Number(json?.proxyAttempts)))
                diagnostic.proxyAttempts = Number(json.proxyAttempts);
        ...
        if (diagnostic.timedOut && !diagnostic.abortOrigin)
            diagnostic.abortOrigin = queryController.signal.aborted ? "unknown" : "fetch_abort_without_local_signal";
...
function openLibraryEmptyReason(rawItems, rawApiResultCount, dropReasons, fetches, failedReason) {
    ...
    if (!mainFetch)
        return "openlibrary_no_main_fetch_diagnostic";
    if (mainFetch.timedOut)
        return "openlibrary_main_fetch_timed_out";
    if (mainFetch.responseShape === "missing_docs_array")
        return "openlibrary_unexpected_response_shape";
    if (Number(mainFetch.docsReturned || 0) === 0)
        return "openlibrary_returned_zero_docs";
    if (rawApiResultCount > 0 && droppedBeforeDocCount >= rawApiResultCount)
        return "openlibrary_docs_dropped_before_normalization";
    if (rawApiResultCount > 0 && droppedBeforeDocCount > 0)
        return "openlibrary_docs_partially_dropped_before_normalization";
    if (failedReason)
        return `openlibrary_failed_before_normalized_rows:${failedReason}`;
    return "openlibrary_no_normalized_rows_after_fetch";
}
function isEnglishOpenLibraryDoc(doc) {
    const languages = Array.isArray(doc?.language) ? doc.language.map((value) => String(value || "").toLowerCase()) : [];
    return languages.length === 0 || languages.includes("eng") || languages.includes("en");
}
function isRelevanceDriftOpenLibraryDoc(doc, query) {
    if (RELEVANCE_DRIFT_QUERY_HINT.test(query))
        return false;
    ...
    if (RELEVANCE_DRIFT_TITLE_HINT.test(`${title} ${subjects}`))
        return true;
...
function openLibraryDocText(doc) {
    return [
        String(doc?.title || ""),
        String(doc?.subtitle || ""),
...
function openLibraryAcademicDocText(doc) {
...
function isOpenLibraryArtifactDoc(doc, query) {
    if (ARTIFACT_QUERY_HINT.test(query))
        return false;
    return ARTIFACT_TITLE_HINT.test(openLibraryDocText(doc));
}
function isLiteraryAnalysisArtifactDoc(doc, query) {
    if (/\b(criticism|critical|analysis|study guide|literary study)\b/i.test(query))
        return false;
    ...
    if (LITERARY_ANALYSIS_TITLE_ARTIFACT_HINT.test(titleText))
        return true;
    if (LITERARY_ANALYSIS_SUBJECT_ARTIFACT_HINT.test(subjectText))
        return true;
    return LITERARY_ANALYSIS_ARTIFACT_HINT.test(fullText) && !hasFictionMetadataEvidence(doc);
}
function isKeywordStuffedMarketingArtifactDoc(doc) {
...
function isMediaStudyArtifactDoc(doc) {
    return MEDIA_STUDY_ARTIFACT_HINT.test(openLibraryDocText(doc));
}
function isTeenAcademicVideoGameCriticismArtifactDoc(doc, profile) {
...
function isTeenWritingGuideArtifactDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    return WRITING_GUIDE_CRITICISM_ARTIFACT_HINT.test(openLibraryDocText(doc));
}
function isTeenAcademicCriticismArtifactDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    ...
    if (isAboutYaLiterature && (hasAcademicCriticismShape || hasAcademicPublisher))
        return true;
    return hasAcademicPublisher && hasAcademicCriticismShape && !hasStrongTeenFictionMetadataEvidence(doc);
}
function isAdultProfileArtifactDoc(doc, profile) {
    ...
    if (SCHOLARLY_CATALOG_ARTIFACT_HINT.test(text))
        return true;
    if (ADULT_ROMANCE_DRIFT_HINT.test(text) && !explicitlyLikesRomance)
        return true;
    if (WRITING_GUIDE_CRITICISM_ARTIFACT_HINT.test(text) && !explicitlyLikesNonfiction)
        return true;
    if (ADULT_NOTES_CRITICISM_NONFICTION_ARTIFACT_HINT.test(text) && !explicitlyLikesNonfiction)
        return true;
    return false;
}
function isProgrammingGuideArtifactDoc(doc) {
    return PROGRAMMING_GUIDE_ARTIFACT_HINT.test(openLibraryDocText(doc));
}
function isSurvivalGuideArtifactDoc(doc) {
    const text = openLibraryDocText(doc);
    if (!SURVIVAL_GUIDE_ARTIFACT_HINT.test(text))
        return false;
    if (/\b(choose your own adventure|survival guide|survival handbook|survival manual|handbook|field guide|star trek survival|kane chronicles survival guide)\b/i.test(text))
        return true;
    return !hasStrongTeenFictionMetadataEvidence(doc);
}
function isAdultDarkRomanceArtifactDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    const text = openLibraryDocText(doc);
    return ADULT_DARK_ROMANCE_ARTIFACT_HINT.test(text) && !hasStrongTeenFictionMetadataEvidence(doc);
}
function hasFictionMetadataEvidence(doc) {
    const subjects = [
        ...(Array.isArray(doc?.subject) ? doc.subject : []),
        ...(Array.isArray(doc?.subject_facet) ? doc.subject_facet : []),
    ].join(" ").toLowerCase();
...
function hasStrongTeenFictionMetadataEvidence(doc) {
    const text = openLibraryDocText(doc).toLowerCase();
    const subjects = [
        ...(Array.isArray(doc?.subject) ? doc.subject : []),
        ...(Array.isArray(doc?.subject_facet) ? doc.subject_facet : []),
...
function hasExplicitTeenOpenLibraryEvidence(doc) {
    const text = openLibraryDocText(doc).toLowerCase();
...
function isTeenBroadScienceFictionClassicDriftDoc(doc, query, profile) {
    if (profile.ageBand !== "teens")
        return false;
    const normalizedQuery = cleanOpenLibraryQueryPart(query);
    if (!/^(science fiction adventure|space adventure|dystopian survival|teen adventure)$/.test(normalizedQuery))
        return false;
    if (hasSpecificTeenYoungAdultEvidence(doc))
        return false;
    const text = openLibraryAcademicDocText(doc).toLowerCase();
    const firstPublishYear = Number(doc?.first_publish_year || doc?.firstPublishYear || 0);
...
function isTeenElementaryDriftDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    if (hasSpecificTeenYoungAdultEvidence(doc))
        return false;
...
function isTeenCollectedClassicPackageDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    if (hasExplicitTeenOpenLibraryEvidence(doc))
        return false;
...
function isTeenClassicOrAdultDriftDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    if (hasExplicitTeenOpenLibraryEvidence(doc))
        return false;
...
function isWeakTeenFitOddTitleDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    const title = String(doc?.title || "").trim().toLowerCase();
    if (!/^(go to hell|hell|damned|damnation)\b/.test(title))
        return false;
    return !hasStrongTeenFictionMetadataEvidence(doc);
}
function isAuthorNameTitleDriftDoc(doc) {
    const title = String(doc?.title || "").trim().toLowerCase();
    if (!title)
        return false;
    const authors = Array.isArray(doc?.author_name) ? doc.author_name.map((name) => String(name || "").trim().toLowerCase()) : [];
    if (authors.includes(title))
        return true;
    if (/^[a-z]+\s+[a-z]+$/.test(title) && !hasFictionMetadataEvidence(doc))
        return true;
    return false;
}
function isLiteralTitleMatchArtifactDoc(doc, query) {
    const title = String(doc?.title || "").toLowerCase();
    if (/\b(fantasy drama book|playing with fantasy)\b/.test(title))
        return true;
...
function openLibrarySeriesKey(doc) {
    const title = String(doc?.title || "").toLowerCase();
...
function teenOpenLibraryHandoffFranchiseKey(item) {
    ...
    if (/\b(?:99\s+)?fear\s+street\b/.test(text))
        return "fear_street";
    if (/\bteen\s+titans\b/.test(text))
        return "teen_titans";
    return openLibrarySeriesKey(doc);
}
function teenOpenLibraryHandoffQueryFamilyKey(item) {
    const query = cleanOpenLibraryQueryPart(String(item?.queryText || item?.simplifiedOpenLibraryQuery || ""));
    if (/\b(teen adventure|action adventure|survival fiction|dystopian survival|fantasy survival)\b/.test(query))
        return "adventure";
    if (/\b(young adult dystopian|dystopian|science fiction|sci-fi|speculative|space)\b/.test(query))
        return "speculative";
    if (/\b(young adult horror|survival horror|horror thriller)\b/.test(query))
        return "horror";
    if (/\b(coming of age|young adult contemporary|teen realistic|contemporary fantasy)\b/.test(query))
        return "contemporary";
    if (/\b(young adult romance|historical romance|romance fantasy)\b/.test(query))
        return "romance";
    if (/\bsports?\b/.test(query))
        return "sports";
    if (/\b(fantasy|magic|magical|paranormal|supernatural)\b/.test(query))
        return "fantasy";
    const queryFamily = String(item?.queryFamily || "").trim();
    if (queryFamily && queryFamily !== "open_library_broad")
        return queryFamily;
    return query || queryFamily;
}
function teenOpenLibraryHasComparableLikedFamilies(queryPlans) {
    const count = Number(queryPlans[0]?.routingDominance?.comparableLikedFamilyCount || 0);
    return Number.isFinite(count) && count >= 2;
}
function diverseTeenOpenLibraryHandoff(items, limit, balanceQueryFamilies = false) {
    const selected = [];
    const deferred = [];
    ...
    if (balanceQueryFamilies && selected.length >= Math.min(limit, 2)) {
        ...
        if (selectedFamilies.size < 2) {
            for (const candidate of items) {
                ...
                if (!candidateFamily || selectedFamilies.has(candidateFamily))
                    continue;
                const candidateFranchise = teenOpenLibraryHandoffFranchiseKey(candidate);
                for (let index = selected.length - 1; index >= 0; index -= 1) {
                    const remainingFranchises = new Set();
                    for (let selectedIndex = 0; selectedIndex < selected.length; selectedIndex += 1) {
                        ...
                        if (franchiseKey)
                            remainingFranchises.add(franchiseKey);
                    }
                    if (candidateFranchise && remainingFranchises.has(candidateFranchise))
                        continue;
...
function openLibraryCollectionRootKey(doc) {
    const title = String(doc?.title || "").toLowerCase();
    if (!title)
        return "";
...
function isTeenInappropriateOpenLibraryDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    const text = openLibraryDocText(doc).toLowerCase();
    if (/\b(lolita|nabokov|erotic|erotica|sexual abuse|incest|pornography)\b/.test(text))
        return true;
    if (/\bnovels?\s+\d{4}\s*-\s*\d{4}\b/.test(text) && /\b(lolita|nabokov)\b/.test(text))
        return true;
    if (ADULT_LOW_TEEN_FIT_HINT.test(text) && !/\b(young adult|juvenile|teen|adolescent)\b/.test(text))
        return true;
    return false;
}
function isOmnibusBundleDriftOpenLibraryDoc(doc, query, profile) {
    if (profile.ageBand !== "teens")
        return false;
    if (/\bomnibus|collected|complete|screenplay|selected works|collection\b/i.test(query))
        return false;
    const title = String(doc?.title || "");
    return /\b(omnibus|collected novels|complete novels|novels?\s+\d{4}\s*-\s*\d{4}|screenplay)\b/i.test(title);
}
function emergencyFallbackHasMeaningfulOverlap(doc, queryPlans) {
...
function hasAdultFantasyHistoricalFallbackOverlap(doc) {
    const text = openLibraryDocText(doc).toLowerCase();
    return /\b(fantasy|magic|magical|historical|history|romance|romantic|adventure|action|comedy|humou?r)\b/.test(text);
}
function isAdultMixedHistoricalMysteryRomanceProfile(profile) {
    if (profile.ageBand !== "adult")
        return false;
    const profileText = [
        ...profile.genreFamily.map((row) => row.value),
...
function hasAdultMixedHistoricalMysteryFallbackOverlap(doc) {
    const text = openLibraryDocText(doc).toLowerCase();
    return /\b(mystery|crime|thriller|suspense|detective|historical|history|romance|romantic)\b/.test(text);
}
function adultUnderfillRecoveryQueries(queryPlans) {
    const routingReason = String(queryPlans[0]?.routingReason || "");
    const plannedQueries = queryPlans.map((plan) => plan.query);
    const routeFallbacks = (() => {
        if (routingReason === "adult_fantasy_adventure_mystery")
            return ["fantasy mystery", "speculative fiction", "adventure fantasy"];
        if (routingReason === "adult_fantasy_romance_adventure")
            return ["romantic fantasy", "mythological fantasy", "adventure fantasy"];
        if (routingReason === "adult_horror" || routingReason === "adult_gothic_horror_fantasy")
            return ["horror fiction", "supernatural thriller", "dark suspense"];
        if (routingReason === "adult_mixed_speculative")
            return ["speculative fiction", "science fiction thriller", "speculative thriller"];
        if (routingReason === "adult_historical_crime_drama")
            return ["crime fiction", "historical mystery", "mystery thriller"];
        if (routingReason === "adult_crime_thriller")
            return ["crime fiction", "mystery thriller", "psychological thriller"];
        if (routingReason === "adult_fantasy_historical_survival")
            return ["fantasy adventure", "historical fantasy", "fantasy romance"];
        return [];
    })();
    return uniqueStrings([...plannedQueries, ...routeFallbacks], 8);
}
function teenUnderfillRecoveryQueries(queryPlans) {
    const routingReason = String(queryPlans[0]?.routingReason || "");
    const plannedQueries = queryPlans.map((plan) => plan.query);
    const routeFallbacks = (() => {
        if (/dystopian|scifi|science/i.test(routingReason))
            return ["young adult dystopian fiction", "dystopian adventure", "science fiction adventure"];
        if (/mystery|heist|psychological|thriller/i.test(routingReason))
            return ["young adult mystery", "teen mystery", "mystery adventure"];
        if (/historical/i.test(routingReason))
            return ["teen historical fiction", "historical adventure", "young adult mystery"];
        if (/horror|paranormal/i.test(routingReason))
            return ["young adult horror", "paranormal mystery", "dark fantasy"];
        if (/contemporary|romance/i.test(routingReason))
            return ["young adult contemporary", "teen realistic fiction", "coming of age novel"];
        if (/fantasy|adventure|survival/i.test(routingReason))
            return ["young adult fantasy", "fantasy adventure", "magical adventure"];
...
function middleGradesProfileSpecificQueries(profile) {
    ...
    const hasDistinctivePositiveSignal = (patterns) => positiveRows.some((row) => patterns.some((pattern) => pattern.test(row.value)));
    const candidates = [
...
function middleGradesTargetedQueryPlan(profile) {
    const positiveRows = [...profile.genreFamily, ...profile.themes]
        .filter((row) => Number(row.weight || 0) > 0)
    ...
    const scoreByFamily = {};
    const likedEvidenceByFamily = {};
    const skipEvidenceByFamily = {};
    const avoidEvidenceByFamily = {};
    const evidenceTitles = (row) => row.evidence.length ? row.evidence : [row.value];
    const hasLikedPositiveRow = (pattern) => positiveRows.some((row) => {
    ...
    for (const def of familyDefinitions) {
        ...
        for (const row of positiveRows) {
            if (!def.patterns.some((pattern) => pattern.test(row.value)))
                continue;
...
function middleGradesTargetedQueryBatch(profile) {
    return middleGradesTargetedQueryPlan(profile).queries;
}
function middleGradesRecoveryQueries(queryPlans) {
    const routingReason = String(queryPlans[0]?.routingReason || "");
    const plannedQueries = queryPlans.map((plan) => plan.query);
    ...
    const routeFallbacks = (() => {
        if (/scifi|science|dystopian/i.test(routingReason))
            return ["middle grade adventure", "middle grade science fiction", "children's fantasy adventure", "middle grade fantasy", ...ageAnchoredUnderfillQueries];
        if (/fantasy_mystery|mystery/i.test(routingReason))
            return ["middle grade mystery", "school mystery", "mystery adventure", "middle grade fantasy mystery"];
        if (/fantasy/i.test(routingReason))
            return ["middle grade fantasy adventure", "middle grade fantasy", "children's fantasy adventure", "middle grade adventure", "middle grade fiction", "children's school stories", "middle grade humor"];
        return [...ageAnchoredUnderfillQueries, "children's funny books"];
    })();
    return uniqueStrings([...plannedQueries.slice(1), ...routeFallbacks], 12);
}
function middleGradesSignalShapedAntiZeroFallback(queryPlans, attemptedQueries, profile) {
    const routingReason = String(queryPlans[0]?.routingReason || "");
    if (/fantasy_mystery|mystery/i.test(routingReason))
        return undefined;
    const positiveRows = [...profile.genreFamily, ...profile.themes]
    ...
    const queryScores = {};
    const positiveEvidenceByQuery = {};
    const avoidEvidenceByQuery = {};
    const reliabilityByQuery = {};
...
function middleGradesRouteAlignedRecoveryQuery(queryPlans, attemptedQueries = new Set()) {
    const routeText = queryPlans
        ...
        .join(" ");
    const firstUnattempted = (queries) => uniqueStrings(queries, queries.length)
...
function middleGradesZeroCandidateFallbackQuery(queryPlans, attemptedQueries = new Set()) {
    const routingReason = String(queryPlans[0]?.routingReason || "");
    const firstUnattempted = (queries) => uniqueStrings(queries, queries.length)
        .find((query) => !attemptedQueries.has(query.toLowerCase()));
    if (/humor|funny/i.test(routingReason)) {
    ...
    if (/fantasy_mystery|mystery/i.test(routingReason))
        return firstUnattempted(["middle grade mystery", "school mystery", "mystery adventure", "middle grade fantasy mystery"]) || "middle grade mystery";
    if (/contemporary|school|friendship|realistic/i.test(routingReason))
        return firstUnattempted(["middle grade realistic fiction", "middle grade school story", "middle grade friendship", "middle grade family story", "middle grade friendship books", "middle grade adventure", "children's funny books"]) || "middle grade realistic fiction";
    if (/fantasy/i.test(routingReason))
        return firstUnattempted(["middle grade fantasy adventure", "middle grade fantasy", "children's fantasy adventure", "middle grade adventure"]) || "middle grade fantasy adventure";
    if (/scifi|science|dystopian|historical|adventure/i.test(routingReason))
        return firstUnattempted(["middle grade adventure", "middle grade fantasy adventure", "middle grade school story"]) || "middle grade adventure";
...
function middleGradesQueryOnlyContinuationQueries(queryPlans, attemptedQueries = new Set()) {
    const routeText = queryPlans
        .map((plan) => [plan.routingReason, plan.query, plan.originalPlannedQuery, ...(plan.facets || [])].filter(Boolean).join(" "))
        .join(" ");
...
function middleGradesEvidenceAwareRecoveryQueries(queryPlans, profile, attemptedQueries = new Set()) {
    const routeText = queryPlans
        .map((plan) => [plan.routingReason, plan.query, plan.originalPlannedQuery, ...(plan.facets || [])].filter(Boolean).join(" "))
        .join(" ");
...
function middleGradesMeaningfulTasteRecoveryQueryPlans(profile, attemptedQueries = new Set(), sameRunLeakageByFamily = {}) {
    ...
    const hasLiked = (pattern) => pattern.test(likedText);
    const hasAvoid = (pattern) => pattern.test(avoidText);
    const cleanShortfallExpansion = Boolean(profile.diagnostics?.forceMiddleGradesCleanCandidateShortfallExpansion);
    const familyDefs = cleanShortfallExpansion ? [
    ...
    const skippedByAvoid = {};
    const skippedByLeakage = {};
    const selectedReasons = {};
    const scores = familyDefs.map((def) => {
        ...
        let skippedReason;
        if (attemptedQueries.has(def.query.toLowerCase()))
            skippedReason = "already_attempted";
        if (!skippedReason && comedyLeakageActive && /comedy|funny|humou?r|playful/i.test(def.query))
            skippedReason = "same_run_comedy_leakage";
        if (!skippedReason && sameRunLeakageByFamily[def.family] >= 2)
            skippedReason = "same_run_family_leakage_or_query_only";
        if (!skippedReason && def.broad && likedSupport < 2)
            skippedReason = "broad_recovery_requires_two_liked_anchors";
        if (!skippedReason && def.family === "friendship_adventure" && hasAvoid(/\b(friendship|friends?)\b/i) && likedSupport < 2)
            skippedReason = "friendship_avoid_without_second_positive_anchor";
        if (!skippedReason && /fantasy|adventure/.test(def.family) && hasAvoid(/\b(fantasy|magic|magical|adventure|quest)\b/i) && likedSupport < 2)
            skippedReason = "fantasy_or_adventure_avoid_requires_second_positive_anchor";
        if (!skippedReason && cleanShortfallExpansion && likedSupport === 0)
            skippedReason = "no_liked_evidence_support_for_clean_expansion";
        if (!skippedReason && likedSupport === 0)
            score -= 6;
        return { query: def.query, family: def.family, anchors: def.anchors, score, reason: reasons.join(";"), skippedReason };
    });
    for (const row of scores) {
        if (row.skippedReason?.includes("avoid"))
            skippedByAvoid[row.family] = row.skippedReason;
        else if (row.skippedReason?.includes("leakage") || row.skippedReason?.includes("query_only"))
            skippedByLeakage[row.family] = row.skippedReason;
    }
    ...
    for (const [index, row] of selected.entries())
        selectedReasons[row.family] = `rank=${index + 1};${row.reason};score=${row.score}`;
    return { queries: selected.map((row) => row.query), scores, skippedByAvoid, skippedByLeakage, selectedReasons };
}
function middleGradesMeaningfulTasteRecoveryQueries(profile, attemptedQueries = new Set()) {
    return middleGradesMeaningfulTasteRecoveryQueryPlans(profile, attemptedQueries).queries;
}
function middleGradesRecoveryQueryAnchor(query) {
    const text = query.toLowerCase();
    if (/\bsuperhero\b/.test(text))
        return "superhero";
    if (/\bocean|sea|island\b/.test(text))
        return "ocean";
    if (/\bsurvival|wilderness\b/.test(text))
        return "survival";
    if (/\brobot|technology\b/.test(text))
        return "robot";
    if (/\bfantasy|magic|mythology\b/.test(text))
        return "fantasy";
    if (/\bfamily\b/.test(text))
        return "family";
    if (/\bschool\b/.test(text))
        return "school";
    if (/\bfriendship|friends?\b/.test(text))
        return "friendship";
    if (/\badventure|fast paced\b/.test(text))
        return "adventure";
    if (/\bcomedy|funny|humou?r\b/.test(text))
        return "humor";
    return "general_fiction";
}
function middleGradesDocumentBackedTasteSignalsFromSourceDoc(doc, profile) {
    const rawDescription = typeof doc?.description === "string" ? doc.description : typeof doc?.description?.value === "string" ? doc.description.value : "";
    const documentText = [
        String(doc?.title || ""),
        String(doc?.subtitle || ""),
...
function middleGradesSourceMeaningfulTasteEligibility(doc, profile) {
    const signals = middleGradesDocumentBackedTasteSignalsFromSourceDoc(doc, profile);
    if (!signals.length)
        return { allowed: false, reason: "zero_doc_backed_taste_match", signals };
    if (signals.every((signal) => signal === "adventure"))
        return { allowed: false, reason: "broad_adventure_only_taste_match", signals };
    return { allowed: true, signals };
}
function middleGradesSourceRouteEvidencePattern(query, routingReason = "") {
    ...
    if (/\b(dragon|mythology|mythological|creature)\b/.test(routeText))
        return /\b(dragon|dragons|myth|myths|mythology|mythological|creature|creatures|magic|magical|fantasy|quest|adventure)\b/i;
    if (/\b(science adventure|science fiction|sci fi|sci-fi|space|dystopian|dystopia)\b/.test(routeText))
        return /\b(science|scientist|experiment|space|planet|galaxy|robot|robots?|technology|invention|dystopian|dystopia|sci fi|sci-fi|science fiction|nonfiction|animals?|nature|wildlife|wolf|wolves)\b/i;
    if (/\b(robot|ai|artificial intelligence|superhero|superheroes)\b/.test(routeText))
        return /\b(robot|robots?|ai|artificial intelligence|technology|invention|superhero|superheroes|powers?)\b/i;
    if (/\b(animal adventure|animals?|nature|wildlife)\b/.test(routeText))
        return /\b(animal|animals|dog|cat|horse|wolf|wolves|wildlife|nature|forest|woods|survival|cozy|community|farm|creature|creatures)\b/i;
    if (/\b(school adventure|school story|school|classroom|children s school stories)\b/.test(routeText))
        return /\b(school|class|classroom|teacher|student|students|friendship|friends?|community|family|comedy|funny|humor|humour)\b/i;
    if (/\b(superhero|super hero)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure)\b/.test(sourceQueryText))
        return /\b(superhero|super hero|heroes|hero|powers?|friendship|friends?|team|adventure|quest)\b/i;
    if (/\bfamily\b/.test(sourceQueryText) && /\badventure\b/.test(sourceQueryText))
        return /\b(family|families|parents?|siblings?|home|superhero|super hero|heroes|hero|powers?|adventure|quest|journey|wild)\b/i;
    if (/\b(ocean|sea|island)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure|fantasy)\b/.test(sourceQueryText))
        return /\b(ocean|sea|island|marine|friendship|friends?|adventure|quest|fantasy|magic|magical)\b/i;
    if (/\b(science|robot|technology)\b/.test(sourceQueryText) && /\b(adventure|friendship|fiction)\b/.test(sourceQueryText))
        return /\b(science|scientist|experiment|robot|robots?|technology|invention|friendship|friends?|adventure|quest)\b/i;
    if (/\b(fantasy|magic|magical|mythology)\b/.test(sourceQueryText) && /\b(friendship|friends?|adventure|family)\b/.test(sourceQueryText))
        return /\b(fantasy|magic|magical|wizard|witch|fairy|fairies|dragon|quest|kingdom|hero|heroic|myth|myths|mythology|adventure|friendship|friends?|family|school)\b/i;
    if (/\b(friendship|community)\b/.test(routeText))
        return /\b(friendship|friends?|community|school|family|team|classroom)\b/i;
    if (/\b(fantasy mystery|mystery adventure|mystery|detective)\b/.test(routeText))
        return /\b(mystery|detective|clue|clues|case|secret|secrets|puzzle|investigate|investigation)\b/i;
    if (/\b(humor|funny|funny family|fantasy humor)\b/.test(routeText))
        return /\b(humor|humour|funny|comedy|comic|joke|laugh|laughs|giggle|silly|school|friendship|friends?|family|quest|adventure|trail)\b/i;
    if (/\b(fantasy adventure|family fantasy|fantasy|magic|magical)\b/.test(routeText))
        return /\b(fantasy|magic|magical|wizard|witch|dragon|quest|kingdom|hero|heroic|adventure)\b/i;
    if (/\b(contemporary|realistic)\b/.test(routeText))
        return /\b(realistic|contemporary|school|classroom|friendship|friends?|family|community)\b/i;
    return undefined;
}
function middleGradesSourceDocumentRouteEvidenceFields(doc, query, routingReason = "") {
    ...
    const normalizeRouteEvidenceText = (value) => String(value || "").toLowerCase().replace(/[^a-z0-9\s-]/g, " ").replace(/\s+/g, " ").trim();
    ...
    if (humorRoute && titleOnly && humorOnlyTitle && !hasPreteenFictionEvidence)
        return [];
    return matched;
}
function middleGradesSourceMediumStrongEvidence(doc, query, routingReason = "") {
    const fields = middleGradesSourceDocumentRouteEvidenceFields(doc, query, routingReason);
    if (fields.some((field) => ["subjects", "description"].includes(field)))
        return true;
...
function middleGradesUnderfillSafeRecoveryQueries(queryPlans, attemptedQueries = new Set()) {
    const routeText = queryPlans
        .map((plan) => [plan.routingReason, plan.query, plan.originalPlannedQuery, ...(plan.facets || [])].filter(Boolean).join(" "))
        .join(" ");
...
function middleGradesRejectedRowsAntiZeroFallbackQuery(queryPlans, attemptedQueries, fetches) {
    const hadReturnedRejectedRows = fetches.some((fetch) => !fetch.diagnosticOnly && !fetch.timedOut && Number(fetch.docsReturned || 0) > 0);
    if (!hadReturnedRejectedRows)
        return undefined;
    const routingReason = String(queryPlans[0]?.routingReason || "");
    const firstUnattempted = (queries) => uniqueStrings(queries, queries.length)
        .find((query) => !attemptedQueries.has(query.toLowerCase()));
    if (/fantasy_mystery|mystery/i.test(routingReason))
        return firstUnattempted(["middle grade adventure"]);
    if (/humor|funny/i.test(routingReason)) {
        const primaryStable = /fantasy/i.test(routingReason) ? ["middle grade fantasy adventure", "middle grade adventure"] : ["middle grade adventure"];
...
function isMiddleGradesFantasyHumorRoute(queryPlans) {
    return /middle_grades_fantasy_humor/i.test(String(queryPlans[0]?.routingReason || ""));
}
function isMiddleGradesHumorDefaultQuery(query) {
    return /\b(humor|funny)\b/i.test(query);
}
function hasMiddleGradesFantasyHumorAlignedQuery(rawItems) {
    return rawItems.some((item) => /\b(adventure|fantasy adventure|friendship)\b/i.test(String(item?.queryText || "")));
}
function isTeenCompatibleOpenLibraryDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return true;
    const firstPublishYear = Number(doc?.first_publish_year || 0);
    if (!firstPublishYear || firstPublishYear >= 1950)
        return true;
    const subjects = [
        ...(Array.isArray(doc?.subject) ? doc.subject : []),
        ...(Array.isArray(doc?.subject_facet) ? doc.subject_facet : []),
...
function middleGradesAgeShapeDiagnostic(doc, query, profile) {
    ...
    const keep = !hasAdultLeakageShape && (hasExplicitMiddleGradesEvidence
...
function hasMiddleGradesAgeShapeEvidence(doc, query, profile) {
    const diagnostic = middleGradesAgeShapeDiagnostic(doc, query, profile);
    if (!diagnostic)
        return true;
    return diagnostic.keep;
}
function isTooYoungTeenOpenLibraryDoc(doc, profile) {
    if (profile.ageBand !== "teens")
        return false;
    if (hasStrongTeenFictionMetadataEvidence(doc))
        return false;
    const text = openLibraryDocText(doc).toLowerCase();
...
function hasKidsAgeShapeEvidence(doc, query, profile) {
    ...
    if (!hasKidsEvidence && (genericNoAgeTitle || oldGenericAdventureTitle))
        return false;
    if (hasKidsEvidence && !/\b(nineteen eighty-four|animal farm|adult)\b/.test(text))
        return true;
    if (queryIsKidsAnchored && hasKidFriendlyShape && !adultOrReferenceShape)
        return true;
    return false;
}
function shouldKeepOpenLibraryDoc(doc, query, profile) {
    if (!isEnglishOpenLibraryDoc(doc))
        return { keep: false, reason: "non_english" };
    if (!Array.isArray(doc?.author_name) || doc.author_name.length === 0)
        return { keep: false, reason: "missing_author" };
    if (isRelevanceDriftOpenLibraryDoc(doc, query))
        return { keep: false, reason: "relevance_drift" };
    if (isOpenLibraryArtifactDoc(doc, query))
        return { keep: false, reason: "artifact_title" };
    if (isLiteraryAnalysisArtifactDoc(doc, query))
        return { keep: false, reason: "literary_analysis_artifact" };
    if (isKeywordStuffedMarketingArtifactDoc(doc))
        return { keep: false, reason: "keyword_stuffed_marketing_artifact" };
    if (isMediaStudyArtifactDoc(doc))
        return { keep: false, reason: "media_study_artifact" };
    if (isTeenAcademicVideoGameCriticismArtifactDoc(doc, profile))
        return { keep: false, reason: "media_study_artifact" };
    if (isTeenAcademicCriticismArtifactDoc(doc, profile))
        return { keep: false, reason: "teen_academic_criticism_artifact" };
    if (isTeenWritingGuideArtifactDoc(doc, profile))
        return { keep: false, reason: "teen_writing_guide_artifact" };
    if (isAdultProfileArtifactDoc(doc, profile))
        return { keep: false, reason: "adult_profile_artifact" };
    if (isProgrammingGuideArtifactDoc(doc))
        return { keep: false, reason: "programming_guide_artifact" };
    if (isSurvivalGuideArtifactDoc(doc))
        return { keep: false, reason: "survival_guide_artifact" };
    if (isAdultDarkRomanceArtifactDoc(doc, profile))
        return { keep: false, reason: "adult_dark_romance_artifact" };
    if (isLiteralTitleMatchArtifactDoc(doc, query))
        return { keep: false, reason: "literal_title_match_artifact" };
    if (isAuthorNameTitleDriftDoc(doc))
        return { keep: false, reason: "author_name_title_drift" };
    if (isWeakTeenFitOddTitleDoc(doc, profile))
        return { keep: false, reason: "weak_odd_title_teen_fit" };
    if (isTeenInappropriateOpenLibraryDoc(doc, profile))
        return { keep: false, reason: "teen_inappropriate_content" };
    if (isTeenElementaryDriftDoc(doc, profile))
        return { keep: false, reason: "too_young_for_teen_artifact" };
    if (isTooYoungTeenOpenLibraryDoc(doc, profile))
        return { keep: false, reason: "too_young_for_teen_artifact" };
    if (isTeenBroadScienceFictionClassicDriftDoc(doc, query, profile))
        return { keep: false, reason: "teen_broad_scifi_classic_drift" };
    if (isTeenBroadQueryClassicDriftDoc(doc, query, profile))
        return { keep: false, reason: "teen_broad_query_classic_drift" };
    if (isTeenClassicOrAdultDriftDoc(doc, profile))
        return { keep: false, reason: "teen_classic_or_adult_drift" };
    if (isTeenCollectedClassicPackageDoc(doc, profile))
        return { keep: false, reason: "teen_collected_classic_package" };
    if (isOmnibusBundleDriftOpenLibraryDoc(doc, query, profile))
        return { keep: false, reason: "adult_literary_content" };
    if (!isTeenCompatibleOpenLibraryDoc(doc, profile))
        return { keep: false, reason: "not_teen_compatible_publication_year" };
    if (!hasMiddleGradesAgeShapeEvidence(doc, query, profile))
        return { keep: false, reason: "middle_grades_age_shape_mismatch" };
    if (!hasKidsAgeShapeEvidence(doc, query, profile))
        return { keep: false, reason: "k2_age_shape_mismatch" };
    return { keep: true };
}
exports.openLibrarySourceAdapter = {
    source: "openLibrary",
    async search(plan, context) {
        ...
        const ageProfile = (0, openLibraryProfiles_1.openLibraryProfileForAgeBand)(context.profile.ageBand);
        const artifactReasonLabels = (0, openLibraryProfiles_1.openLibraryArtifactReasonLabels)(ageProfile);
        const middleGradesDeepDebugActivationSourceRaw = String(context.profile.diagnostics?.middleGradesDeepDebugActivationSource || "");
        const debugMiddleGradesDeepTrace = ageProfile.key === "middleGrades" && Boolean(context.profile.diagnostics?.debugMiddleGradesNoTimeouts
        ...
        const sourceBudgetMs = ageProfile.key === "middleGrades"
            ...
            : plan.timeoutMs;
        const middleGradesActualNetworkTimeoutMs = (remainingBudgetMs) => {
        ...
        const forceMiddleGradesCleanCandidateShortfallExpansion = ageProfile.key === "middleGrades" && Boolean(context.profile.diagnostics?.forceMiddleGradesCleanCandidateShortfallExpansion);
        const forceKidsCleanCandidateShortfallExpansion = ageProfile.key === "k2" && Boolean(context.profile.diagnostics?.forceKidsCleanCandidateShortfallExpansion);
        ...
        const cleanExpansionQueryPlan = forceMiddleGradesCleanCandidateShortfallExpansion
            ? middleGradesMeaningfulTasteRecoveryQueryPlans(context.profile, new Set(), {})
            : undefined;
        ...
        const queryPlans = forceMiddleGradesCleanCandidateShortfallExpansion
            ? cleanExpansionQueries.map((query, index) => ({
                query,
        ...
        const middleGradesLikedEvidenceFirstBatchFamilies = ageProfile.key === "middleGrades"
            ...
            : [];
        const middleGradesDebugTrace = debugMiddleGradesDeepTrace ? {
        ...
        const rawItems = [];
        const middleGradesExpandedScoringPool = [];
        const middleGradesExpandedScoringKeys = new Set();
        const rawTitles = [];
        const dropReasons = {};
        const expansionDroppedBeforeScoringByReason = {};
        const expansionDroppedBeforeScoringTitles = {};
        const fetches = [];
        const acceptedSeriesKeys = new Set();
        const acceptedCollectionRootKeys = new Set();
        const acceptedDocKeys = new Set();
        const artifactSuppressedTitles = [];
        const seriesSuppressedTitles = [];
        const sameRootCollectionCollapsedTitles = [];
        const duplicateRootBlockedReturnedTitle = [];
        ...
        let middleGradesDelayedRetryTimeoutMs;
        let middleGradesTimeoutBudgetRemainingBeforeRetry;
        let middleGradesAntiZeroFallbackShapedQuery;
        let middleGradesAntiZeroFallbackShapingSignals;
        let middleGradesFallbackCandidateQueries;
        let middleGradesFallbackQueryScores;
        let middleGradesFallbackQueryReliability;
        let middleGradesPositiveEvidenceByFallbackQuery;
        let middleGradesAvoidEvidenceByFallbackQuery;
        let middleGradesSelectedFallbackQueryReason;
        let middleGradesWhyHigherTasteFallbackLost;
        const middleGradesFallbackAttemptOrder = [];
        const middleGradesRemainingBudgetBeforeEachFallback = {};
        const middleGradesFallbackOutcomes = [];
        let middleGradesFallbackSlateSpecificityScore;
        let middleGradesStrongerSignalDroppedFromFallbackQuery;
        let middleGradesWhyFallbackOnlyAcceptedAsFinal;
        let middleGradesRouteAlignedRecoveryAttemptedAfterFallback = false;
        let middleGradesRouteAlignedRecoverySkippedReason;
        let middleGradesUnderfillSafeRecoveryAttempted = false;
        const middleGradesUnderfillSafeRecoveryQueriesAttempted = [];
        let middleGradesUnderfillSafeRecoveryAcceptedCount = 0;
        let middleGradesUnderfillSafeRecoverySkippedReason;
        ...
        const middleGradesProfileSpecificQueriesAttempted = [];
        let middleGradesProfileSpecificQueriesTimedOut = 0;
        let middleGradesProfileSpecificQueriesAcceptedCount = 0;
        let middleGradesFallbackStartedOnlyAfterProfileQueriesExhausted;
        let middleGradesLockQualityRetryAttempted = false;
        const middleGradesLockQualityRetryQueries = [];
        let middleGradesLockQualityRetryAcceptedCount = 0;
        let middleGradesFinalReturnedDespiteLockQualityFailReason;
        let middleGradesRejectedAllRowsAsQueryOnly = false;
        let middleGradesQueryOnlyRejectionTriggeredContinuation = false;
        let middleGradesQueryOnlyFirstRejectionFetchCount;
        const middleGradesContinuedAfterQueryOnlyRejectionQueries = [];
        const middleGradesUnattemptedSpecificQueriesAfterQueryOnlyRejection = [];
        let middleGradesContinuedAfterQueryOnlyRejectionAcceptedCount = 0;
        let middleGradesRecoveryExhaustionReasonDetailed;
        const middleGradesEvidenceAwareRecoveryQueriesAttempted = [];
        ...
        const middleGradesMediumStrongEvidenceQueriesAttempted = [];
        const middleGradesMediumStrongEvidenceAcceptedTitles = [];
        let middleGradesWeakEvidenceFinalizedBecause;
        ...
        const middleGradesMeaningfulTasteRecoveryQueriesAttempted = [];
        const middleGradesMeaningfulTasteRecoveryAcceptedTitles = [];
        const middleGradesMeaningfulTasteRecoveryRejectedTitlesByReason = {};
        const middleGradesRecoveryQueryAnchorByQuery = {};
        let middleGradesRecoveryHumorUsedAsAnchorBlocked = false;
        let middleGradesRecoveryConcreteFictionQueryUsed = false;
        const middleGradesRecoveryQueryFamilyRejectedForLeakageCount = {};
        let middleGradesRecoveryFamilyScores = [];
        let middleGradesRecoveryFamiliesSkippedByAvoidEvidence = {};
        let middleGradesRecoveryFamiliesSkippedBySameRunLeakage = {};
        let middleGradesRecoveryFamilyExecutionOrderReason = {};
        const middleGradesRecoveryFamilyYieldByFamily = {};
        let middleGradesRecoveryEarlyFinalGateApplied = false;
        const middleGradesRecoveryEarlyFinalGateRejectedByReason = {};
        const middleGradesRecoveryAcceptedLikelyFinalSurvivorTitles = [];
        const middleGradesRecoveryAcceptedButPredictedDropTitles = [];
        ...
        const middleGradesPerQueryBudgetReserved = {};
        let middleGradesSkippedRemainingQueriesDueToBudgetExhaustion = false;
        const middleGradesPlannedSpecificQueriesUnattemptedAtTimeout = [];
        const middleGradesTargetedQueriesAttempted = [];
        let middleGradesTargetedQueriesAcceptedCount = 0;
        const middleGradesTargetedQueriesRejectedByReason = {};
        let middleGradesDocsReturnedButAllDropped = 0;
        const middleGradesAllDroppedContinuationQuery = [];
        const middleGradesReliableVariantAttempted = [];
        ...
        const middleGradesFetchMode = "staggered";
        ...
        const middleGradesSameFamilyContinuationQueriesAttempted = [];
        const middleGradesSameFamilyContinuationQuerySet = new Set();
        let middleGradesRawRejectedButContinuationSkippedReason;
        let middleGradesUnderfilledAfterDirectUsableDocs = false;
        let middleGradesDirectUsableDocsButRecoveryContinued = false;
        let middleGradesUnderfillStopReasonDetailed;
        ...
        let middleGradesActualRemainingBudgetBeforeRecoveryMs;
        let middleGradesBroadFallbackStartedBeforeTargetedExhaustion = false;
        let middleGradesFinalUnderfillTargetedExhaustionReason;
        let teenTimeoutCircuitBreakerStage;
        let middleGradesTimeoutCircuitBreakerStage;
        let k2TimeoutCircuitBreakerStage;
        ...
        const middleGradesEarlyCandidateCapSuppressedTitles = [];
        const middleGradesMediumStrongRawItems = () => ageProfile.key === "middleGrades"
            ? rawItems.filter((item) => middleGradesSourceMediumStrongEvidence(item?.rawOpenLibraryDoc || item, String(item?.queryText || ""), String(item?.routingReason || "")))
            : [];
        const middleGradesMeaningfulTasteExpandedPoolItems = () => {
            if (ageProfile.key !== "middleGrades")
                return [];
            const byKey = new Map();
            for (const item of [...middleGradesExpandedScoringPool, ...rawItems]) {
            ...
            return Array.from(byKey.values()).filter((item) => middleGradesSourceMeaningfulTasteEligibility(item?.rawOpenLibraryDoc || item, context.profile).allowed);
        };
        const middleGradesAttemptedQueries = () => new Set(fetches.filter((fetch) => !fetch.diagnosticOnly).map((fetch) => String(fetch.query || "").toLowerCase()).filter(Boolean));
        const teenTimeoutLikeFetch = (fetch) => Boolean(fetch.timedOut
            || /timeout|openlibrary_http_502/i.test(String(fetch.failedReason || "")));
        const teenConsecutiveTimeoutLikeFetchCount = () => {
            if (ageProfile.key !== "teen")
                return 0;
            let count = 0;
            const realFetches = fetches.filter((fetch) => !fetch.diagnosticOnly);
            for (let index = realFetches.length - 1; index >= 0; index -= 1) {
                if (!teenTimeoutLikeFetch(realFetches[index]))
                    break;
                count += 1;
            }
            return count;
        };
        const teenTimeoutCircuitOpen = (stage) => {
            if (ageProfile.key !== "teen")
                return false;
            if (teenConsecutiveTimeoutLikeFetchCount() < TEEN_OPEN_LIBRARY_TIMEOUT_CIRCUIT_BREAKER_LIMIT)
                return false;
        ...
        const middleGradesTimeoutLikeFetch = (fetch) => Boolean(fetch.timedOut
            || /timeout|openlibrary_http_502/i.test(String(fetch.failedReason || "")));
        const middleGradesConsecutiveTimeoutLikeFetchCount = () => {
            if (ageProfile.key !== "middleGrades")
                return 0;
            let count = 0;
            const realFetches = fetches.filter((fetch) => !fetch.diagnosticOnly);
            for (let index = realFetches.length - 1; index >= 0; index -= 1) {
                if (!middleGradesTimeoutLikeFetch(realFetches[index]))
                    break;
                count += 1;
            }
            return count;
        };
        const middleGradesTimeoutCircuitOpen = (stage) => {
            if (ageProfile.key !== "middleGrades")
                return false;
            if (middleGradesConsecutiveTimeoutLikeFetchCount() < MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CIRCUIT_BREAKER_LIMIT)
                return false;
            ...
            if (!middleGradesRecoveryExhaustionReasonDetailed)
                middleGradesRecoveryExhaustionReasonDetailed = `timeout_circuit_breaker:${stage}`;
            return true;
        };
        const k2TimeoutLikeFetch = (fetch) => Boolean(fetch.timedOut
            || /timeout|openlibrary_http_502/i.test(String(fetch.failedReason || "")));
        const k2ConsecutiveTimeoutLikeFetchCount = () => {
            if (ageProfile.key !== "k2")
                return 0;
            let count = 0;
            const realFetches = fetches.filter((fetch) => !fetch.diagnosticOnly);
            for (let index = realFetches.length - 1; index >= 0; index -= 1) {
                if (!k2TimeoutLikeFetch(realFetches[index]))
                    break;
                count += 1;
            }
            return count;
        };
        const k2TimeoutCircuitOpen = (stage) => {
            if (ageProfile.key !== "k2")
                return false;
            if (k2ConsecutiveTimeoutLikeFetchCount() < K2_OPEN_LIBRARY_TIMEOUT_CIRCUIT_BREAKER_LIMIT)
                return false;
        ...
        const middleGradesUnattemptedSpecificQueries = () => ageProfile.key === "middleGrades" ? middleGradesQueryOnlyContinuationQueries(queryPlans, middleGradesAttemptedQueries()) : [];
        const middleGradesUnattemptedEvidenceAwareQueries = () => ageProfile.key === "middleGrades" ? middleGradesEvidenceAwareRecoveryQueries(queryPlans, context.profile, middleGradesAttemptedQueries()) : [];
        const middleGradesUnattemptedTargetedQueries = () => ageProfile.key === "middleGrades" ? middleGradesTargetedQueriesForRun.filter((query) => !middleGradesAttemptedQueries().has(query.toLowerCase())) : [];
        ...
        const middleGradesUnattemptedReliableVariantQueries = () => ageProfile.key === "middleGrades" ? (middleGradesTargetedPlanForRun?.reliableVariantQueries || []).filter((query) => !middleGradesAttemptedQueries().has(query.toLowerCase())) : [];
        const middleGradesPlannedSpecificQueries = () => ageProfile.key === "middleGrades" ? uniqueStrings([
        ...
        const middleGradesUnattemptedPlannedSpecificQueries = () => {
        ...
        const rememberMiddleGradesUnattemptedAtTimeout = () => {
            if (ageProfile.key !== "middleGrades")
                return;
            for (const query of middleGradesUnattemptedPlannedSpecificQueries()) {
                if (!middleGradesPlannedSpecificQueriesUnattemptedAtTimeout.includes(query))
                    middleGradesPlannedSpecificQueriesUnattemptedAtTimeout.push(query);
            }
        };
        const shouldAcceptMiddleGradesSourceDoc = (doc, query, queryPlan, stage) => {
            if (ageProfile.key !== "middleGrades")
                return true;
            if (queryPlan.emergencyFallback || queryPlan.fallbackAlignment === "anti_zero")
                return true;
            ...
            const title = String(doc?.title || "").trim();
            if (title)
                artifactSuppressedTitles.push(title);
            middleGradesRejectedAllRowsAsQueryOnly = true;
            if (middleGradesQueryOnlyFirstRejectionFetchCount === undefined)
                middleGradesQueryOnlyFirstRejectionFetchCount = fetches.filter((fetch) => !fetch.diagnosticOnly).length;
        ...
        const rememberRecoveryEarlyFinalGateRejection = (reason, title) => {
        ...
        const middleGradesRecoveryLikelyFinalRejectionReason = (doc, query, queryPlan, meaningfulTasteSignals) => {
            const title = String(doc?.title || "").trim();
            ...
            if (/\b(funny|humor|humour|comedy|comic|joke|laugh|giggle)\b/i.test(titleText) && !fictionAgeEvidence && !nonHumorEvidence)
                return "humor_keyword_only_leakage";
            if (routeFields.length === 0)
                return "recovery_query_quality_query_only_cap";
            if (routeFields.every((field) => field === "title" || field === "subtitle") && meaningfulTasteSignals.length <= 1 && /\b(funny|humor|humour|comedy|comic|joke|laugh|giggle)\b/i.test(titleText))
                return "humor_keyword_only_leakage";
            if (routeFields.every((field) => field === "title" || field === "subtitle") && meaningfulTasteSignals.every((signal) => signal === "adventure"))
                return "broad_adventure_only_taste_match";
            return undefined;
        };
        const reserveCollectionRoot = (doc, title, stage) => {
            if (ageProfile.key !== "middleGrades")
                return true;
        ...
        const rememberMiddleGradesAntiZeroFallbackShape = (fallback) => {
            if (!fallback)
                return;
        ...
        const markMiddleGradesFallbackStarted = () => {
            if (middleGradesFallbackStartedOnlyAfterProfileQueriesExhausted !== undefined)
                return;
        ...
        const traceMiddleGradesFetch = (diagnostic, stoppedReason) => {
        ...
        const traceMiddleGradesRawDoc = (doc, query, queryPlan, stage, accepted, reason) => {
            if (!middleGradesDebugTrace)
                return;
            const title = String(doc?.title || "").trim();
        ...
        const rememberMiddleGradesExpandedScoringCandidate = (doc, queryPlan, stage, meaningfulTasteSignals = []) => {
            if (ageProfile.key !== "middleGrades" || !debugMiddleGradesDeepTrace)
                return;
            const title = String(doc?.title || "").trim();
            if (!title)
                return;
            ...
            if (middleGradesExpandedScoringKeys.has(key))
                return;
            middleGradesExpandedScoringKeys.add(key);
            const normalized = normalizeOpenLibraryDoc(doc, queryPlan);
            normalized.scoringHandoffStage = stage;
            normalized.scoringHandoffSource = "expanded_debug_pool";
            if (stage === "meaningful_taste_recovery")
                normalized.meaningfulTasteRecovery = true;
            if (meaningfulTasteSignals.length) {
                normalized.meaningfulTasteRecoveryDocumentSignals = meaningfulTasteSignals;
                normalized.themes = uniqueStrings([...(Array.isArray(normalized.themes) ? normalized.themes : []), ...meaningfulTasteSignals], 24);
            }
            middleGradesExpandedScoringPool.push(normalized);
        };
        const traceMiddleGradesNormalizedCandidate = (item, sourceDoc, queryPlan) => {
            if (!middleGradesDebugTrace)
                return;
        ...
        const recordExpansionDroppedBeforeScoring = (reason, title) => {
            if (!forceMiddleGradesCleanCandidateShortfallExpansion)
                return;
            ...
            if (!titles.includes(title) && titles.length < 50)
                titles.push(title);
            expansionDroppedBeforeScoringTitles[normalizedReason] = titles;
        };
        const acceptMiddleGradesDoc = (doc, queryPlan, stage, requireMeaningfulTaste = false) => {
            ...
            if (!quality.keep) {
                ...
                if (middleGradesTargetedQuerySet.has(query.toLowerCase()))
                    middleGradesTargetedQueriesRejectedByReason[reason] = Number(middleGradesTargetedQueriesRejectedByReason[reason] || 0) + 1;
                if (artifactReasonLabels.has(reason) || /artifact|inappropriate|middle_grades_age_shape/.test(reason))
                    artifactSuppressedTitles.push(title);
                recordExpansionDroppedBeforeScoring(reason, title);
                traceMiddleGradesRawDoc(doc, query, queryPlan, stage, false, reason);
                return false;
            }
            let meaningfulTasteSignals = [];
            if (requireMeaningfulTaste) {
                ...
                if (!tasteEligibility.allowed && !cleanExpansionStage) {
                    ...
                    if (stage === "meaningful_taste_recovery")
                        rememberRecoveryEarlyFinalGateRejection(reason, title);
                    recordExpansionDroppedBeforeScoring(reason, title);
                    traceMiddleGradesRawDoc(doc, query, queryPlan, stage, false, reason);
                    return false;
                }
            ...
            if (acceptedDocKeys.has(docKey)) {
                ...
                if (stage === "meaningful_taste_recovery")
                    rememberRecoveryEarlyFinalGateRejection("duplicate_doc", title);
            ...
            if (!cleanExpansionStage && seriesKey && acceptedSeriesKeys.has(seriesKey)) {
                ...
                if (stage === "meaningful_taste_recovery")
                    rememberRecoveryEarlyFinalGateRejection("duplicate_root_or_series", title);
            ...
            if (!cleanExpansionStage && !shouldAcceptMiddleGradesSourceDoc(doc, query, queryPlan, stage)) {
                if (middleGradesTargetedQuerySet.has(query.toLowerCase()))
                    middleGradesTargetedQueriesRejectedByReason.query_only_or_missing_document_evidence = Number(middleGradesTargetedQueriesRejectedByReason.query_only_or_missing_document_evidence || 0) + 1;
                if (stage === "meaningful_taste_recovery")
                    rememberRecoveryEarlyFinalGateRejection("recovery_query_quality_query_only_cap", title);
            ...
            if (seriesKey && !cleanExpansionStage)
                acceptedSeriesKeys.add(seriesKey);
            acceptedDocKeys.add(docKey);
            const normalized = normalizeOpenLibraryDoc(doc, queryPlan);
            if (stage === "meaningful_taste_recovery")
                normalized.meaningfulTasteRecovery = true;
            if (cleanExpansionStage) {
                normalized.cleanCandidateShortfallExpansion = true;
                normalized.scoringHandoffStage = "clean_candidate_shortfall_expansion";
                normalized.scoringHandoffSource = "clean_candidate_shortfall_expansion";
            }
            if (meaningfulTasteSignals.length) {
                normalized.meaningfulTasteRecoveryDocumentSignals = meaningfulTasteSignals;
                normalized.themes = uniqueStrings([...(Array.isArray(normalized.themes) ? normalized.themes : []), ...meaningfulTasteSignals], 24);
        ...
        for (const [queryPlanIndex, queryPlan] of queryPlans.entries()) {
            ...
            if (ageProfile.key === "teen" && teenTimeoutCircuitOpen("main_query"))
                break;
            if (ageProfile.key === "k2" && k2TimeoutCircuitOpen("main_query"))
                break;
            ...
            const teenDistributedSpecificTimeoutMs = teenTimeoutCascadeRemainingMs !== undefined && teenSpecificTimeoutCascadeRemainingQueries > 0
                ? Math.min(teenTimeoutCascadeRemainingMs, Math.max(TEEN_OPEN_LIBRARY_TIMEOUT_CASCADE_SPECIFIC_QUERY_FLOOR_MS, Math.min(TEEN_OPEN_LIBRARY_TIMEOUT_CASCADE_SPECIFIC_QUERY_CAP_MS, Math.floor(teenTimeoutCascadeRemainingMs / teenSpecificTimeoutCascadeRemainingQueries))))
                : undefined;
            ...
            const middleGradesDistributedTimeoutMs = middleGradesTimeoutCascadeRemainingMs !== undefined && middleGradesTimeoutCascadeRemainingQueries > 0
                ? Math.min(middleGradesTimeoutCascadeRemainingMs, Math.max(MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_FLOOR_MS, Math.min(MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_CAP_MS, Math.floor(middleGradesTimeoutCascadeRemainingMs / middleGradesTimeoutCascadeRemainingQueries))))
                : undefined;
            ...
            if (middleGradesQueryIsProfileSpecific)
                middleGradesProfileSpecificQueriesAttempted.push(query);
            if (middleGradesQueryIsTargeted) {
                if (middleGradesUnderfilledAfterDirectUsableDocs && rawItems.length < Math.min(ageProfile.docLimit, 5))
                    middleGradesDirectUsableDocsButRecoveryContinued = true;
                if (!middleGradesTargetedQueriesAttempted.includes(query))
                    middleGradesTargetedQueriesAttempted.push(query);
                ...
                if (middleGradesReliableVariantQuerySet.has(query.toLowerCase()) && !middleGradesReliableVariantAttempted.includes(query)) {
                    ...
                    if (middleGradesFirstBatchFamily && middleGradesTargetedPlanForRun?.familyByQuery?.[query] === middleGradesFirstBatchFamily)
                        middleGradesFirstBatchReliableVariantUsed = true;
                }
                if (!middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(query))
                    middleGradesEvidenceAwareRecoveryQueriesAttempted.push(query);
                middleGradesEvidenceAwareRecoveryAttempted = true;
                if (middleGradesTimedOutFetchCount > 0 && /\b(children|chapter book)\b/i.test(query))
                    middleGradesBrittleQueryTimedOutThenShortQueryAttempted = true;
            }
            ...
            if (diagnostic.timedOut) {
                if (ageProfile.key === "middleGrades") {
                    rememberMiddleGradesUnattemptedAtTimeout();
                    if (middleGradesQueryIsTargeted && middleGradesFirstBatchFamily && middleGradesTargetedPlanForRun?.familyByQuery?.[query] === middleGradesFirstBatchFamily)
                        middleGradesFirstBatchSpecificQueryTimedOutCount += 1;
                }
                if (middleGradesQueryIsProfileSpecific)
                    middleGradesProfileSpecificQueriesTimedOut += 1;
                ...
                if (ageProfile.key === "teen" && teenTimeoutCircuitOpen("main_query"))
                    break;
                if (ageProfile.key === "middleGrades" && middleGradesTimeoutCircuitOpen("main_query"))
                    break;
                if (ageProfile.key === "k2" && k2TimeoutCircuitOpen("main_query"))
                    break;
                    ...
                    for (const fallbackQuery of ["mystery thriller", "crime fiction", "historical mystery", "romantic suspense"].filter((query) => !attemptedMainQueries.has(query.toLowerCase()))) {
                        ...
                        for (const doc of fallbackDocs) {
                            ...
                            if (!quality.keep) {
                                ...
                                dropReasons[reason] = Number(dropReasons[reason] || 0) + 1;
                                if (/artifact|inappropriate/.test(reason))
                                    artifactSuppressedTitles.push(title);
                                continue;
                            }
                            ...
                            if (seriesKey)
                                acceptedSeriesKeys.add(seriesKey);
                            acceptedDocKeys.add(docKey);
                            if (!shouldAcceptMiddleGradesSourceDoc(doc, fallbackQuery, fallbackPlan, "fallback"))
                                continue;
                            ...
                            if (rawItems.length >= Math.min(ageProfile.docLimit, 5))
                                break;
                        }
                        const timeoutRecoveryTarget = adultPrimaryQueryTimedOutTwice ? Math.min(ageProfile.docLimit, 5) : 3;
                        if (rawItems.length >= timeoutRecoveryTarget || rawItems.length >= Math.min(ageProfile.docLimit, 5))
                            break;
                    }
                    if (rawItems.length > 0)
                ...
                if (rawItems.length >= timeoutRecoveryTarget || rawItems.length >= middleGradesCandidatePoolLimit)
                    break;
            ...
            for (const doc of docs) {
                if (ageProfile.key === "middleGrades") {
                    ...
                    if (accepted) {
                        if (middleGradesQueryIsTargeted) {
                            ...
                            if (middleGradesReliableVariantQuerySet.has(query.toLowerCase()))
                                middleGradesReliableVariantAcceptedCount += 1;
                            if (middleGradesFirstBatchParallelQueries.map((candidate) => candidate.toLowerCase()).includes(query.toLowerCase()))
                                middleGradesFirstBatchParallelAcceptedCount += 1;
                        }
                        if (middleGradesRejectedAllRowsAsQueryOnly) {
                            if (!middleGradesContinuedAfterQueryOnlyRejectionQueries.includes(query))
                                middleGradesContinuedAfterQueryOnlyRejectionQueries.push(query);
                            middleGradesContinuedAfterQueryOnlyRejectionAcceptedCount += 1;
                        }
                        if (rawItems.length >= middleGradesCandidatePoolLimit)
                            break;
                    }
                    continue;
                }
                ...
                if (!quality.keep) {
                    ...
                    if (middleGradesQueryIsTargeted)
                        middleGradesTargetedQueriesRejectedByReason[reason] = Number(middleGradesTargetedQueriesRejectedByReason[reason] || 0) + 1;
                    if (artifactReasonLabels.has(reason))
                        artifactSuppressedTitles.push(title);
                    continue;
                }
                const docKey = String(doc?.key || doc?.cover_edition_key || doc?.edition_key?.[0] || `${title}:${Array.isArray(doc?.author_name) ? doc.author_name[0] : ""}`).toLowerCase();
                ...
                if (!reserveCollectionRoot(doc, title, "main"))
                    continue;
                ...
                if (rawItems.length >= acceptTarget)
                    break;
            }
            if (ageProfile.key === "middleGrades" && docs.length > 0 && rawItems.length === acceptedBeforeThisQuery) {
                ...
                if (diagnostic.fetchPath === "direct")
                    middleGradesDirectFetchReturnedRawButAllRejected += 1;
                ...
                if (continuation) {
                    ...
                    if (!middleGradesAllDroppedContinuationQuery.includes(continuation))
                        middleGradesAllDroppedContinuationQuery.push(continuation);
                }
                else {
            ...
            if (rawItems.length >= cleanDocTarget || rawItems.length >= middleGradesCandidatePoolLimit)
                break;
            if (rawItems.length > 0) {
                openLibraryTopUpRan = true;
        ...
        if (queryPlans[0]?.routingReason === "adult_fantasy_historical_survival" && rawItems.length < 3 && !context.signal?.aborted) {
            ...
            for (const fallbackQuery of fallbackQueries) {
                ...
                if (rawItems.length >= 3 || rawItems.length >= Math.min(ageProfile.docLimit, 5))
                    break;
            }
        ...
        if (ageProfile.key === "teen" && rawItems.length > 0 && rawItems.length < Math.min(ageProfile.docLimit, 5) && !context.signal?.aborted) {
            ...
            for (const recoveryQuery of recoveryQueries) {
                if (teenTimeoutCircuitOpen("underfill_recovery"))
                    break;
                ...
                const recoveryTimeoutMs = Math.max(500, Math.min(3000, remainingBudgetMs));
                const recoveryPlan = {
                    query: recoveryQuery,
                    originalPlannedQuery: queries[0] || "",
                    queryCascadeIndex: queryPlans.length + fetches.filter((fetch) => !fetch.diagnosticOnly).length,
                ...
                for (const doc of recoveryDocs) {
                    ...
                    if (seriesKey)
                        acceptedSeriesKeys.add(seriesKey);
                    acceptedDocKeys.add(docKey);
                    if (!shouldAcceptMiddleGradesSourceDoc(doc, recoveryQuery, recoveryPlan, "recovery"))
                        continue;
        ...
        if (ageProfile.key === "middleGrades" && rawItems.length === 0 && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("delayed_final_retry")) {
            ...
            if (delayedRetryIsAntiZeroFallback)
                rememberMiddleGradesAntiZeroFallbackShape(signalShapedDelayedRetryFallback);
            if (allAttemptedLaneQueriesTimedOut && delayedRetryQuery && !middleGradesTimeoutCircuitOpen("delayed_final_retry")) {
                ...
                if (!debugMiddleGradesDeepTrace && delayedRetryRemainingBudgetMs < MIDDLE_GRADES_OPEN_LIBRARY_DELAYED_RETRY_MIN_BUDGET_MS) {
                    ...
                    const reserveFinalSafeRecoveryBudgetMs = !debugMiddleGradesDeepTrace && finalSafeFallbackAfterDelayedRetry && !attemptedAfterDelayedRetry.has(finalSafeFallbackAfterDelayedRetry.toLowerCase())
                        ? Math.min(MIDDLE_GRADES_OPEN_LIBRARY_FINAL_SAFE_RECOVERY_MIN_BUDGET_MS, Math.max(0, delayedRetryRemainingBudgetMs - MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_FLOOR_MS))
                        : 0;
                    ...
                    const delayedRetryBudgetAfterReserveMs = Math.max(MIDDLE_GRADES_OPEN_LIBRARY_TIMEOUT_CASCADE_QUERY_FLOOR_MS, delayedRetryRemainingBudgetMs - reserveFinalSafeRecoveryBudgetMs);
                    ...
                    if (diagnostic.timedOut) {
                        if (delayedRetryIsAntiZeroFallback)
                            middleGradesFallbackOutcomes.push(`${delayedRetryQuery}:timed_out`);
                        ...
                        if (delayedRetryIsAntiZeroFallback)
                            middleGradesFallbackOutcomes.push(`${delayedRetryQuery}:failed:${diagnostic.failedReason}`);
                        ...
                        if (delayedRetryIsAntiZeroFallback)
                            middleGradesFallbackOutcomes.push(`${delayedRetryQuery}:succeeded:${delayedRetryDocs.length}`);
                        ...
                        for (const doc of delayedRetryDocs) {
                            ...
                            if (!quality.keep) {
                                ...
                                dropReasons[reason] = Number(dropReasons[reason] || 0) + 1;
                                if (/artifact|inappropriate|middle_grades_age_shape/.test(reason))
                                    artifactSuppressedTitles.push(title);
                                continue;
                            }
                            const docKey = String(doc?.key || doc?.cover_edition_key || doc?.edition_key?.[0] || `${title}:${Array.isArray(doc?.author_name) ? doc.author_name[0] : ""}`).toLowerCase();
                            ...
                            if (seriesKey)
                                acceptedSeriesKeys.add(seriesKey);
                            acceptedDocKeys.add(docKey);
                            if (!shouldAcceptMiddleGradesSourceDoc(doc, delayedRetryQuery, delayedRetryPlan, "delayed_retry"))
                                continue;
        ...
        if (ageProfile.key === "middleGrades" && !forceMiddleGradesCleanCandidateShortfallExpansion && rawItems.length < Math.min(ageProfile.docLimit, 5) && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("underfill_recovery")) {
            ...
            const recoveryTarget = Math.min(ageProfile.docLimit, 5);
            for (const recoveryQuery of recoveryQueries) {
                if (middleGradesTimeoutCircuitOpen("underfill_recovery"))
                    break;
                const elapsedBeforeRecoveryMs = Date.now() - Date.parse(startedAt);
                const remainingBudgetMs = sourceBudgetMs - elapsedBeforeRecoveryMs;
                ...
                for (const doc of recoveryDocs) {
                    ...
                    rawItems.push(normalizeOpenLibraryDoc(doc, recoveryPlan));
                    if (middleGradesRejectedAllRowsAsQueryOnly) {
                        if (!middleGradesContinuedAfterQueryOnlyRejectionQueries.includes(recoveryQuery))
                            middleGradesContinuedAfterQueryOnlyRejectionQueries.push(recoveryQuery);
                        middleGradesContinuedAfterQueryOnlyRejectionAcceptedCount += 1;
                    }
                ...
                if (diagnostic.timedOut) {
                    ...
                    for (const doc of finalSafeDocs) {
                        ...
                        if (seriesKey)
                            acceptedSeriesKeys.add(seriesKey);
                        acceptedDocKeys.add(docKey);
                        if (!shouldAcceptMiddleGradesSourceDoc(doc, finalSafeQuery, finalSafePlan, "final_safe_recovery"))
                            continue;
        ...
        if (ageProfile.key === "middleGrades" && debugMiddleGradesDeepTrace && rawItems.length > 0 && middleGradesMediumStrongRawItems().length < middleGradesMediumStrongEvidenceTargetCount && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("medium_strong_evidence_search")) {
            ...
            for (const evidenceQuery of evidenceQueries) {
                if (middleGradesTimeoutCircuitOpen("medium_strong_evidence_search"))
                    break;
                if (middleGradesMediumStrongRawItems().length >= middleGradesMediumStrongEvidenceTargetCount)
                    break;
                if (!middleGradesMediumStrongEvidenceQueriesAttempted.includes(evidenceQuery))
                    middleGradesMediumStrongEvidenceQueriesAttempted.push(evidenceQuery);
                if (!middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(evidenceQuery))
                    middleGradesEvidenceAwareRecoveryQueriesAttempted.push(evidenceQuery);
                const evidencePlan = {
                    query: evidenceQuery,
                    originalPlannedQuery: queries[0] || "",
                    queryCascadeIndex: queryPlans.length + fetches.filter((fetch) => !fetch.diagnosticOnly).length,
                ...
                for (const doc of evidenceDocs) {
                    ...
                    if (!shouldAcceptMiddleGradesSourceDoc(doc, evidenceQuery, evidencePlan, "medium_strong_evidence_search"))
                        continue;
                    if (!reserveCollectionRoot(doc, title, "medium_strong_evidence_search"))
                        continue;
                    if (seriesKey)
                        acceptedSeriesKeys.add(seriesKey);
                    acceptedDocKeys.add(docKey);
                    ...
                    if (middleGradesMediumStrongRawItems().length >= middleGradesMediumStrongEvidenceTargetCount)
                        break;
                    if (rawItems.length >= middleGradesCandidatePoolLimit && middleGradesMediumStrongRawItems().length >= middleGradesMediumStrongEvidenceTargetCount)
                        break;
                }
            }
            if (middleGradesMediumStrongRawItems().length < middleGradesMediumStrongEvidenceTargetCount) {
        ...
        if (ageProfile.key === "middleGrades" && !forceMiddleGradesCleanCandidateShortfallExpansion && rawItems.length > 0 && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("post_fallback_route_recovery")) {
            const fallbackOnlySoFar = rawItems.every((item) => item?.fallbackAlignment === "anti_zero" || item?.emergencyFallback);
            if (fallbackOnlySoFar) {
                ...
                if (!routeAlignedAfterFallbackQuery) {
                    ...
                    const routeAlignedAfterFallbackPlan = {
                    ...
                    if (diagnostic.timedOut) {
                        ...
                        for (const doc of routeAlignedAfterFallbackDocs) {
                            ...
                            if (seriesKey)
                                acceptedSeriesKeys.add(seriesKey);
                            acceptedDocKeys.add(docKey);
                            if (!shouldAcceptMiddleGradesSourceDoc(doc, routeAlignedAfterFallbackQuery, routeAlignedAfterFallbackPlan, "post_fallback_route_recovery"))
                                continue;
                            if (!reserveCollectionRoot(doc, title, "post_fallback_route_recovery"))
                                continue;
        ...
        if (ageProfile.key === "middleGrades" && !forceMiddleGradesCleanCandidateShortfallExpansion && rawItems.length < Math.min(ageProfile.docLimit, 5) && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("underfill_safe_recovery")) {
            ...
            if (startedUnderfillAtFour && safeRecoveryQueries.length > 0)
                middleGradesUnderfilledAtFourDespiteAlignedCandidates = true;
            if (!safeRecoveryQueries.length)
                middleGradesUnderfillSafeRecoverySkippedReason = "no_unattempted_reliable_underfill_query";
            for (const underfillQuery of safeRecoveryQueries) {
                if (middleGradesTimeoutCircuitOpen("underfill_safe_recovery"))
                    break;
                if (rawItems.length >= underfillTarget || context.signal?.aborted)
                    break;
                ...
                if (middleGradesUnderfilledAfterDirectUsableDocs)
                    middleGradesDirectUsableDocsButRecoveryContinued = true;
                if (startedUnderfillAtFour)
                    middleGradesUnderfillRecoveryAttemptedAfterFour = true;
                ...
                if (underfillQueryIsTargeted) {
                    if (!middleGradesTargetedQueriesAttempted.includes(underfillQuery))
                        middleGradesTargetedQueriesAttempted.push(underfillQuery);
                    if (!middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(underfillQuery))
                        middleGradesEvidenceAwareRecoveryQueriesAttempted.push(underfillQuery);
                    middleGradesEvidenceAwareRecoveryAttempted = true;
                }
                ...
                for (const doc of underfillDocs) {
                    ...
                    if (!quality.keep) {
                        ...
                        dropReasons[reason] = Number(dropReasons[reason] || 0) + 1;
                        if (underfillQueryIsTargeted)
                            middleGradesTargetedQueriesRejectedByReason[quality.reason || "quality_filter"] = Number(middleGradesTargetedQueriesRejectedByReason[quality.reason || "quality_filter"] || 0) + 1;
                        if (/artifact|inappropriate|middle_grades_age_shape/.test(reason))
                            artifactSuppressedTitles.push(title);
                        continue;
                    }
                    ...
                    if (!reserveCollectionRoot(doc, title, "underfill_safe_recovery"))
                        continue;
                    ...
                    if (middleGradesRejectedAllRowsAsQueryOnly) {
                        if (!middleGradesContinuedAfterQueryOnlyRejectionQueries.includes(underfillQuery))
                            middleGradesContinuedAfterQueryOnlyRejectionQueries.push(underfillQuery);
                        middleGradesContinuedAfterQueryOnlyRejectionAcceptedCount += 1;
                    }
                    middleGradesUnderfillSafeRecoveryAcceptedCount += 1;
                    if (startedUnderfillAtFour)
                        middleGradesUnderfillRecoveryAcceptedAfterFour += 1;
                    ...
                    if (rawItems.length >= underfillTarget)
                        break;
                }
            }
            if (!middleGradesUnderfillSafeRecoverySkippedReason && rawItems.length < underfillTarget && middleGradesUnderfillSafeRecoveryAttempted)
                middleGradesUnderfillSafeRecoverySkippedReason = "reliable_underfill_queries_exhausted";
        }
        if (ageProfile.key === "middleGrades" && !forceMiddleGradesCleanCandidateShortfallExpansion && rawItems.length > 0 && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("lock_quality_retry")) {
            const routeAlignedCountNow = rawItems.filter((item) => item?.fallbackAlignment !== "anti_zero" && !item?.emergencyFallback).length;
            const fallbackOnlyNow = rawItems.length > 0 && routeAlignedCountNow === 0;
            const genericDefaultNow = rawItems.filter((item) => /\b(humor|funny|school story|school adventure|adventure)\b/i.test(String(item?.queryText || item?.queryFamily || "")) && !/\b(friendship|family|contemporary|realistic|mystery|ai|robot|superhero|science|ocean|animal|nature)\b/i.test([item?.title, item?.subtitle, item?.subjects, item?.description].flat().join(" "))).length >= Math.min(4, rawItems.length);
            ...
            if (shouldRetryForLockQuality) {
                ...
                for (const retryQuery of retryQueries) {
                    if (middleGradesTimeoutCircuitOpen("lock_quality_retry"))
                        break;
                    ...
                    if (retryQueryIsTargeted) {
                        if (!middleGradesTargetedQueriesAttempted.includes(retryQuery))
                            middleGradesTargetedQueriesAttempted.push(retryQuery);
                        if (!middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(retryQuery))
                            middleGradesEvidenceAwareRecoveryQueriesAttempted.push(retryQuery);
                        middleGradesEvidenceAwareRecoveryAttempted = true;
                    }
                    const retryPlan = {
                        query: retryQuery,
                    ...
                    if (middleGradesLockQualityRetryAcceptedCount > 0 && rawItems.filter((item) => item?.fallbackAlignment !== "anti_zero" && !item?.emergencyFallback).length >= 3)
                        break;
                }
        ...
        if (ageProfile.key === "middleGrades" && !forceMiddleGradesCleanCandidateShortfallExpansion && (middleGradesRejectedAllRowsAsQueryOnly || middleGradesHadProfileSpecificTimeout || rawItems.length > 0) && rawItems.length < Math.min(ageProfile.docLimit, 5) && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("query_only_continuation")) {
            ...
            if (continuationQueries.length > 0) {
                ...
                if (middleGradesHadProfileSpecificTimeout && evidenceAwareQueries.length > 0)
                    middleGradesBrittleQueryTimedOutThenShortQueryAttempted = true;
                if (middleGradesRejectedAllRowsAsQueryOnly)
                    middleGradesQueryOnlyRejectionTriggeredContinuation = true;
                ...
                for (const continuationQuery of continuationQueries) {
                    if (middleGradesTimeoutCircuitOpen("query_only_continuation"))
                        break;
                    if (rawItems.length >= continuationTarget)
                        break;
                    ...
                    if (!middleGradesContinuedAfterQueryOnlyRejectionQueries.includes(continuationQuery))
                        middleGradesContinuedAfterQueryOnlyRejectionQueries.push(continuationQuery);
                    ...
                    if (continuationQueryIsTargeted) {
                        if (!middleGradesTargetedQueriesAttempted.includes(continuationQuery))
                            middleGradesTargetedQueriesAttempted.push(continuationQuery);
                        if (!middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(continuationQuery))
                            middleGradesEvidenceAwareRecoveryQueriesAttempted.push(continuationQuery);
                        middleGradesEvidenceAwareRecoveryAttempted = true;
                    }
                    if (evidenceAwareQueries.includes(continuationQuery) && !middleGradesEvidenceAwareRecoveryQueriesAttempted.includes(continuationQuery))
                        middleGradesEvidenceAwareRecoveryQueriesAttempted.push(continuationQuery);
                    ...
                    for (const doc of continuationDocs) {
                        ...
                        middleGradesContinuedAfterQueryOnlyRejectionAcceptedCount += 1;
                        if (evidenceAwareQueries.includes(continuationQuery) && !continuationQueryIsTargeted)
                            middleGradesEvidenceAwareRecoveryAcceptedCount += 1;
            ...
            if (!middleGradesRecoveryExhaustionReasonDetailed && rawItems.length < continuationTarget)
                middleGradesRecoveryExhaustionReasonDetailed = "query_only_continuation_exhausted_before_target";
        }
        const forceMiddleGradesMeaningfulTasteRecovery = ageProfile.key === "middleGrades" && Boolean(context.profile.diagnostics?.forceMiddleGradesMeaningfulTasteRecovery);
        if (ageProfile.key === "middleGrades" && debugMiddleGradesDeepTrace && !forceMiddleGradesCleanCandidateShortfallExpansion && !context.signal?.aborted && !middleGradesTimeoutCircuitOpen("meaningful_taste_recovery")) {
            ...
            const expandedPoolCountBeforeRecovery = new Set(([...middleGradesExpandedScoringPool, ...rawItems]
                .map((item) => String(item?.sourceId || item?.key || item?.workKey || `${String(item?.title || "").trim()}:${Array.isArray(item?.authors) ? item.authors[0] : ""}`).toLowerCase())
                .filter(Boolean))).size;
            if (forceMiddleGradesMeaningfulTasteRecovery || (expandedPoolCountBeforeRecovery >= 20 && meaningfulCountBeforeRecovery < meaningfulTarget)) {
                const priorRecoveryDiagnostics = (context.profile.diagnostics?.priorMiddleGradesRecoverySourceDiagnostics || {});
                const priorSelectionRejectedReasons = (context.profile.diagnostics?.priorMiddleGradesRecoveryRejectedReasons || {});
                const sameRunRecoveryFamilyFailures = {
                    ...(priorRecoveryDiagnostics.recoveryQueryFamilyRejectedForLeakageCount || {}),
                    ...middleGradesRecoveryQueryFamilyRejectedForLeakageCount,
                };
                if (Array.isArray(priorSelectionRejectedReasons.humorKeywordOnlyRejectedTitles) && priorSelectionRejectedReasons.humorKeywordOnlyRejectedTitles.length)
                    sameRunRecoveryFamilyFailures.humor = Number(sameRunRecoveryFamilyFailures.humor || 0) + priorSelectionRejectedReasons.humorKeywordOnlyRejectedTitles.length;
                if (Array.isArray(priorSelectionRejectedReasons.adultOrYaHumorLeakageRejectedTitles) && priorSelectionRejectedReasons.adultOrYaHumorLeakageRejectedTitles.length)
                    sameRunRecoveryFamilyFailures.humor = Number(sameRunRecoveryFamilyFailures.humor || 0) + priorSelectionRejectedReasons.adultOrYaHumorLeakageRejectedTitles.length;
                ...
                if (recoveryQueries.length > 0)
                    middleGradesMeaningfulTasteRecoveryTriggered = true;
                for (const recoveryQuery of recoveryQueries) {
                    if (middleGradesTimeoutCircuitOpen("meaningful_taste_recovery"))
                        break;
                    if (!forceMiddleGradesMeaningfulTasteRecovery && middleGradesMeaningfulTasteExpandedPoolItems().length >= meaningfulTarget)
                        break;
                    if (!middleGradesMeaningfulTasteRecoveryQueriesAttempted.includes(recoveryQuery))
                        middleGradesMeaningfulTasteRecoveryQueriesAttempted.push(recoveryQuery);
                    ...
                    if (/\b(middle grade|children)\b/i.test(recoveryQuery) && /\bfiction|series\b/i.test(recoveryQuery))
                        middleGradesRecoveryConcreteFictionQueryUsed = true;
                    const recoveryPlan = {
                        query: recoveryQuery,
                        originalPlannedQuery: queries[0] || "",
                        queryCascadeIndex: queryPlans.length + fetches.filter((fetch) => !fetch.diagnosticOnly).length,
                    ...
                    for (const doc of recoveryDocs) {
                        ...
                        if (accepted) {
                            if (title)
                                middleGradesMeaningfulTasteRecoveryAcceptedTitles.push(title);
                            ...
                            if (/humor|leakage/i.test(reason))
                                middleGradesRecoveryQueryFamilyRejectedForLeakageCount[recoveryAnchor] = Number(middleGradesRecoveryQueryFamilyRejectedForLeakageCount[recoveryAnchor] || 0) + 1;
                        }
                        if (!forceMiddleGradesMeaningfulTasteRecovery && middleGradesMeaningfulTasteExpandedPoolItems().length >= meaningfulTarget)
                            break;
                    }
        ...
        if (rawItems.length === 1 && queryPlans[0]?.routingReason === "dominant_horror_survival_psychological" && !context.signal?.aborted) {
            ...
            const fallbackPlan = { query: fallbackQuery, originalPlannedQuery: queries[0] || "", queryCascadeIndex: queryPlans.length, queryFamily: "horror_survival", facets: ["horror", "survival", "psychological"], emergencyFallback: true, routingReason: "horror_survival_underfill_fallback" };
            ...
            for (const doc of fallbackDocs) {
                ...
                if (rawItems.length >= Math.min(ageProfile.minCleanDocs, 3))
                    break;
            }
        ...
        if (!rawItems.length && !forceMiddleGradesCleanCandidateShortfallExpansion && !context.signal?.aborted && !teenMainQueryTimedOutDuringRun && !middleGradesAllRealFetchesTimedOutDuringRun) {
            ...
            if (diagnostic.timedOut && !failedReason)
                failedReason = diagnostic.failedReason || "openlibrary_probe_timed_out";
            if (probeDocs.length) {
                ...
                for (const doc of probeDocs) {
                    ...
                    if (!shouldAcceptMiddleGradesSourceDoc(doc, probeQuery, probePlan, "emergency_probe"))
                        continue;
                    ...
                    if (rawItems.length >= Math.min(ageProfile.minCleanDocs, 4))
                        break;
                }
        ...
        const middleGradesAntiZeroFallbackSuccessCount = ageProfile.key === "middleGrades"
            ? rawItems.filter((item) => item?.fallbackAlignment === "anti_zero" || item?.emergencyFallback).length
            : undefined;
        ...
        const middleGradesGenericDefaultSlateDetected = ageProfile.key === "middleGrades" && rawItems.length > 0
            ? rawItems.filter((item) => /\b(humor|funny|school story|school adventure|adventure)\b/i.test(String(item?.queryText || item?.queryFamily || "")) && !/\b(friendship|family|contemporary|realistic|mystery|ai|robot|superhero)\b/i.test([item?.title, item?.subtitle, item?.subjects, item?.description].flat().join(" "))).length >= Math.min(4, rawItems.length)
            : undefined;
        ...
        const selectedUniqueRootCount = ageProfile.key === "middleGrades"
            ? new Set(rawItems.map((item) => openLibraryCollectionRootKey(item) || String(item?.title || "").toLowerCase()).filter(Boolean)).size
            : undefined;
        ...
        if (middleGradesDebugTrace) {
            ...
            for (const fetch of fetches) {
                ...
                if (!tracedFetchKeys.has(key))
                    traceMiddleGradesFetch(fetch);
            }
            middleGradesDebugTrace.selectionTrace.push(...rawItems.map((item) => ({
                title: item?.title,
                selected: true,
        ...
        const middleGradesDebugCompactSummary = middleGradesDebugTrace ? {
            ...
            best20SelectedEligibleDocs: rawItems.slice(0, 20).map((item) => ({ title: item?.title, query: item?.queryText, routingReason: item?.routingReason, fallbackAlignment: item?.fallbackAlignment })),
        ...
        const status = rawItems.length ? "succeeded" : (allMainFetchesTimedOut && !middleGradesHasUnattemptedSpecificAfterTimeout) ? "timed_out" : failedReason ? "failed" : "empty";
        ...
        const middleGradesMediumStrongEvidenceAcceptedTitlesForDiagnostics = ageProfile.key === "middleGrades"
            ? uniqueStrings([
                ...
                ...middleGradesMediumStrongRawItems().map((item) => String(item?.title || "")).filter(Boolean),
            ], 20)
            : [];
        const openLibraryScoringHandoffEligiblePool = ageProfile.key === "middleGrades" && debugMiddleGradesDeepTrace
            ? (() => {
                ...
                const pooled = Array.from(byKey.values()).map((item) => {
                    if (!middleGradesMeaningfulTasteRecoveryTriggered)
                        return item;
                ...
                const recoveryItems = pooled.filter((item) => item?.meaningfulTasteRecovery || item?.scoringHandoffStage === "meaningful_taste_recovery");
                const nonRecoveryItems = pooled.filter((item) => !recoveryItems.includes(item));
        ...
        const middleGradesMeaningfulTasteRecoveryItemsInHandoff = ageProfile.key === "middleGrades" && middleGradesMeaningfulTasteRecoveryTriggered
            ? openLibraryScoringHandoffItems.filter((item) => item?.meaningfulTasteRecovery || item?.scoringHandoffStage === "meaningful_taste_recovery")
            : [];
        ...
        return {
            ...
            diagnostics: {
                ...
                queries,
                rawTitles: uniqueStrings(rawTitles, 10),
                firstReturnedTitles: uniqueStrings(openLibraryScoringHandoffItems.map((item) => item?.title), 5),
                ...
                mediumStrongCandidatesSeenAcrossAllQueries: ageProfile.key === "middleGrades" ? middleGradesMediumStrongRawItems().map((item) => String(item?.title || "")).filter(Boolean).slice(0, 50) : undefined,
                weakFallbackCandidatesHeldBack: ageProfile.key === "middleGrades" ? rawItems.filter((item) => !middleGradesSourceMediumStrongEvidence(item?.rawOpenLibraryDoc || item, String(item?.queryText || ""), String(item?.routingReason || ""))).map((item) => String(item?.title || "")).filter(Boolean).slice(0, 50) : undefined,
                ...
                expansionMergedTitles: cleanCandidateShortfallExpansionActive ? uniqueStrings(openLibraryScoringHandoffItems.map((item) => item?.title), 20) : undefined,
                ...
                middleGradesDeepDebugActivationSource: ageProfile.key === "middleGrades" ? middleGradesDeepDebugActivationSource : undefined,
