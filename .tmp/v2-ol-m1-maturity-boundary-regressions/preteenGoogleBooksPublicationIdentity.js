...
exports.preteenGoogleBooksPublicationIdentityAudit = preteenGoogleBooksPublicationIdentityAudit;
exports.annotatePreteenGoogleBooksPublicationIdentity = annotatePreteenGoogleBooksPublicationIdentity;
...
function pushIf(target, condition, evidence) {
    if (condition)
        target.push(evidence);
}
function rawVolumeInfo(candidate) {
    const raw = (candidate.raw || {});
    return raw.volumeInfo && typeof raw.volumeInfo === "object" ? raw.volumeInfo : {};
}
function preteenGoogleBooksMetadata(candidate) {
    const raw = (candidate.raw || {});
...
function preteenGoogleBooksPublicationIdentityAudit(candidate) {
    ...
    pushIf(artifactEvidence, /\blist of books for (?:high[-\s]?school|school) libraries\b/.test(publicationIdentityTitle)
        ...
        || /\binstitutional librar(?:y|ies)\s+(?:book\s+list|catalog(?:ue)?|list)\b/.test(publicationIdentityTitle), "institutional_school_library_book_list_identity");
    pushIf(artifactEvidence, /^(?:[a-z0-9]+\s+){1,6}list\s+of\s+(?:[a-z0-9]+\s+){0,5}(?:books?|novels?|fiction|stories|reads|literature)$/.test(publicationIdentityTitle), "curated_literature_list_identity");
    ...
    if (artifactEvidence.includes("school_publication_identity"))
        identity = "school_publication";
    else if (artifactEvidence.includes("institutional_school_library_book_list_identity"))
        identity = "institutional_library_list";
    else if (artifactEvidence.some((item) => item.includes("sampler")))
        identity = "sampler";
    else if (artifactEvidence.includes("sneak_preview_identity"))
        identity = "sneak_preview";
    else if (artifactEvidence.includes("excerpt_identity"))
        identity = "excerpt";
    else if (artifactEvidence.includes("promotional_material_identity"))
        identity = "promotional_material";
    else if (artifactEvidence.includes("teacher_or_classroom_material_identity"))
        identity = "teacher_resource";
    else if (artifactEvidence.includes("study_guide_identity"))
        identity = "study_guide";
    else if (artifactEvidence.includes("workbook_identity"))
        identity = "workbook";
    else if (artifactEvidence.includes("activity_book_identity"))
        identity = "activity_book";
    else if (artifactEvidence.includes("catalog_identity") || artifactEvidence.includes("curated_literature_list_identity"))
        identity = "catalog";
    else if (artifactEvidence.includes("reference_identity"))
        identity = "reference";
    else if (artifactEvidence.includes("educational_nonfiction_category"))
        identity = "nonfiction";
    else if (narrativeEvidence.includes("manga_metadata"))
        identity = "manga";
    else if (narrativeEvidence.includes("light_novel_metadata"))
        identity = "light_novel";
    else if (narrativeEvidence.includes("graphic_novel_category"))
        identity = "graphic_novel";
    else if (narrativeEvidence.includes("middle_grade_metadata") || narrativeEvidence.includes("juvenile_fiction_category"))
        identity = "middle_grade_novel";
    else if (narrativeEvidence.includes("chapter_book_metadata"))
        identity = "chapter_book";
    else if (narrativeEvidence.length > 0)
        identity = "narrative_book";
    const hardArtifactIdentities = new Set([
        "sampler",
        "sneak_preview",
        "excerpt",
    ...
    const narrativeConfidenceSource = unique(narrativeEvidence.map((item) => {
        ...
        if (/publisher/.test(item))
            return `publisher:${item}`;
        if (/story|fiction|novel/.test(item))
            return `description_or_title:${item}`;
        return item;
    }));
...
function annotatePreteenGoogleBooksPublicationIdentity(candidate, audit = preteenGoogleBooksPublicationIdentityAudit(candidate)) {
