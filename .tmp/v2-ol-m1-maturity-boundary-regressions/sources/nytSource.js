...
exports.nytSourceAdapter = void 0;
...
function getNytApiKey() {
    return String(process.env.NYT_BOOKS_API_KEY
...
const nytRateLimiter = new NytWindowRateLimiter(5, 60000);
const nytDailyCache = new Map();
function todayUtcDate() {
    return new Date().toISOString().slice(0, 10);
}
function getCachedList(listSlug) {
    const entry = nytDailyCache.get(listSlug);
    if (!entry)
        return null;
    if (entry.day !== todayUtcDate()) {
        nytDailyCache.delete(listSlug);
        return null;
    }
    const fetchDiag = {
        query: listSlug,
...
function setCachedList(listSlug, result) {
...
const nytInflight = new Map();
function inferFamilyFromQuery(query) {
    const q = normalizeText(query);
    if (!q)
        return "general";
    if (/\b(psychological thriller|crime thriller|conspiracy thriller|fugitive thriller|manhunt thriller|abduction thriller|thriller)\b/.test(q))
        return "thriller";
    if (/\b(psychological mystery|detective mystery|cold case mystery|mystery)\b/.test(q))
        return "mystery";
    if (/\b(psychological horror|survival horror|haunted|horror)\b/.test(q))
        return "horror";
    if (/\b(science fiction|sci fi|sci-fi|dystopian|space opera|speculative)\b/.test(q))
        return "science_fiction";
    if (/\b(epic fantasy|dark fantasy|magic fantasy|fantasy)\b/.test(q))
        return "fantasy";
    if (/\b(historical fiction|historical novel|period fiction|historical|19th century|victorian|gilded age|civil war)\b/.test(q))
        return "historical";
    if (/\b(romance|love story|second chance romance|gothic romance|historical romance)\b/.test(q))
        return "romance";
    return "general";
}
function nytListsForFamily(family) {
    if (family === "romance") {
...
function selectRequestedLists(plan, _profile) {
    ...
    for (const intent of plan.intents || []) {
        ...
        for (const list of nytListsForFamily(family))
            fromQueries.add(list);
    }
...
function parseNytBooksFromRoot(root, fallbackList) {
    const books = Array.isArray(root?.books) ? root.books : [];
    return books
        .filter((book) => book && (book.title || book.book_title))
        .map((book) => ({
...
function parseNytBooks(payload, fallbackList) {
    return parseNytBooksFromRoot(payload?.results ?? {}, fallbackList);
}
async function fetchAndPopulateFromOverview(requestedLists, apiKey, signal) {
    ...
    const fetchDiag = { query: "__overview__", fetchStartedAt, timedOut: false };
    const doFetch = async () => {
        try {
    ...
    if (result.quotaBlocked) {
        ...
        const waitMs = result.retryAfterMs ?? 15000;
        await sleep(waitMs);
        await nytRateLimiter.waitTurn();
    ...
    const overviewLists = Array.isArray(result.json?.results?.lists) ? result.json.results.lists : [];
...
async function fetchNytList(requestedList, apiKey, signal) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (retryAfterMs)
                fetchDiagnostic.retryAfterMs = retryAfterMs;
...
function inferredMaturityFromAgeGroup(value) {
    const normalized = normalizeText(value);
    if (!normalized)
        return undefined;
    if (normalized.includes("young adult"))
        return "teens";
    ...
    async search(plan, context) {
        ...
        if (uncachedLists.length >= 3) {
            ...
            if (!existingOverviewInflight) {
                nytInflight.set(ifKey, overviewPromise);
            }
            try {
                await overviewPromise;
            }
            finally {
                if (!existingOverviewInflight && nytInflight.get(ifKey) === overviewPromise)
                    nytInflight.delete(ifKey);
            }
        }
        for (const list of requestedLists) {
            ...
            if (runQuotaBlocked) {
                const skippedDiag = { query: list, timedOut: false, status: "failed", failedReason: "quota_blocked_skipped", quotaBlocked: true };
            ...
            if (existingInflight) {
                ...
                if (r.quotaBlocked && !runQuotaBlocked) {
                    runQuotaBlocked = true;
                    runRetryAfterMs = r.retryAfterMs;
                }
                settled.push(r);
                continue;
            }
            ...
            const liveFetch = (async () => {
                ...
                if (r.quotaBlocked) {
                    const waitMs = r.retryAfterMs ?? 15000;
                    await sleep(waitMs);
                    await nytRateLimiter.waitTurn();
            ...
            try {
                ...
                if (nytInflight.get(ifKey) === liveFetch)
                    nytInflight.delete(ifKey);
            }
        }
        const endpointCalledByList = {};
        const httpStatusByList = {};
        const booksPerList = {};
        const cacheHitByList = {};
        const returnedLists = [];
        const fetches = [...overviewFetchDiags];
        let firstFailure = "";
        for (const row of settled) {
            endpointCalledByList[row.requestedList] = row.endpoint;
            if (Number.isFinite(Number(row.httpStatus)))
                httpStatusByList[row.requestedList] = Number(row.httpStatus);
            const returnedList = String(row.returnedListEncoded || row.returnedListName || "").trim();
            if (returnedList)
                returnedLists.push(returnedList);
            ...
            if (!firstFailure && row.failedReason)
                firstFailure = row.failedReason;
        }
