...
function Cover({ book, hidden = false, thumbnail = false }: { book: StoryBook; hidden?: boolean; thumbnail?: boolean }) {
  const [failed, setFailed] = useState(false);
  useEffect(() => setFailed(false), [book.coverUrl]);
  return <View style={[styles.cover, hidden && styles.blurredCover, thumbnail && styles.handCover]} accessible={!hidden} accessibilityElementsHidden={hidden} importantForAccessibility={hidden ? "no-hide-descendants" : "auto"} accessibilityLabel={hidden ? undefined : `Cover of ${book.title}`}>
    {book.coverUrl && !failed ? <Image source={{ uri: book.coverUrl }} blurRadius={hidden ? 8 : 0} onError={() => setFailed(true)} style={[styles.coverImage, hidden && styles.defocusedImage]} accessible={false} /> : hidden ? <View style={styles.secretCover}><Text style={styles.secretMark}>◇</Text></View> : <View style={[styles.coverImage, { backgroundColor: "#899894" }]} />}
