...
import { normalizeUnwrittenMapChoiceEventV1 } from "../lib/recommendationGames/unwrittenMap";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const event = normalizeUnwrittenMapChoiceEventV1(req.body);
  ...
  try {
    await put(
      `recommendation-games/the-unwritten-map/v1/${safePlayer}/${safeEvent}.json`,
