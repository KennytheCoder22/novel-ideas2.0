...
const mappings = [
  ...
  ["melanies-game", "Melanie's Game", "book_logo.png", 'route: "/games/melanies-game"'],
];
...
for (const [id, title, image, route] of mappings) {
  ...
  const imagePath = image === "book_logo.png" ? path.join(root, "assets", image) : path.join(root, "assets", "games", image);
...
assert(source.includes('liveArtwork: "melanies-library"'), "Melanie's Game must use its live cat-and-books portal artwork");
assert(source.includes('name="cat"'), "Melanie's live portal artwork must include the black cat treatment");

const optimizedBytes = mappings.reduce((total, [, , image]) => (
  total + fs.statSync(image === "book_logo.png" ? path.join(root, "assets", image) : path.join(root, "assets", "games", image)).size
), 0);
