...
function execute() {
  return definitions.profiles.map((profile) => characterizeGcdFixture(
    profile,
...
assert(!networkAttempted, "fixture-only GCD characterization must not attempt network access");
equal(stableJson(first), stableJson(second), "GCD characterization must be deterministic");
equal(stableJson(gcdProductionHashes(repoRoot)), stableJson(hashesBefore), "production hashes must remain unchanged");
...
for (const profile of definitions.profiles) {
  ...
  equal(byId[profile.profileId].productionBoundary.productionAdapterState, "adapter_not_implemented", `${profile.profileId} production boundary`);
  equal(byId[profile.profileId].comparison.performed, false, `${profile.profileId} comparison boundary`);
}

const speculative = byId["gcd-adult-speculative-ensemble"];
...
assert(speculative.identity.readingUnitKindHistogram.collected_volume === 3, "Trade and two binding editions must classify as collected volumes");
...
assert(speculative.identity.collapseGroups.some((group) => group.publicationIds.includes("gcd-publication:1001") && group.publicationIds.includes("gcd-publication:1002")), "Variant cover must collapse at readable/recommendation identity");
assert(speculative.identity.collapseGroups.some((group) => group.publicationIds.includes("gcd-publication:1201") && group.publicationIds.includes("gcd-publication:1202")), "Hardcover and paperback must collapse at readable/recommendation identity");
const trade = speculative.records.find((record) => record.sourceRecordIdentity.id === "gcd:1101");
...
const horror = byId["gcd-adult-horror-mystery"];
...
const teenFantasy = byId["gcd-teen-fantasy-adventure"];
assert(teenFantasy.identity.missingSequenceSourceRecordIds.includes("gcd:3001"), "Missing volume numbering must remain unknown");
assert(teenFantasy.identity.incompleteCreatorCreditSourceRecordIds.includes("gcd:3201"), "Incomplete creator credit must remain explicit");
assert(teenFantasy.identity.dateConflictSourceRecordIds.includes("gcd:3101") && teenFantasy.identity.dateConflictSourceRecordIds.includes("gcd:3102"), "Conflicting publication dates must be preserved across equivalent editions");
equal(teenFantasy.metadataCoverage.audienceAuthority.rate, 0, "Teen audience authority limitation");

const superhero = byId["gcd-teen-superhero-identity"];
equal(superhero.identity.ambiguousCount, 1, "Ambiguous issue-versus-collection shape must not be guessed");
assert(superhero.identity.ambiguousSourceRecordIds.includes("gcd:4101"), "Ambiguous source record lineage");
assert(superhero.identity.collapseGroups.some((group) => group.publicationIds.includes("gcd-publication:4001") && group.publicationIds.includes("gcd-publication:4002")), "Superhero variant must collapse without losing publication identity");

const preteen = byId["gcd-preteen-humor-adventure"];
...
const manga = byId["gcd-teen-manga-volume"];
...
for (const artifact of first) {
  ...
  equal(artifact.licensingBoundary.liveMetadataCaptured, false, "no live GCD metadata");
  assert(!/https:\/\/www\.comics\.org\/(issue|series)\//i.test(stableJson(artifact.records)), "fixture records must not contain live GCD entity URLs");
}
