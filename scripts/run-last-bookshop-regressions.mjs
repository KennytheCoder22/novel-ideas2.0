...
const stageArtwork = require(resolve(root, "lib/recommendationGames/lastBookshopStageArtwork.ts"));
...
async function main() {
  ...
  assert(appSource.includes('<BookshopBackdrop phase="title"'), "title screen does not render the environmental artwork");
  assert(appSource.includes('testID="last-bookshop-title-begin"'), "live title action is missing");
  assert(appSource.includes("accessibilityLabel={buttonLabel}"), "title button accessibility label missing");
  assert(appSource.includes('hasProgress ? "Continue the Night" : "Turn the Key'), "saved progress must expose a visible continue treatment");
  assert(!appSource.includes("titleArtworkButton"), "title screen must not use an invisible screenshot hotspot");
  assert(appSource.includes("onError={() => setArtworkFailed(true)}"), "title environment failure fallback missing");
  ...
  const stageArtworkEntries = Object.entries(stageArtwork.LAST_BOOKSHOP_STAGE_ARTWORK);
  assert(stageArtworkEntries.length === 7, "every game phase must resolve to environmental artwork");
  const stageArtworkPaths = stageArtworkEntries.map(([phase, artworkPath]) => {
    assert(typeof artworkPath === "string" && artworkPath.endsWith("-environment.webp"), `${phase} stage artwork mapping missing`);
    assert(existsSync(artworkPath), `${phase} stage artwork asset does not exist`);
    const bytes = readFileSync(artworkPath);
    assert(
      bytes.subarray(0, 4).toString("ascii") === "RIFF"
        && bytes.subarray(8, 12).toString("ascii") === "WEBP",
      `${phase} stage artwork must be WebP`,
    );
    assert(statSync(artworkPath).size < 100_000, `${phase} stage artwork exceeds the web payload budget`);
    return artworkPath;
  });
  assert(new Set(stageArtworkPaths).size === 5, "title, arrival, shelves, counter, and result must use distinct environments");
  assert(appSource.includes("function BookshopBackdrop"), "continuous gameplay artwork backdrop missing");
  assert(appSource.includes("<BookshopBackdrop phase={phase as LastBookshopStageArtwork}"), "gameplay phases do not share the environmental frame");
  assert(appSource.includes("key={phase}"), "each new game stage must reset the shared scroll position");
