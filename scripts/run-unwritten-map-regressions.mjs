...
async function main() {
  ...
  assert(inventory.required === 108 && inventory.approved === 72 && inventory.missing === 36
    && inventory.encounters.approved === 12 && inventory.encounters.missing === 0
    && inventory.choices.approved === 32 && inventory.choices.missing === 16
  ...
  assert(presentationDiagnostics.filter((issue) => issue.code === "missing_required_asset").length === 36,
    "full focal-art audit must stay explicitly blocked until all 36 remaining choice/result assets are supplied");
