...
function buildTeenOpenLibraryQueryPlans(plan: SourcePlan, profile: TasteProfile, ageProfile: OpenLibraryAgeProfile): OpenLibraryQueryPlan[] {
  ...
  const likedFantasyAdventureWeight = likedFantasyAdventureRows.reduce(
    (sum, row) => sum + Math.abs(Number(row.weight || 0)),
    0
  );
  const strongestOtherLikedFamilyWeight = Math.max(
    likedMysteryWeight,
    likedHistoricalWeight,
    likedContemporaryWeight,
    likedRomanceWeight,
    likedHorrorFamilyWeight,
    likedSportsWeight,
  ...
  const independentLikedFamilyWeight = (pattern: RegExp): number => {
    ...
    for (const row of signalRows) {
      ...
      for (const item of evidence) {
        ...
        weightByLikedItem.set(
          key,
          Math.max(
            weightByLikedItem.get(key) || 0,
            Math.abs(Number(row.weight || 0))
          )
        );
      }
    }

    return [...weightByLikedItem.values()].reduce(
...
export const openLibrarySourceAdapter: SourceAdapterV2 = {
  ...
  async search(plan: SourcePlan, context: { profile: TasteProfile; signal?: AbortSignal }): Promise<SourceResult> {
    ...
    const openLibraryScoringHandoffSource: SourceDiagnosticV2["openLibraryScoringHandoffSource"] = ageProfile.key === "middleGrades"
      ...
      : ageProfile.key === "teen"
        ? "teen_diverse_eligible_pool" as unknown as SourceDiagnosticV2["openLibraryScoringHandoffSource"]
        : undefined;
